...
bool btcnew_qt::eventloop_processor::event (QEvent * event_a)
{
	assert (dynamic_cast<btcnew_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<btcnew_qt::eventloop_event *> (event_a)->action ();
	return true;
}

btcnew_qt::eventloop_event::eventloop_event (std::function<void ()> const & action_a) :
QEvent (QEvent::Type::User),
action (action_a)
{
}

btcnew_qt::self_pane::self_pane (btcnew_qt::wallet & wallet_a, btcnew::account const & account_a) :
window (new QWidget),
layout (new QVBoxLayout),
self_layout (new QHBoxLayout),
self_window (new QWidget),
your_account_label (new QLabel ("Your Bitcoin New account:")),
...
wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("%1% %2% network") % BTCNEW_VERSION_STRING % network).c_str ());
...
void btcnew_qt::self_pane::set_balance_text (std::pair<btcnew::uint128_t, btcnew::uint128_t> balance_a)
{
...
btcnew_qt::accounts::accounts (btcnew_qt::wallet & wallet_a) :
...
void btcnew_qt::accounts::refresh_wallet_balance ()
{
	auto transaction (this->wallet.wallet_m->wallets.tx_begin_read ());
...
void btcnew_qt::accounts::refresh ()
{
...
btcnew_qt::import::import (btcnew_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
btcnew_qt::history::history (btcnew::ledger & ledger_a, btcnew::account const & account_a, btcnew_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
...
namespace
{
class short_text_visitor : public btcnew::block_visitor
{
public:
	short_text_visitor (btcnew::transaction const & transaction_a, btcnew::ledger & ledger_a) :
...
void btcnew_qt::history::refresh ()
{
...
btcnew_qt::block_viewer::block_viewer (btcnew_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void btcnew_qt::block_viewer::rebroadcast_action (btcnew::block_hash const & hash_a)
{
	auto done (true);
...
btcnew_qt::account_viewer::account_viewer (btcnew_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this] () {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (BTCNEW): ") + wallet.format_balance (balance.first));
...
btcnew_qt::stats_viewer::stats_viewer (btcnew_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
refresh (new QPushButton ("Refresh")),
...
void btcnew_qt::stats_viewer::refresh_stats ()
{
	model->removeRows (0, model->rowCount ());
...
btcnew_qt::status::status (btcnew_qt::wallet & wallet_a) :
wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (btcnew_qt::status_types::nominal);
	set_text ();
}

void btcnew_qt::status::erase (btcnew_qt::status_types status_a)
{
	assert (status_a != btcnew_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void btcnew_qt::status::insert (btcnew_qt::status_types status_a)
{
	assert (status_a != btcnew_qt::status_types::nominal);
...
std::string btcnew_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case btcnew_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case btcnew_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case btcnew_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case btcnew_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case btcnew_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case btcnew_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case btcnew_qt::status_types::nominal:
...
std::string btcnew_qt::status::color ()
{
	assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
btcnew_qt::wallet::wallet (QApplication & application_a, btcnew_qt::eventloop_processor & processor_a, btcnew::node & node_a, std::shared_ptr<btcnew::wallet> wallet_a, btcnew::account & account_a) :
rendering_ratio (btcnew::Mbtcnew_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
...
void btcnew_qt::wallet::ongoing_refresh ()
{
	std::weak_ptr<btcnew_qt::wallet> wallet_w (shared_from_this ());
...
void btcnew_qt::wallet::start ()
{
	ongoing_refresh ();
	std::weak_ptr<btcnew_qt::wallet> this_w (shared_from_this ());
					...
					if (actual <= balance)
					{
						...
						if (this_l->wallet_m->store.valid_password (transaction))
						{
							...
							this_l->node.background ([this_w, account_l, actual] () {
								if (auto this_l = this_w.lock ())
								{
									this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w] (std::shared_ptr<btcnew::block> block_a) {
										if (auto this_l = this_w.lock ())
										{
	...
	node.observers.blocks.add ([this_w] (btcnew::election_status const & status_a, btcnew::account const & account_a, btcnew::uint128_t const & amount_a, bool) {
		if (auto this_l = this_w.lock ())
		{
	...
	node.observers.account_balance.add ([this_w] (btcnew::account const & account_a, bool is_pending) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a] () {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (btcnew_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (btcnew_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.endpoint.add ([this_w] (std::shared_ptr<btcnew::transport::channel>) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w] () {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a] () {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (btcnew_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (btcnew_qt::status_types::synchronizing);
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working] () {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (btcnew_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (btcnew_qt::status_types::working);
					}
				}
			}));
		}
...
void btcnew_qt::wallet::update_connected ()
{
	if (node.network.empty ())
	{
		active_status.insert (btcnew_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (btcnew_qt::status_types::disconnected);
	}
}

void btcnew_qt::wallet::empty_password ()
{
...
void btcnew_qt::wallet::change_rendering_ratio (btcnew::uint128_t const & rendering_ratio_a)
{
...
std::string btcnew_qt::wallet::format_balance (btcnew::uint128_t const & balance) const
{
	auto balance_str = btcnew::amount (balance).format_balance (rendering_ratio, 3, false);
	auto unit = std::string ("BTCNEW");
	if (rendering_ratio == btcnew::kbtcnew_ratio)
	{
		unit = std::string ("kbtcnew");
	}
	else if (rendering_ratio == btcnew::btcnew_ratio)
	{
		unit = std::string ("btcnew");
	}
	else if (rendering_ratio == btcnew::raw_ratio)
	{
		unit = std::string ("raw");
	}
...
void btcnew_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void btcnew_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

btcnew_qt::settings::settings (btcnew_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
password (new QLineEdit),
...
wallet (wallet_a)
{
	...
	QObject::connect (change_rep, &QPushButton::released, [this] () {
		btcnew::account representative_l;
...
void btcnew_qt::settings::refresh_representative ()
{
...
void btcnew_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void btcnew_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (btcnew_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (btcnew_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (btcnew_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (btcnew_qt::status_types::vulnerable);
	}
}

btcnew_qt::advanced_actions::advanced_actions (btcnew_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
mbtcnew_unit (new QRadioButton ("Mbtcnew")),
kbtcnew_unit (new QRadioButton ("kbtcnew")),
btcnew_unit (new QRadioButton ("btcnew")),
...
wallet (wallet_a)
{
	ratio_group->addButton (mbtcnew_unit);
	ratio_group->addButton (kbtcnew_unit);
	ratio_group->addButton (btcnew_unit);
	ratio_group->addButton (raw_unit);
	ratio_group->setId (mbtcnew_unit, 0);
	ratio_group->setId (kbtcnew_unit, 1);
	ratio_group->setId (btcnew_unit, 2);
	ratio_group->setId (raw_unit, 3);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (mbtcnew_unit);
	scale_layout->addWidget (kbtcnew_unit);
	scale_layout->addWidget (btcnew_unit);
	...
	QObject::connect (mbtcnew_unit, &QRadioButton::toggled, [this] () {
		if (mbtcnew_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (mbtcnew_unit));
			this->wallet.change_rendering_ratio (btcnew::Mbtcnew_ratio);
		}
	});
	QObject::connect (kbtcnew_unit, &QRadioButton::toggled, [this] () {
		if (kbtcnew_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (kbtcnew_unit));
			this->wallet.change_rendering_ratio (btcnew::kbtcnew_ratio);
		}
	});
	QObject::connect (btcnew_unit, &QRadioButton::toggled, [this] () {
		if (btcnew_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (btcnew_unit));
			this->wallet.change_rendering_ratio (btcnew::btcnew_ratio);
		}
	});
	QObject::connect (raw_unit, &QRadioButton::toggled, [this] () {
		if (raw_unit->isChecked ())
		{
			...
			this->wallet.change_rendering_ratio (btcnew::raw_ratio);
		}
	});
	auto selected_ratio_id (QSettings ().value (saved_ratio_key, ratio_group->id (mbtcnew_unit)).toInt ());
	...
	QObject::connect (peers_bootstrap, &QPushButton::released, [this] () {
		...
		auto error (btcnew::parse_endpoint (bootstrap_line->text ().toStdString (), endpoint));
		if (!error)
		{
...
void btcnew_qt::advanced_actions::refresh_peers ()
{
...
void btcnew_qt::advanced_actions::refresh_ledger ()
{
	...
	for (auto i (wallet.node.ledger.store.latest_begin (transaction)), j (wallet.node.ledger.store.latest_end ()); i != j; ++i)
	{
		...
		std::string balance;
		btcnew::amount (info.balance.number () / wallet.rendering_ratio).encode_dec (balance);
...
void btcnew_qt::advanced_actions::refresh_stats ()
{
	wallet.stats_viewer.refresh_stats ();
}

btcnew_qt::block_entry::block_entry (btcnew_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
block (new QPlainTextEdit),
status (new QLabel),
...
wallet (wallet_a)
{
	...
	QObject::connect (process, &QPushButton::released, [this] () {
		...
		try
		{
			...
			auto block_l (btcnew::deserialize_block_json (tree));
			if (block_l != nullptr)
			{
...
btcnew_qt::block_creation::block_creation (btcnew_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void btcnew_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void btcnew_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void btcnew_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void btcnew_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void btcnew_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
			...
			if (!error)
			{
				...
				if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						(void)error;
						assert (!error);
						btcnew::state_block send (account_l, info.head, info.representative, balance - amount_l.number (), destination_l, key, account_l, 0);
...
void btcnew_qt::block_creation::create_receive ()
{
	btcnew::block_hash source_l;
	...
	if (!error)
	{
		...
		if (block_l != nullptr)
		{
			...
			{
				btcnew::pending_key pending_key (destination, source_l);
				...
				if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							btcnew::state_block receive (pending_key.account, info.head, info.representative, info.balance.number () + pending.amount.number (), source_l, key, pending_key.account, 0);
...
void btcnew_qt::block_creation::create_change ()
{
	btcnew::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				if (!error)
				{
					btcnew::state_block change (account_l, info.head, representative_l, info.balance, 0, key, account_l, 0);
...
void btcnew_qt::block_creation::create_open ()
{
	btcnew::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
				...
				if (!destination.is_zero ())
				{
					...
					if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
					{
						...
						if (error)
						{
							...
							if (!error)
							{
								btcnew::state_block open (pending_key.account, 0, representative_l, pending.amount, source_l, key, pending_key.account, 0);
