...
public:
	...
	{
		sock.async_connect (btcnew::tcp_endpoint (boost::asio::ip::address_v6::loopback (), port), [this] (boost::system::error_code const & ec) {
			if (!ec)
			{
...
void enable_ipc_transport_tcp (btcnew::ipc::ipc_config_tcp_socket & transport_tcp, uint16_t ipc_port)
{
	transport_tcp.enabled = true;
	transport_tcp.port = ipc_port;
}

void enable_ipc_transport_tcp (btcnew::ipc::ipc_config_tcp_socket & transport_tcp)
{
...
void reset_confirmation_height (btcnew::block_store & store, btcnew::account const & account)
{
...
void check_block_response_count (btcnew::system & system, btcnew::rpc & rpc, boost::property_tree::ptree & request, uint64_t size_count)
{
...
public:
	...
	void reset ()
	{
		btcnew::thread_role::set (btcnew::thread_role::name::unknown);
	}
...
TEST (rpc, account_balance)
{
	...
	btcnew::ipc::ipc_server ipc_server (*node, node_rpc_config);
	btcnew::rpc_config rpc_config (true);
	btcnew::ipc_rpc_processor ipc_rpc_processor (system.io_ctx, rpc_config);
	btcnew::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	boost::property_tree::ptree request;
	request.put ("action", "account_balance");
	request.put ("account", btcnew::test_genesis_key.pub.to_account ());
	test_response response (request, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response.status == 0)
	{
...
TEST (rpc, account_create)
{
	...
	ASSERT_EQ (std::error_code (btcnew::error_common::invalid_index).message (), response2.json.get<std::string> ("error"));
}
...
TEST (rpc, account_weight)
{
	...
	btcnew::change_block block (latest, key.pub, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	ASSERT_EQ (btcnew::process_result::progress, node1.process (block).code);
	scoped_io_thread_name_change scoped_thread_name_io;
	enable_ipc_transport_tcp (node1.config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server (node1, node_rpc_config);
	btcnew::rpc_config rpc_config (true);
	btcnew::ipc_rpc_processor ipc_rpc_processor (system.io_ctx, rpc_config);
	btcnew::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
...
TEST (rpc, validate_account_invalid)
{
	...
	rpc.start ();
	std::string account;
	btcnew::test_genesis_key.pub.encode_account (account);
	account[0] ^= 0x1;
	boost::property_tree::ptree request;
	request.put ("action", "validate_account_number");
...
TEST (rpc, send)
{
	...
	request.put ("source", btcnew::test_genesis_key.pub.to_account ());
	request.put ("destination", btcnew::test_genesis_key.pub.to_account ());
	request.put ("amount", "100");
	system.deadline_set (10s);
	boost::thread thread2 ([&system] () {
		while (system.nodes[0]->balance (btcnew::test_genesis_key.pub) == btcnew::genesis_amount)
		{
			ASSERT_NO_ERROR (system.poll ());
		}
	});
...
TEST (rpc, send_fail)
{
	...
	ASSERT_EQ (std::error_code (btcnew::error_common::account_not_found_wallet).message (), response.json.get<std::string> ("error"));
	thread2.join ();
}
...
TEST (rpc, send_work)
{
	...
	ASSERT_EQ (std::error_code (btcnew::error_common::invalid_work).message (), response.json.get<std::string> ("error"));
	request.erase ("work");
	request.put ("work", btcnew::to_string_hex (*system.nodes[0]->work_generate_blocking (system.nodes[0]->latest (btcnew::test_genesis_key.pub))));
	test_response response2 (request, rpc.config.port, system.io_ctx);
	system.deadline_set (10s);
	while (response2.status == 0)
	{
	...
	ASSERT_EQ (system.nodes[0]->latest (btcnew::test_genesis_key.pub), block);
}

TEST (rpc, send_work_disabled)
{
	btcnew::system system (24000, 0);
	...
	request.put ("amount", "100");
	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (btcnew::error_common::disabled_work_generation).message (), response.json.get<std::string> ("error"));
	}
}
...
TEST (rpc, send_idempotent)
{
	...
	request.put ("action", "send");
	request.put ("source", btcnew::test_genesis_key.pub.to_account ());
	request.put ("destination", btcnew::account (0).to_account ());
	request.put ("amount", (btcnew::genesis_amount - (btcnew::genesis_amount / 4)).convert_to<std::string> ());
	...
	ASSERT_FALSE (block.decode_hex (block_text));
	ASSERT_TRUE (system.nodes[0]->ledger.block_exists (block));
	ASSERT_EQ (system.nodes[0]->balance (btcnew::test_genesis_key.pub), btcnew::genesis_amount / 4);
	test_response response2 (request, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response2.status == 0)
	{
	...
	ASSERT_EQ (std::error_code (btcnew::error_common::insufficient_balance).message (), response3.json.get<std::string> ("error"));
}
...
TEST (rpc, wallet_representative)
{
	...
	ASSERT_EQ (account_text1, btcnew::genesis_account.to_account ());
}
...
TEST (rpc, wallet_representative_set_force)
{
	...
	while (representative != key.pub)
	{
		...
		if (!system.nodes[0]->store.account_get (transaction, btcnew::test_genesis_key.pub, info))
		{
...
TEST (rpc, wallet_create_seed)
{
	...
	scoped_io_thread_name_change scoped_thread_name_io;
	btcnew::raw_key seed;
	btcnew::random_pool::generate_block (seed.data.bytes.data (), seed.data.bytes.size ());
	auto prv = btcnew::deterministic_key (seed, 0);
	auto pub (btcnew::pub_key (prv));
	auto node = system.nodes.front ();
	enable_ipc_transport_tcp (node->config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server (*node, node_rpc_config);
...
TEST (rpc, wallet_export)
{
	...
	btcnew::kdf kdf;
	btcnew::wallet_store store (error, kdf, transaction, btcnew::genesis_account, 1, "0", wallet_json);
	ASSERT_FALSE (error);
	ASSERT_TRUE (store.exists (transaction, btcnew::test_genesis_key.pub));
}
...
TEST (rpc, account_move)
{
	...
	destination->insert_adhoc (btcnew::test_genesis_key.prv);
	btcnew::keypair key;
	auto source_id = btcnew::random_wallet_id ();
	...
	ASSERT_TRUE (destination->exists (btcnew::test_genesis_key.pub));
	auto transaction (system.nodes[0]->wallets.tx_begin_read ());
...
TEST (rpc, block)
{
	...
	request.put ("hash", system.nodes[0]->latest (btcnew::genesis_account).to_string ());
	test_response response (request, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response.status == 0)
	{
...
TEST (rpc, chain)
{
	...
	system.wallet (0)->insert_adhoc (btcnew::test_genesis_key.prv);
	btcnew::keypair key;
	auto genesis (system.nodes[0]->latest (btcnew::test_genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto block (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key.pub, 1));
	...
	std::vector<btcnew::block_hash> blocks;
	for (auto i (blocks_node.begin ()), n (blocks_node.end ()); i != n; ++i)
	{
		blocks.push_back (btcnew::block_hash (i->second.get<std::string> ("")));
	}
...
{
	btcnew::system system (24000, 1);
	std::unordered_map<btcnew::account, btcnew::block_hash> source;
	{
		...
		for (auto i (0); i < 1000; ++i)
		{
			...
			system.nodes[0]->store.account_put (transaction, key.pub, btcnew::account_info (hash, 0, 0, 0, 0, 0, btcnew::epoch::epoch_0));
		}
	}
	scoped_io_thread_name_change scoped_thread_name_io;
	btcnew::keypair key;
	...
	request.put ("account", btcnew::account (0).to_account ());
	request.put ("count", std::to_string (std::numeric_limits<uint64_t>::max ()));
	test_response response (request, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response.status == 0)
	...
	std::unordered_map<btcnew::account, btcnew::block_hash> frontiers;
	...
	ASSERT_EQ (1, frontiers.erase (btcnew::test_genesis_key.pub));
...
TEST (rpc, history)
{
	btcnew::system system (24000, 1);
	system.wallet (0)->insert_adhoc (btcnew::test_genesis_key.prv);
	auto change (system.wallet (0)->change_action (btcnew::test_genesis_key.pub, btcnew::test_genesis_key.pub));
	ASSERT_NE (nullptr, change);
	auto send (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, btcnew::test_genesis_key.pub, system.nodes[0]->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (*send, btcnew::test_genesis_key.pub, system.nodes[0]->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, receive);
	auto node0 (system.nodes[0]);
	btcnew::genesis genesis;
	btcnew::state_block usend (btcnew::genesis_account, node0->latest (btcnew::genesis_account), btcnew::genesis_account, btcnew::genesis_amount - btcnew::Gbtcnew_ratio, btcnew::genesis_account, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.nodes[0]->work_generate_blocking (node0->latest (btcnew::genesis_account)));
	btcnew::state_block ureceive (btcnew::genesis_account, usend.hash (), btcnew::genesis_account, btcnew::genesis_amount, usend.hash (), btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.nodes[0]->work_generate_blocking (usend.hash ()));
	btcnew::state_block uchange (btcnew::genesis_account, ureceive.hash (), btcnew::keypair ().pub, btcnew::genesis_amount, 0, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.nodes[0]->work_generate_blocking (ureceive.hash ()));
	{
		auto transaction (node0->store.tx_begin_write ());
		ASSERT_EQ (btcnew::process_result::progress, node0->ledger.process (transaction, usend).code);
		ASSERT_EQ (btcnew::process_result::progress, node0->ledger.process (transaction, ureceive).code);
		ASSERT_EQ (btcnew::process_result::progress, node0->ledger.process (transaction, uchange).code);
	}
	scoped_io_thread_name_change scoped_thread_name_io;
	enable_ipc_transport_tcp (node0->config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server (*node0, node_rpc_config);
	btcnew::rpc_config rpc_config (true);
	btcnew::ipc_rpc_processor ipc_rpc_processor (system.io_ctx, rpc_config);
	btcnew::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	...
	ASSERT_EQ (btcnew::test_genesis_key.pub.to_account (), std::get<1> (history_l[0]));
	ASSERT_EQ (btcnew::Gbtcnew_ratio.convert_to<std::string> (), std::get<2> (history_l[0]));
	ASSERT_EQ (5, history_l.size ());
	ASSERT_EQ ("send", std::get<0> (history_l[1]));
	ASSERT_EQ (usend.hash ().to_string (), std::get<3> (history_l[1]));
	ASSERT_EQ (btcnew::test_genesis_key.pub.to_account (), std::get<1> (history_l[1]));
	ASSERT_EQ (btcnew::Gbtcnew_ratio.convert_to<std::string> (), std::get<2> (history_l[1]));
	ASSERT_EQ ("receive", std::get<0> (history_l[2]));
	ASSERT_EQ (btcnew::test_genesis_key.pub.to_account (), std::get<1> (history_l[2]));
	...
	ASSERT_EQ (btcnew::test_genesis_key.pub.to_account (), std::get<1> (history_l[3]));
	...
	ASSERT_EQ (btcnew::test_genesis_key.pub.to_account (), std::get<1> (history_l[4]));
	ASSERT_EQ (btcnew::genesis_amount.convert_to<std::string> (), std::get<2> (history_l[4]));
...
TEST (rpc, account_history)
{
	...
	rpc.start ();
	{
		...
		request.put ("account", btcnew::genesis_account.to_account ());
		request.put ("count", 100);
		test_response response (request, rpc.config.port, system.io_ctx);
		while (response.status == 0)
		{
	...
	auto send2 (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, account2, system.nodes[0]->config.receive_minimum.number ()));
	...
	// Test filter for receive state blocks
	{
		...
		boost::property_tree::ptree other_account;
		other_account.put ("", btcnew::test_genesis_key.pub.to_account ());
		boost::property_tree::ptree filtered_accounts;
		filtered_accounts.push_back (std::make_pair ("", other_account));
		request.add_child ("account_filter", filtered_accounts);
		request.put ("count", 100);
...
TEST (rpc, process_block)
{
	...
	scoped_io_thread_name_change scoped_thread_name_io;
	btcnew::keypair key;
	auto latest (system.nodes[0]->latest (btcnew::test_genesis_key.pub));
	auto & node1 (*system.nodes[0]);
	btcnew::send_block send (latest, key.pub, 100, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	enable_ipc_transport_tcp (node1.config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server (node1, node_rpc_config);
	btcnew::rpc_config rpc_config (true);
	...
	request.put ("block", json);
	{
		...
		ASSERT_EQ (200, response.status);
		system.deadline_set (10s);
		while (system.nodes[0]->latest (btcnew::test_genesis_key.pub) != send.hash ())
		{
			ASSERT_NO_ERROR (system.poll ());
		}
	...
	request.put ("json_block", true);
	{
		...
		ASSERT_EQ (200, response.status);
		std::error_code ec (btcnew::error_blocks::invalid_block);
...
TEST (rpc, process_block_with_work_watcher)
{
	...
	btcnew::keypair key;
	auto latest (system.nodes[0]->latest (btcnew::test_genesis_key.pub));
	auto send (std::make_shared<btcnew::state_block> (btcnew::test_genesis_key.pub, latest, btcnew::test_genesis_key.pub, btcnew::genesis_amount - 100, btcnew::test_genesis_key.pub, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (latest)));
	uint64_t difficulty1 (0);
	btcnew::work_validate (*send, &difficulty1);
...
TEST (rpc, process_republish)
{
	...
	ASSERT_EQ (200, response.status);
	system.deadline_set (10s);
	while (system.nodes[1]->latest (btcnew::test_genesis_key.pub) != send.hash ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (rpc, process_subtype_send)
{
	...
	auto latest (system.nodes[0]->latest (btcnew::test_genesis_key.pub));
	auto & node1 (*system.nodes[0]);
	btcnew::state_block send (btcnew::genesis_account, latest, btcnew::genesis_account, btcnew::genesis_amount - btcnew::Gbtcnew_ratio, key.pub, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	enable_ipc_transport_tcp (node1.config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server (node1, node_rpc_config);
	btcnew::rpc_config rpc_config (true);
	...
	ASSERT_EQ (200, response.status);
	std::error_code ec (btcnew::error_rpc::invalid_subtype_balance);
...
TEST (rpc, process_subtype_open)
{
	...
	btcnew::state_block send (btcnew::genesis_account, latest, btcnew::genesis_account, btcnew::genesis_amount - btcnew::Gbtcnew_ratio, key.pub, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	{
		...
		ASSERT_EQ (btcnew::process_result::progress, node1.ledger.process (transaction, send).code);
	}
	scoped_io_thread_name_change scoped_thread_name_io;
	node1.active.start (std::make_shared<btcnew::state_block> (send));
	btcnew::state_block open (key.pub, 0, key.pub, btcnew::Gbtcnew_ratio, send.hash (), key.prv, key.pub, *node1.work_generate_blocking (key.pub));
	enable_ipc_transport_tcp (node1.config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server (node1, node_rpc_config);
	btcnew::rpc_config rpc_config (true);
...
TEST (rpc, process_subtype_receive)
{
	...
	auto latest (system.nodes[0]->latest (btcnew::test_genesis_key.pub));
	auto & node1 (*system.nodes[0]);
	btcnew::state_block send (btcnew::genesis_account, latest, btcnew::genesis_account, btcnew::genesis_amount - btcnew::Gbtcnew_ratio, btcnew::test_genesis_key.pub, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	{
		auto transaction (node1.store.tx_begin_write ());
		ASSERT_EQ (btcnew::process_result::progress, node1.ledger.process (transaction, send).code);
	}
	scoped_io_thread_name_change scoped_thread_name_io;
	node1.active.start (std::make_shared<btcnew::state_block> (send));
	btcnew::state_block receive (btcnew::test_genesis_key.pub, send.hash (), btcnew::test_genesis_key.pub, btcnew::genesis_amount, send.hash (), btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *node1.work_generate_blocking (send.hash ()));
	enable_ipc_transport_tcp (node1.config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server (node1, node_rpc_config);
	btcnew::rpc_config rpc_config (true);
	...
	ASSERT_EQ (200, response2.status);
	ec = btcnew::error_rpc::invalid_subtype_previous;
	ASSERT_EQ (response2.json.get<std::string> ("error"), ec.message ());
	request.put ("subtype", "receive");
	test_response response3 (request, rpc.config.port, system.io_ctx);
	while (response3.status == 0)
	...
	system.deadline_set (10s);
	while (system.nodes[1]->latest (btcnew::test_genesis_key.pub) != receive.hash ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (rpc, payment_begin_end)
{
	...
	uint64_t work (0);
	while (!btcnew::work_validate (root1, work))
	{
		++work;
		ASSERT_LT (work, 50);
	}
...
TEST (rpc, payment_end_nonempty)
{
	...
	request1.put ("wallet", wallet_id.to_string ());
	request1.put ("account", btcnew::test_genesis_key.pub.to_account ());
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	{
...
TEST (rpc, payment_zero_balance)
{
	...
	btcnew::account account;
	ASSERT_FALSE (account.decode_account (account_text));
	ASSERT_NE (btcnew::test_genesis_key.pub, account);
}
...
TEST (rpc, payment_wait)
{
	...
	request1.put ("amount", btcnew::amount (btcnew::Mbtcnew_ratio).to_string_dec ());
	...
	scoped_thread_name_io.reset ();
	system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key.pub, btcnew::Mbtcnew_ratio);
	...
	request1.put ("amount", btcnew::amount (btcnew::Mbtcnew_ratio * 2).to_string_dec ());
	test_response response3 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response3.status == 0)
	{
...
TEST (rpc, peers)
{
	btcnew::system system (24000, 2);
	scoped_io_thread_name_change scoped_thread_name_io;
	btcnew::endpoint endpoint (boost::asio::ip::address_v6::from_string ("fc00::1"), 4000);
	auto node = system.nodes.front ();
...
TEST (rpc, pending)
{
	...
	btcnew::keypair key1;
	system.wallet (0)->insert_adhoc (btcnew::test_genesis_key.prv);
	auto block1 (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key1.pub, 100));
	scoped_io_thread_name_change scoped_thread_name_io;
	system.deadline_set (5s);
	...
	request.put ("count", "100");
	{
		...
		btcnew::block_hash hash (blocks_node.begin ()->second.get<std::string> (""));
	...
	request.put ("sorting", "true"); // Sorting test
	{
		...
		auto & blocks_node (response.json.get_child ("blocks"));
		ASSERT_EQ (1, blocks_node.size ());
		btcnew::block_hash hash (blocks_node.begin ()->first);
		ASSERT_EQ (block1->hash (), hash);
	...
	request.put ("threshold", "100"); // Threshold test
	{
		...
		auto & blocks_node (response.json.get_child ("blocks"));
		ASSERT_EQ (1, blocks_node.size ());
		std::unordered_map<btcnew::block_hash, btcnew::uint128_union> blocks;
		for (auto i (blocks_node.begin ()), j (blocks_node.end ()); i != j; ++i)
		{
			btcnew::block_hash hash;
			hash.decode_hex (i->first);
			btcnew::uint128_union amount;
	...
	request.put ("min_version", "true");
	{
		...
		auto & blocks_node (response.json.get_child ("blocks"));
		ASSERT_EQ (1, blocks_node.size ());
		std::unordered_map<btcnew::block_hash, btcnew::uint128_union> amounts;
		std::unordered_map<btcnew::block_hash, btcnew::account> sources;
		for (auto i (blocks_node.begin ()), j (blocks_node.end ()); i != j; ++i)
		{
			btcnew::block_hash hash;
			hash.decode_hex (i->first);
		...
		ASSERT_EQ (sources[block1->hash ()], btcnew::test_genesis_key.pub);
	}
...
TEST (rpc, search_pending)
{
	...
	auto latest (system.nodes[0]->latest (btcnew::test_genesis_key.pub));
	btcnew::send_block block (latest, btcnew::test_genesis_key.pub, btcnew::genesis_amount - system.nodes[0]->config.receive_minimum.number (), btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.nodes[0]->work_generate_blocking (latest));
	{
		auto transaction (system.nodes[0]->store.tx_begin_write ());
		ASSERT_EQ (btcnew::process_result::progress, system.nodes[0]->ledger.process (transaction, block).code);
	}
	scoped_io_thread_name_change scoped_thread_name_io;
	auto node = system.nodes.front ();
	enable_ipc_transport_tcp (node->config.ipc_config.transport_tcp);
...
TEST (rpc, version)
{
	...
	ASSERT_EQ (boost::str (boost::format ("Bitcoin New %1%") % BTCNEW_VERSION_STRING), response1.json.get<std::string> ("node_vendor"));
	...
	auto genesis_open (node1->latest (btcnew::test_genesis_key.pub));
...
TEST (rpc, work_generate)
{
	...
	auto verify_response = [node, &rpc, &system] (auto & request, auto & hash) {
		...
		ASSERT_FALSE (btcnew::from_string_hex (work_text, work));
		ASSERT_FALSE (btcnew::work_validate (hash, work, &result_difficulty));
		auto response_difficulty_text (response.json.get<std::string> ("difficulty"));
		uint64_t response_difficulty;
		ASSERT_FALSE (btcnew::from_string_hex (response_difficulty_text, response_difficulty));
		ASSERT_EQ (result_difficulty, response_difficulty);
		auto multiplier = response.json.get<double> ("multiplier");
		ASSERT_NEAR (btcnew::difficulty::to_multiplier (result_difficulty, node->network_params.network.publish_threshold), multiplier, 1e-6);
	};
...
TEST (rpc, work_generate_difficulty)
{
	...
	request.put ("hash", hash.to_string ());
	{
		...
		request.put ("difficulty", btcnew::to_string_hex (difficulty));
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (10s);
		while (response.status == 0)
		{
		...
		ASSERT_EQ (btcnew::difficulty::to_multiplier (result_difficulty, node->network_params.network.publish_threshold), multiplier);
	...
	}
	{
		...
		ASSERT_EQ (200, response.status);
		std::error_code ec (btcnew::error_rpc::difficulty_limit);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	}
}
...
TEST (rpc, work_generate_multiplier)
{
	...
	request.put ("hash", hash.to_string ());
	{
		...
		request.put ("difficulty", btcnew::to_string_hex (0xff00000000000000));
	...
	}
	{
		...
		ASSERT_EQ (200, response.status);
		std::error_code ec (btcnew::error_rpc::bad_multiplier_format);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	}
	{
		double max_multiplier (btcnew::difficulty::to_multiplier (node->config.max_work_generate_difficulty, node->network_params.network.publish_threshold));
...
TEST (rpc, work_peer_bad)
{
	...
	system.deadline_set (5s);
	while (btcnew::work_validate (hash1, work))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (rpc, work_peer_one)
{
	...
	system.deadline_set (5s);
	while (btcnew::work_validate (key1.pub, work))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (rpc, work_peer_many)
{
	...
	btcnew::system system3 (24002, 1);
	btcnew::system system4 (24003, 1);
	auto & node1 (*system1.nodes[0]);
	auto & node2 (*system2.nodes[0]);
	auto & node3 (*system3.nodes[0]);
	auto & node4 (*system4.nodes[0]);
	btcnew::keypair key;
	btcnew::rpc_config config2 (true);
	config2.port += 0;
	scoped_io_thread_name_change scoped_thread_name_io;
	enable_ipc_transport_tcp (node2.config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server2 (node2, node_rpc_config);
	btcnew::ipc_rpc_processor ipc_rpc_processor2 (system2.io_ctx, config2);
	btcnew::rpc rpc2 (system2.io_ctx, config2, ipc_rpc_processor2);
	rpc2.start ();
	btcnew::rpc_config config3 (true);
	config3.port += 1;
	enable_ipc_transport_tcp (node3.config.ipc_config.transport_tcp, node3.network_params.network.default_ipc_port + 1);
	btcnew::ipc::ipc_server ipc_server3 (node3, node_rpc_config);
	btcnew::ipc_rpc_processor ipc_rpc_processor3 (system3.io_ctx, config3);
	btcnew::rpc rpc3 (system3.io_ctx, config3, ipc_rpc_processor3);
	rpc3.start ();
	btcnew::rpc_config config4 (true);
	config4.port += 2;
	enable_ipc_transport_tcp (node4.config.ipc_config.transport_tcp, node4.network_params.network.default_ipc_port + 2);
	btcnew::ipc::ipc_server ipc_server4 (node4, node_rpc_config);
	btcnew::ipc_rpc_processor ipc_rpc_processor4 (system4.io_ctx, config4);
	btcnew::rpc rpc4 (system2.io_ctx, config4, ipc_rpc_processor4);
	rpc4.start ();
...
TEST (rpc, block_count)
{
	...
	// Should be able to get all counts even when enable_control is false.
	{
		...
		btcnew::node_rpc_config node_rpc_config;
		btcnew::ipc::ipc_server ipc_server (node1, node_rpc_config);
		btcnew::rpc_config rpc_config (false);
		btcnew::ipc_rpc_processor ipc_rpc_processor (system.io_ctx, rpc_config);
		btcnew::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
		rpc.start ();
		boost::property_tree::ptree request1;
...
TEST (rpc, available_supply)
{
	...
	scoped_thread_name_io.reset ();
	auto block2 (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, 0, 100)); // Sending to burning 0 account
	scoped_thread_name_io.renew ();
	test_response response3 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response3.status == 0)
...
TEST (rpc, mrai_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (btcnew::Mbtcnew_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, mrai_from_raw)
{
	...
	request1.put ("amount", btcnew::Mbtcnew_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	{
...
TEST (rpc, krai_to_raw)
{
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (btcnew::kbtcnew_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}
...
TEST (rpc, krai_from_raw)
{
	...
	request1.put ("amount", btcnew::kbtcnew_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	{
...
TEST (rpc, btcnew_to_raw)
{
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "btcnew_to_raw");
	request1.put ("amount", "1");
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (btcnew::btcnew_ratio.convert_to<std::string> (), response1.json.get<std::string> ("amount"));
}

TEST (rpc, btcnew_from_raw)
{
	btcnew::system system (24000, 1);
	auto & node1 (*system.nodes[0]);
	scoped_io_thread_name_change scoped_thread_name_io;
	...
	rpc.start ();
	boost::property_tree::ptree request1;
	request1.put ("action", "btcnew_from_raw");
	request1.put ("amount", btcnew::btcnew_ratio.convert_to<std::string> ());
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	{
...
TEST (rpc, bootstrap)
{
	...
	auto latest (system1.nodes[0]->latest (btcnew::test_genesis_key.pub));
	btcnew::send_block send (latest, btcnew::genesis_account, 100, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system1.nodes[0]->work_generate_blocking (latest));
	{
		auto transaction (system1.nodes[0]->store.tx_begin_write ());
		ASSERT_EQ (btcnew::process_result::progress, system1.nodes[0]->ledger.process (transaction, send).code);
	}
	scoped_io_thread_name_change scoped_thread_name_io;
	...
	btcnew::ipc::ipc_server ipc_server (*node, node_rpc_config);
	btcnew::rpc_config rpc_config (true);
	btcnew::ipc_rpc_processor ipc_rpc_processor (system0.io_ctx, rpc_config);
	btcnew::rpc rpc (system0.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	boost::property_tree::ptree request;
	...
	while (system0.nodes[0]->latest (btcnew::genesis_account) != system1.nodes[0]->latest (btcnew::genesis_account))
	{
...
TEST (rpc, representatives)
{
	...
	std::vector<btcnew::account> representatives;
	...
	ASSERT_EQ (btcnew::genesis_account, representatives[0]);
}
...
TEST (rpc, wallet_change_seed)
{
	...
	btcnew::random_pool::generate_block (seed.data.bytes.data (), seed.data.bytes.size ());
	{
		...
		btcnew::raw_key seed0;
		btcnew::random_pool::generate_block (seed0.data.bytes.data (), seed0.data.bytes.size ());
...
TEST (rpc, wallet_frontiers)
{
	...
	ASSERT_EQ (200, response.status);
	auto & frontiers_node (response.json.get_child ("frontiers"));
	std::vector<btcnew::account> frontiers;
	for (auto i (frontiers_node.begin ()), n (frontiers_node.end ()); i != n; ++i)
	{
		frontiers.push_back (btcnew::account (i->second.get<std::string> ("")));
	}
	ASSERT_EQ (1, frontiers.size ());
	ASSERT_EQ (system0.nodes[0]->latest (btcnew::genesis_account), frontiers[0]);
}
...
TEST (rpc, work_validate)
{
	...
	request.put ("hash", hash.to_string ());
	request.put ("work", btcnew::to_string_hex (work1));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
		...
		uint64_t difficulty;
		ASSERT_FALSE (btcnew::from_string_hex (difficulty_text, difficulty));
		ASSERT_GE (difficulty, params.network.publish_threshold);
		double multiplier (response.json.get<double> ("multiplier"));
		ASSERT_NEAR (multiplier, btcnew::difficulty::to_multiplier (difficulty, params.network.publish_threshold), 1e-6);
	}
	uint64_t work2 (0);
	request.put ("work", btcnew::to_string_hex (work2));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
	...
	uint64_t result_difficulty;
	ASSERT_FALSE (btcnew::work_validate (hash, work1, &result_difficulty));
	ASSERT_GE (result_difficulty, params.network.publish_threshold);
	request.put ("work", btcnew::to_string_hex (work1));
	request.put ("difficulty", btcnew::to_string_hex (result_difficulty));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
	...
	request.put ("work", btcnew::to_string_hex (work1));
	request.put ("difficulty", btcnew::to_string_hex (difficulty4));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
	...
	request.put ("work", btcnew::to_string_hex (work3));
	{
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
...
TEST (rpc, republish)
{
	...
	btcnew::genesis genesis;
	auto & node1 (*system.nodes[0]);
	auto latest (node1.latest (btcnew::test_genesis_key.pub));
	btcnew::send_block send (latest, key.pub, 100, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	node1.process (send);
	btcnew::open_block open (send.hash (), key.pub, key.pub, key.prv, key.pub, *node1.work_generate_blocking (key.pub));
	ASSERT_EQ (btcnew::process_result::progress, node1.process (open).code);
	scoped_io_thread_name_change scoped_thread_name_io;
	enable_ipc_transport_tcp (node1.config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server (node1, node_rpc_config);
	...
	ASSERT_EQ (200, response.status);
	system.deadline_set (10s);
	while (system.nodes[1]->balance (btcnew::test_genesis_key.pub) == btcnew::genesis_amount)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	auto & blocks_node (response.json.get_child ("blocks"));
...
TEST (rpc, deterministic_key)
{
	...
	btcnew::account account0 (system0.wallet (0)->deterministic_insert ());
	btcnew::account account1 (system0.wallet (0)->deterministic_insert ());
	btcnew::account account2 (system0.wallet (0)->deterministic_insert ());
	auto & node = system0.nodes.front ();
	scoped_io_thread_name_change scoped_thread_name_io;
	enable_ipc_transport_tcp (node->config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
...
TEST (rpc, accounts_balances)
{
	...
	entry.put ("", btcnew::test_genesis_key.pub.to_account ());
	...
	for (auto & balances : response.json.get_child ("balances"))
	{
		...
		ASSERT_EQ (btcnew::test_genesis_key.pub.to_account (), account_text);
...
TEST (rpc, accounts_frontiers)
{
	...
	for (auto & frontiers : response.json.get_child ("frontiers"))
	{
		...
		ASSERT_EQ (system.nodes[0]->latest (btcnew::genesis_account), frontier_text);
	}
}
...
TEST (rpc, accounts_pending)
{
	...
	request.put ("count", "100");
	{
		...
		for (auto & blocks : response.json.get_child ("blocks"))
		{
			...
			btcnew::block_hash hash1 (blocks.second.begin ()->second.get<std::string> (""));
	...
	request.put ("sorting", "true"); // Sorting test
	{
		...
		for (auto & blocks : response.json.get_child ("blocks"))
		{
			std::string account_text (blocks.first);
			ASSERT_EQ (key1.pub.to_account (), account_text);
			btcnew::block_hash hash1 (blocks.second.begin ()->first);
			ASSERT_EQ (block1->hash (), hash1);
...
TEST (rpc, blocks)
{
	...
	boost::property_tree::ptree entry;
	boost::property_tree::ptree peers_l;
	entry.put ("", system.nodes[0]->latest (btcnew::genesis_account).to_string ());
	peers_l.push_back (std::make_pair ("", entry));
	...
	for (auto & blocks : response.json.get_child ("blocks"))
	{
		...
		ASSERT_EQ (system.nodes[0]->latest (btcnew::genesis_account).to_string (), hash_text);
...
TEST (rpc, wallet_info)
{
	...
	btcnew::keypair key;
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key.pub, 1));
	btcnew::account account (system.wallet (0)->deterministic_insert ());
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_write ());
...
TEST (rpc, wallet_balances)
{
	...
	auto send (system0.wallet (0)->send_action (btcnew::test_genesis_key.pub, key.pub, 1));
	scoped_thread_name_io.renew ();
...
TEST (rpc, pending_exists)
{
	...
	btcnew::keypair key1;
	system.wallet (0)->insert_adhoc (btcnew::test_genesis_key.prv);
	auto hash0 (system.nodes[0]->latest (btcnew::genesis_account));
	auto block1 (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key1.pub, 100));
	scoped_io_thread_name_change scoped_thread_name_io;
	system.deadline_set (5s);
	while (system.nodes[0]->active.active (*block1))
...
TEST (rpc, wallet_pending)
{
	...
	auto block1 (system0.wallet (0)->send_action (btcnew::test_genesis_key.pub, key1.pub, 100));
	auto iterations (0);
	scoped_io_thread_name_change scoped_thread_name_io;
	...
	for (auto & pending : response.json.get_child ("blocks"))
	{
		std::string account_text (pending.first);
		ASSERT_EQ (key1.pub.to_account (), account_text);
		btcnew::block_hash hash1 (pending.second.begin ()->second.get<std::string> (""));
		ASSERT_EQ (block1->hash (), hash1);
	}
	request.put ("threshold", "100"); // Threshold test
	test_response response0 (request, rpc.config.port, system0.io_ctx);
...
TEST (rpc, work_get)
{
	btcnew::system system (24000, 1);
	system.wallet (0)->insert_adhoc (btcnew::test_genesis_key.prv);
	system.wallet (0)->work_cache_blocking (btcnew::test_genesis_key.pub, system.nodes[0]->latest (btcnew::test_genesis_key.pub));
	auto node = system.nodes.front ();
	scoped_io_thread_name_change scoped_thread_name_io;
	enable_ipc_transport_tcp (node->config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	...
	uint64_t work (1);
	auto transaction (system.nodes[0]->wallets.tx_begin_read ());
	system.nodes[0]->wallets.items.begin ()->second->store.work_get (transaction, btcnew::genesis_account, work);
	ASSERT_EQ (btcnew::to_string_hex (work), work_text);
}
...
TEST (rpc, work_set)
{
	...
	request.put ("wallet", system.nodes[0]->wallets.items.begin ()->first.to_string ());
	request.put ("account", btcnew::test_genesis_key.pub.to_account ());
	request.put ("work", btcnew::to_string_hex (work0));
	test_response response (request, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response.status == 0)
	{
	...
	uint64_t work1 (1);
	auto transaction (system.nodes[0]->wallets.tx_begin_read ());
	system.nodes[0]->wallets.items.begin ()->second->store.work_get (transaction, btcnew::genesis_account, work1);
...
TEST (rpc, wallet_republish)
{
	...
	btcnew::genesis genesis;
	btcnew::keypair key;
	while (key.pub < btcnew::test_genesis_key.pub)
	{
		btcnew::keypair key1;
		key.pub = key1.pub;
	...
	btcnew::open_block open (send.hash (), key.pub, key.pub, key.prv, key.pub, *node1.work_generate_blocking (key.pub));
	ASSERT_EQ (btcnew::process_result::progress, system.nodes[0]->process (open).code);
	scoped_io_thread_name_change scoped_thread_name_io;
	enable_ipc_transport_tcp (node1.config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server (node1, node_rpc_config);
...
TEST (rpc, delegators)
{
	...
	btcnew::send_block send (latest, key.pub, 100, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	system.nodes[0]->process (send);
	btcnew::open_block open (send.hash (), btcnew::test_genesis_key.pub, key.pub, key.prv, key.pub, *node1.work_generate_blocking (key.pub));
	ASSERT_EQ (btcnew::process_result::progress, system.nodes[0]->process (open).code);
	enable_ipc_transport_tcp (node1.config.ipc_config.transport_tcp);
	scoped_io_thread_name_change scoped_thread_name_io;
	btcnew::node_rpc_config node_rpc_config;
	...
	ASSERT_EQ ("100", delegators.get<std::string> (btcnew::test_genesis_key.pub.to_account ()));
...
TEST (rpc, account_info)
{
	...
	request.put ("account", btcnew::account ().to_account ());
	...
	// Test for a non existing account
	{
		...
		ASSERT_EQ (error.get (), std::error_code (btcnew::error_common::account_not_found).message ());
	}
	...
	btcnew::send_block send (latest, key.pub, 100, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	system.nodes[0]->process (send);
	auto time (btcnew::seconds_since_epoch ());
	{
		auto transaction = node1.store.tx_begin_write ();
		node1.store.confirmation_height_put (transaction, btcnew::test_genesis_key.pub, 1);
	}
	scoped_thread_name_io.renew ();
	...
	request.put ("representative", "1");
	{
		...
		ASSERT_EQ (btcnew::test_genesis_key.pub.to_account (), representative2);
	}
}
...
TEST (rpc, json_block_input)
{
	...
	system.wallet (0)->insert_adhoc (key.prv);
	auto & node1 (*system.nodes[0]);
	btcnew::state_block send (btcnew::genesis_account, node1.latest (btcnew::test_genesis_key.pub), btcnew::genesis_account, btcnew::genesis_amount - btcnew::Gbtcnew_ratio, key.pub, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, 0);
	scoped_io_thread_name_change scoped_thread_name_io;
	enable_ipc_transport_tcp (node1.config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server (node1, node_rpc_config);
	...
	btcnew::state_block block (json_error, response.json.get_child ("block"));
	ASSERT_FALSE (json_error);

	ASSERT_FALSE (btcnew::validate_message (key.pub, send.hash (), block.block_signature ()));
...
TEST (rpc, json_block_output)
{
	...
	bool json_error{ false };
	btcnew::send_block send_from_json (json_error, response.json.get_child ("contents"));
	ASSERT_FALSE (json_error);
}
...
TEST (rpc, blocks_info)
{
	...
	auto check_blocks = [&system] (test_response & response) {
		for (auto & blocks : response.json.get_child ("blocks"))
		{
			...
			ASSERT_EQ (btcnew::genesis_amount.convert_to<std::string> (), amount_text);
			...
			ASSERT_EQ (btcnew::genesis_amount.convert_to<std::string> (), balance_text);
	...
	std::string random_hash = btcnew::block_hash ().to_string ();
	...
	request.add_child ("hashes", hashes);
	{
		...
		ASSERT_EQ (200, response.status);
		ASSERT_EQ (std::error_code (btcnew::error_blocks::not_found).message (), response.json.get<std::string> ("error"));
	}
...
TEST (rpc, blocks_info_subtype)
{
	...
	system.wallet (0)->insert_adhoc (btcnew::test_genesis_key.prv);
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, btcnew::test_genesis_key.pub, btcnew::Gbtcnew_ratio));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (*send, key.pub, btcnew::Gbtcnew_ratio));
	ASSERT_NE (nullptr, receive);
	auto change (system.wallet (0)->change_action (btcnew::test_genesis_key.pub, key.pub));
	ASSERT_NE (nullptr, change);
	scoped_io_thread_name_change scoped_thread_name_io;
	enable_ipc_transport_tcp (node1.config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
...
TEST (rpc, ledger)
{
	...
	auto & node1 (*system.nodes[0]);
	auto latest (node1.latest (btcnew::test_genesis_key.pub));
	auto genesis_balance (btcnew::genesis_amount);
	auto send_amount (genesis_balance - 100);
	genesis_balance -= send_amount;
	btcnew::send_block send (latest, key.pub, genesis_balance, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *node1.work_generate_blocking (latest));
	node1.process (send);
	btcnew::open_block open (send.hash (), btcnew::test_genesis_key.pub, key.pub, key.prv, key.pub, *node1.work_generate_blocking (key.pub));
	ASSERT_EQ (btcnew::process_result::progress, node1.process (open).code);
	auto time (btcnew::seconds_since_epoch ());
	...
	request.put ("representative", true);
	{
		...
		for (auto & account : response.json.get_child ("accounts"))
		{
			...
			ASSERT_EQ (btcnew::test_genesis_key.pub.to_account (), representative.get ());
		}
	}
	// Test threshold
	request.put ("count", 2);
	...
	btcnew::send_block send2 (send.hash (), key.pub, genesis_balance, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *node1.work_generate_blocking (send.hash ()));
	scoped_thread_name_io.reset ();
	node1.process (send2);
	scoped_thread_name_io.renew ();
...
TEST (rpc, block_create)
{
	...
	btcnew::send_block send (latest, key.pub, 100, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, send_work);
	auto open_work = *node1.work_generate_blocking (key.pub);
	btcnew::open_block open (send.hash (), btcnew::test_genesis_key.pub, key.pub, key.prv, key.pub, open_work);
	scoped_io_thread_name_change scoped_thread_name_io;
	enable_ipc_transport_tcp (node1.config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server (node1, node_rpc_config);
	...
	request.put ("work", btcnew::to_string_hex (send_work));
	test_response response (request, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response.status == 0)
	{
	...
	auto send_block (btcnew::deserialize_block_json (block_l));
	...
	request1.put ("representative", btcnew::test_genesis_key.pub.to_account ());
	request1.put ("source", send.hash ().to_string ());
	request1.put ("work", btcnew::to_string_hex (open_work));
	test_response response1 (request1, rpc.config.port, system.io_ctx);
	system.deadline_set (5s);
	while (response1.status == 0)
	{
	...
	auto open_block (btcnew::deserialize_block_json (block_l));
	...
	btcnew::change_block change (open.hash (), key.pub, key.prv, key.pub, change_work);
	request1.put ("type", "change");
	request1.put ("work", btcnew::to_string_hex (change_work));
	...
	auto change_block (btcnew::deserialize_block_json (block_l));
	ASSERT_EQ (change.hash (), change_block->hash ());
	scoped_thread_name_io.reset ();
	ASSERT_EQ (btcnew::process_result::progress, node1.process (change).code);
	btcnew::send_block send2 (send.hash (), key.pub, 0, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *node1.work_generate_blocking (send.hash ()));
	ASSERT_EQ (btcnew::process_result::progress, system.nodes[0]->process (send2).code);
	scoped_thread_name_io.renew ();
	boost::property_tree::ptree request2;
	...
	request2.put ("work", btcnew::to_string_hex (*node1.work_generate_blocking (change.hash ())));
	...
	auto receive_block (btcnew::deserialize_block_json (block_l));
...
TEST (rpc, block_create_state)
{
	...
	request.put ("representative", btcnew::test_genesis_key.pub.to_account ());
	request.put ("balance", (btcnew::genesis_amount - btcnew::Gbtcnew_ratio).convert_to<std::string> ());
	request.put ("link", key.pub.to_account ());
	request.put ("work", btcnew::to_string_hex (*system.nodes[0]->work_generate_blocking (genesis.hash ())));
	auto node = system.nodes.front ();
	scoped_io_thread_name_change scoped_thread_name_io;
	enable_ipc_transport_tcp (node->config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	...
	boost::property_tree::ptree block_l;
	boost::property_tree::read_json (block_stream, block_l);
	auto state_block (btcnew::deserialize_block_json (block_l));
	ASSERT_NE (nullptr, state_block);
	ASSERT_EQ (btcnew::block_type::state, state_block->type ());
	ASSERT_EQ (state_hash, state_block->hash ().to_string ());
	scoped_thread_name_io.reset ();
	auto process_result (system.nodes[0]->process (*state_block));
	ASSERT_EQ (btcnew::process_result::progress, process_result.code);
}
...
TEST (rpc, block_create_state_open)
{
	...
	btcnew::genesis genesis;
	system.wallet (0)->insert_adhoc (btcnew::test_genesis_key.prv);
	auto send_block (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key.pub, btcnew::Gbtcnew_ratio));
	...
	request.put ("representative", btcnew::test_genesis_key.pub.to_account ());
	request.put ("balance", btcnew::Gbtcnew_ratio.convert_to<std::string> ());
	request.put ("link", send_block->hash ().to_string ());
	request.put ("work", btcnew::to_string_hex (*system.nodes[0]->work_generate_blocking (key.pub)));
	auto node = system.nodes.front ();
	scoped_io_thread_name_change scoped_thread_name_io;
	enable_ipc_transport_tcp (node->config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
...
TEST (rpc, block_create_state_request_work)
{
	...
	for (auto previous : previous_test_input)
	{
		...
		boost::property_tree::read_json (block_stream, block_l);
		auto block (btcnew::deserialize_block_json (block_l));
		ASSERT_NE (nullptr, block);
		ASSERT_FALSE (btcnew::work_validate (*block));
	}
}
...
TEST (rpc, wallet_create_fail)
{
	...
	ASSERT_EQ (std::error_code (btcnew::error_common::wallet_lmdb_max_dbs).message (), response.json.get<std::string> ("error"));
}
...
TEST (rpc, wallet_add_watch)
{
	...
	std::string success (response.json.get<std::string> ("success"));
	ASSERT_TRUE (success.empty ());
	ASSERT_TRUE (system.wallet (0)->exists (btcnew::test_genesis_key.pub));

	// Make sure using special wallet key as pubkey fails
	btcnew::public_key bad_key (1);
	...
	std::error_code ec (btcnew::error_common::bad_public_key);
...
TEST (rpc, online_reps)
{
	...
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server (*system.nodes[1], node_rpc_config);
	btcnew::rpc_config rpc_config (true);
	btcnew::ipc_rpc_processor ipc_rpc_processor (system.io_ctx, rpc_config);
	btcnew::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	...
	ASSERT_EQ (btcnew::test_genesis_key.pub.to_account (), item->second.get<std::string> (""));
	...
	ASSERT_EQ (btcnew::test_genesis_key.pub.to_account (), item2->first);
	auto weight2 (item2->second.get<std::string> ("weight"));
	ASSERT_EQ (system.nodes[1]->weight (btcnew::test_genesis_key.pub).convert_to<std::string> (), weight2);
	//Test accounts filter
	scoped_thread_name_io.reset ();
	auto new_rep (system.wallet (1)->deterministic_insert ());
	auto send (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, new_rep, system.nodes[0]->config.receive_minimum.number ()));
	scoped_thread_name_io.renew ();
	ASSERT_NE (nullptr, send);
	system.deadline_set (5s);
	...
	scoped_thread_name_io.reset ();
	auto change (system.wallet (0)->change_action (btcnew::test_genesis_key.pub, new_rep));
	scoped_thread_name_io.renew ();
	ASSERT_NE (nullptr, change);
	system.deadline_set (5s);
...
TEST (rpc, confirmation_height_currently_processing)
{
	...
	auto previous_genesis_chain_hash = node->latest (btcnew::test_genesis_key.pub);
	{
		constexpr auto num_blocks = 1000;
		auto transaction = node->store.tx_begin_write ();
		for (auto i = num_blocks; i > 0; --i)
		{
			btcnew::send_block send (previous_genesis_chain_hash, btcnew::genesis_account, btcnew::genesis_amount - btcnew::Gbtcnew_ratio + i + 1, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (previous_genesis_chain_hash));
			ASSERT_EQ (btcnew::process_result::progress, node->ledger.process (transaction, send).code);
			previous_genesis_chain_hash = send.hash ();
		}

		btcnew::keypair key1;
		btcnew::send_block send (previous_genesis_chain_hash, key1.pub, btcnew::genesis_amount - btcnew::Gbtcnew_ratio - 1, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (previous_genesis_chain_hash));
		ASSERT_EQ (btcnew::process_result::progress, node->ledger.process (transaction, send).code);
		previous_genesis_chain_hash = send.hash ();
	}
	...
	std::shared_ptr<btcnew::block> frontier;
	{
	...
	// Make the same request, it should now return an error
	{
		...
		ASSERT_EQ (200, response.status);
		std::error_code ec (btcnew::error_rpc::confirmation_height_not_processing);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	}
}
...
TEST (rpc, confirmation_history)
{
	...
	btcnew::amount tally_num;
	tally_num.decode_dec (tally);
	assert (tally_num == btcnew::genesis_amount || tally_num == (btcnew::genesis_amount - btcnew::Gbtcnew_ratio));
	system.stop ();
}
...
TEST (rpc, confirmation_history_hash)
{
	...
	system.wallet (0)->insert_adhoc (btcnew::test_genesis_key.prv);
	ASSERT_TRUE (system.nodes[0]->active.list_confirmed ().empty ());
	auto send1 (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key.pub, btcnew::Gbtcnew_ratio));
	auto send2 (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key.pub, btcnew::Gbtcnew_ratio));
	auto send3 (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key.pub, btcnew::Gbtcnew_ratio));
	scoped_io_thread_name_change scoped_thread_name_io;
	system.deadline_set (10s);
	...
	btcnew::amount tally_num;
	tally_num.decode_dec (tally);
	assert (tally_num == btcnew::genesis_amount || tally_num == (btcnew::genesis_amount - btcnew::Gbtcnew_ratio) || tally_num == (btcnew::genesis_amount - 2 * btcnew::Gbtcnew_ratio) || tally_num == (btcnew::genesis_amount - 3 * btcnew::Gbtcnew_ratio));
	system.stop ();
}
...
TEST (rpc, block_confirm)
{
	...
	system.wallet (0)->insert_adhoc (btcnew::test_genesis_key.prv);
	btcnew::genesis genesis;
	auto send1 (std::make_shared<btcnew::state_block> (btcnew::test_genesis_key.pub, genesis.hash (), btcnew::test_genesis_key.pub, btcnew::genesis_amount - btcnew::Gbtcnew_ratio, btcnew::test_genesis_key.pub, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.nodes[0]->work_generate_blocking (genesis.hash ())));
	{
		auto transaction (system.nodes[0]->store.tx_begin_write ());
...
TEST (rpc, block_confirm_confirmed)
{
	...
	auto node (std::make_shared<btcnew::node> (system.io_ctx, path, system.alarm, config, system.work));
	node->start ();
	...
	ASSERT_EQ (0, node->stats.count (btcnew::stat::type::error, btcnew::stat::detail::http_callback, btcnew::stat::dir::out));
	scoped_io_thread_name_change scoped_thread_name_io;
	enable_ipc_transport_tcp (node->config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server (*node, node_rpc_config);
	...
	// Check callback
	system.deadline_set (5s);
	while (node->stats.count (btcnew::stat::type::error, btcnew::stat::detail::http_callback, btcnew::stat::dir::out) == 0)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	// Callback result is error because callback target port isn't listening
	ASSERT_EQ (1, node->stats.count (btcnew::stat::type::error, btcnew::stat::detail::http_callback, btcnew::stat::dir::out));
	node->stop ();
}
...
TEST (rpc, stats_clear)
{
	...
	btcnew::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
	system.nodes[0]->stats.inc (btcnew::stat::type::ledger, btcnew::stat::dir::in);
	ASSERT_EQ (1, system.nodes[0]->stats.count (btcnew::stat::type::ledger, btcnew::stat::dir::in));
	boost::property_tree::ptree request;
	...
	std::string success (response.json.get<std::string> ("success"));
	ASSERT_TRUE (success.empty ());
	ASSERT_EQ (0, system.nodes[0]->stats.count (btcnew::stat::type::ledger, btcnew::stat::dir::in));
...
TEST (rpc, unchecked)
{
	...
	auto open2 (std::make_shared<btcnew::state_block> (key.pub, 0, key.pub, 2, key.pub, key.prv, key.pub, *system.work.generate (key.pub)));
...
TEST (rpc, unchecked_get)
{
	...
	request.put ("hash", open->hash ().to_string ());
	{
		...
		ASSERT_LE (timestamp, btcnew::seconds_since_epoch ());
	}
	request.put ("json_block", true);
	{
		test_response response (request, rpc.config.port, system.io_ctx);
...
TEST (rpc, unopened)
{
	btcnew::system system (24000, 1);
	system.wallet (0)->insert_adhoc (btcnew::test_genesis_key.prv);
	btcnew::account account1 (1), account2 (account1.number () + 1);
	auto genesis (system.nodes[0]->latest (btcnew::test_genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto send (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, account1, 1));
	ASSERT_NE (nullptr, send);
	auto send2 (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, account2, 10));
	ASSERT_NE (nullptr, send2);
	auto node = system.nodes.front ();
	scoped_io_thread_name_change scoped_thread_name_io;
	enable_ipc_transport_tcp (node->config.ipc_config.transport_tcp);
	...
	}
	{
		...
		boost::property_tree::ptree request;
		request.put ("action", "unopened");
		request.put ("account", btcnew::account (account2.number () + 1).to_account ());
		test_response response (request, rpc.config.port, system.io_ctx);
		system.deadline_set (5s);
		while (response.status == 0)
		{
...
TEST (rpc, unopened_burn)
{
	...
	auto genesis (system.nodes[0]->latest (btcnew::test_genesis_key.pub));
	ASSERT_FALSE (genesis.is_zero ());
	auto send (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, btcnew::burn_account, 1));
	ASSERT_NE (nullptr, send);
	auto node = system.nodes.front ();
	scoped_io_thread_name_change scoped_thread_name_io;
	enable_ipc_transport_tcp (node->config.ipc_config.transport_tcp);
...
TEST (rpc, wallet_history)
{
	...
	btcnew::genesis genesis;
	system.wallet (0)->insert_adhoc (btcnew::test_genesis_key.prv);
	auto timestamp1 (btcnew::seconds_since_epoch ());
	auto send (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, btcnew::test_genesis_key.pub, node0->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send);
	std::this_thread::sleep_for (std::chrono::milliseconds (1000));
	auto timestamp2 (btcnew::seconds_since_epoch ());
	auto receive (system.wallet (0)->receive_action (*send, btcnew::test_genesis_key.pub, node0->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, receive);
	btcnew::keypair key;
	std::this_thread::sleep_for (std::chrono::milliseconds (1000));
	auto timestamp3 (btcnew::seconds_since_epoch ());
	auto send2 (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key.pub, node0->config.receive_minimum.number ()));
	ASSERT_NE (nullptr, send2);
	system.deadline_set (10s);
	auto node = system.nodes.front ();
	scoped_io_thread_name_change scoped_thread_name_io;
	...
	ASSERT_EQ (btcnew::test_genesis_key.pub.to_account (), std::get<4> (history_l[0]));
	...
	ASSERT_EQ (btcnew::test_genesis_key.pub.to_account (), std::get<4> (history_l[1]));
	...
	ASSERT_EQ (btcnew::test_genesis_key.pub.to_account (), std::get<4> (history_l[2]));
	ASSERT_EQ (std::to_string (timestamp1), std::get<5> (history_l[2]));
	// Genesis block
	ASSERT_EQ ("receive", std::get<0> (history_l[3]));
	ASSERT_EQ (btcnew::test_genesis_key.pub.to_account (), std::get<1> (history_l[3]));
	ASSERT_EQ (btcnew::genesis_amount.convert_to<std::string> (), std::get<2> (history_l[3]));
	ASSERT_EQ (genesis.hash ().to_string (), std::get<3> (history_l[3]));
	ASSERT_EQ (btcnew::test_genesis_key.pub.to_account (), std::get<4> (history_l[3]));
}
...
TEST (rpc, sign_hash)
{
	...
	ASSERT_EQ (200, response.status);
	std::error_code ec (btcnew::error_rpc::sign_hash_disabled);
	ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	...
	ASSERT_FALSE (btcnew::validate_message (key.pub, send.hash (), signature));
}
...
TEST (rpc, memory_stats)
{
	...
	auto vote (std::make_shared<btcnew::vote> (key.pub, key.prv, 0, hashes));
...
TEST (rpc, block_confirmed)
{
	...
	ASSERT_EQ (200, response.status);
	ASSERT_EQ (std::error_code (btcnew::error_blocks::invalid_block_hash).message (), response.json.get<std::string> ("error"));
	...
	ASSERT_EQ (200, response1.status);
	ASSERT_EQ (std::error_code (btcnew::error_blocks::not_found).message (), response1.json.get<std::string> ("error"));
	...
	// Open an account directly in the ledger
	{
		...
		btcnew::send_block send1 (latest, key.pub, 300, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (latest));
		ASSERT_EQ (btcnew::process_result::progress, node->ledger.process (transaction, send1).code);

		btcnew::open_block open1 (send1.hash (), btcnew::genesis_account, key.pub, key.prv, key.pub, *system.work.generate (key.pub));
		ASSERT_EQ (btcnew::process_result::progress, node->ledger.process (transaction, open1).code);
	}
	scoped_thread_name_io.renew ();
	...
	auto send = std::make_shared<btcnew::send_block> (latest, key.pub, 10, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (latest));
...
TEST (rpc, database_txn_tracker)
{
	// First try when database tracking is disabled
	{
		...
		request.put ("action", "database_txn_tracker");
		{
			...
			ASSERT_EQ (200, response.status);
			std::error_code ec (btcnew::error_common::tracking_not_enabled);
			ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
		}
	}
	...
	auto check_not_correct_amount = [&system, &request, &rpc_port = rpc.config.port]() {
		...
		ASSERT_EQ (200, response.status);
		std::error_code ec (btcnew::error_common::invalid_amount);
		ASSERT_EQ (response.json.get<std::string> ("error"), ec.message ());
	};
	// clang-format on
	...
	auto thread_name = btcnew::thread_role::get_string (btcnew::thread_role::name::rpc_process_container);
...
TEST (rpc, active_difficulty)
{
	...
	auto expected_multiplier{ (1.5 + 4.2 + (trend_size - 2) * 1) / trend_size };
	{
		...
		ASSERT_FALSE (btcnew::from_string_hex (network_minimum_text, network_minimum));
		...
		ASSERT_FALSE (btcnew::from_string_hex (network_current_text, network_current));
		ASSERT_EQ (btcnew::difficulty::from_multiplier (expected_multiplier, node->network_params.network.publish_threshold), network_current);
...
TEST (rpc, simultaneous_calls)
{
	...
	scoped_io_thread_name_change scoped_thread_name_io;
	auto node = system.nodes.front ();
	btcnew::thread_runner runner (system.io_ctx, node->config.io_threads);
	enable_ipc_transport_tcp (node->config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server (*node, node_rpc_config);
	btcnew::rpc_config rpc_config (true);
...
TEST (rpc, in_process)
{
	...
	btcnew::rpc_config rpc_config (true);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::inprocess_rpc_handler inprocess_rpc_handler (*node, node_rpc_config);
	btcnew::rpc rpc (system.io_ctx, rpc_config, inprocess_rpc_handler);
	rpc.start ();
	boost::property_tree::ptree request;
	request.put ("action", "account_balance");
	request.put ("account", btcnew::test_genesis_key.pub.to_account ());
...
{
	btcnew::jsonconfig rpc;
	rpc.put ("address", "::1");
	rpc.put ("port", 11111);

	bool updated = false;
	auto data_path = btcnew::unique_path ();
	boost::filesystem::create_directory (data_path);
	btcnew::node_rpc_config btcnew_rpc_config;
	btcnew_rpc_config.deserialize_json (updated, rpc, data_path);
	ASSERT_TRUE (updated);

	// Check that the rpc config file is created
	auto rpc_path = btcnew::get_rpc_config_path (data_path);
...
TEST (rpc, deprecated_account_format)
{
	...
	std::string account_text (btcnew::test_genesis_key.pub.to_account ());
	account_text[4] = '-';
...
TEST (rpc, epoch_upgrade)
{
	btcnew::system system (24000, 1);
	auto node = system.nodes.front ();
	btcnew::keypair key1, key2, key3;
	btcnew::genesis genesis;
	btcnew::keypair epoch_signer (btcnew::test_genesis_key);
	auto send1 (std::make_shared<btcnew::state_block> (btcnew::test_genesis_key.pub, genesis.hash (), btcnew::test_genesis_key.pub, btcnew::genesis_amount - 1, key1.pub, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (genesis.hash ()))); // to opened account
	ASSERT_EQ (btcnew::process_result::progress, node->process (*send1).code);
	auto send2 (std::make_shared<btcnew::state_block> (btcnew::test_genesis_key.pub, send1->hash (), btcnew::test_genesis_key.pub, btcnew::genesis_amount - 2, key2.pub, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (send1->hash ()))); // to unopened account (pending)
	ASSERT_EQ (btcnew::process_result::progress, node->process (*send2).code);
	auto send3 (std::make_shared<btcnew::state_block> (btcnew::test_genesis_key.pub, send2->hash (), btcnew::test_genesis_key.pub, btcnew::genesis_amount - 3, 0, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (send2->hash ()))); // to burn (0)
	ASSERT_EQ (btcnew::process_result::progress, node->process (*send3).code);
	btcnew::account max_account (std::numeric_limits<btcnew::uint256_t>::max ());
	auto send4 (std::make_shared<btcnew::state_block> (btcnew::test_genesis_key.pub, send3->hash (), btcnew::test_genesis_key.pub, btcnew::genesis_amount - 4, max_account, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (send3->hash ()))); // to max account
	ASSERT_EQ (btcnew::process_result::progress, node->process (*send4).code);
	auto open (std::make_shared<btcnew::state_block> (key1.pub, 0, key1.pub, 1, send1->hash (), key1.prv, key1.pub, *system.work.generate (key1.pub)));
	ASSERT_EQ (btcnew::process_result::progress, node->process (*open).code);
	// Check accounts epochs
	{
		...
		{
			btcnew::account_info info (i->second);
			ASSERT_EQ (info.epoch (), btcnew::epoch::epoch_0);
		}
	}
	enable_ipc_transport_tcp (node->config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	...
	// Check upgrade
	{
		...
		ASSERT_TRUE (node->store.account_exists (transaction, std::numeric_limits<btcnew::uint256_t>::max ()));
		ASSERT_FALSE (node->store.account_exists (transaction, 0));
	}

	// Epoch 2 upgrade
	auto genesis_latest (node->latest (btcnew::test_genesis_key.pub));
	auto send5 (std::make_shared<btcnew::state_block> (btcnew::test_genesis_key.pub, genesis_latest, btcnew::test_genesis_key.pub, btcnew::genesis_amount - 5, 0, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (genesis_latest))); // to burn (0)
	ASSERT_EQ (btcnew::process_result::progress, node->process (*send5).code);
	auto send6 (std::make_shared<btcnew::state_block> (btcnew::test_genesis_key.pub, send5->hash (), btcnew::test_genesis_key.pub, btcnew::genesis_amount - 6, key1.pub, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (send5->hash ()))); // to key1 (again)
	ASSERT_EQ (btcnew::process_result::progress, node->process (*send6).code);
	auto key1_latest (node->latest (key1.pub));
	auto send7 (std::make_shared<btcnew::state_block> (key1.pub, key1_latest, key1.pub, 0, key3.pub, key1.prv, key1.pub, *system.work.generate (key1_latest))); // to key3
	ASSERT_EQ (btcnew::process_result::progress, node->process (*send7).code);
	{
		// Check pending entry
		auto transaction (node->store.tx_begin_read ());
		btcnew::pending_info info;
		ASSERT_FALSE (node->store.pending_get (transaction, btcnew::pending_key (key3.pub, send7->hash ()), info));
		ASSERT_EQ (btcnew::epoch::epoch_1, info.epoch);
	}
	...
	// Check upgrade
	{
		...
		{
			btcnew::account_info info (i->second);
			ASSERT_EQ (info.epoch (), btcnew::epoch::epoch_2);
		}
		ASSERT_TRUE (node->store.account_exists (transaction, key1.pub));
		ASSERT_TRUE (node->store.account_exists (transaction, key2.pub));
...
TEST (rpc, account_lazy_start)
{
	...
	auto node1 = system.add_node (btcnew::node_config (24000, system.logging), node_flags);
	btcnew::genesis genesis;
	btcnew::keypair key;
	// Generating test chain
	auto send1 (std::make_shared<btcnew::state_block> (btcnew::test_genesis_key.pub, genesis.hash (), btcnew::test_genesis_key.pub, btcnew::genesis_amount - btcnew::Gbtcnew_ratio, key.pub, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (genesis.hash ())));
	...
	enable_ipc_transport_tcp (node2->config.ipc_config.transport_tcp);
	btcnew::node_rpc_config node_rpc_config;
	btcnew::ipc::ipc_server ipc_server (*node2, node_rpc_config);
	btcnew::rpc_config rpc_config (true);
	btcnew::ipc_rpc_processor ipc_rpc_processor (system.io_ctx, rpc_config);
	btcnew::rpc rpc (system.io_ctx, rpc_config, ipc_rpc_processor);
	rpc.start ();
