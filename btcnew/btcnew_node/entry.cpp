...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("debug_output_last_backtrace_dump", "Displays the contents of the latest backtrace in the event of a btcnew_node crash")
		...
		("debug_profile_process", "Profile active blocks processing (only for btcnew_test_network)")
		("debug_profile_votes", "Profile votes processing (only for btcnew_test_network)")
	...
	if (ec == btcnew::error_cli::unknown_command)
	{
		...
		{
			btcnew::inactive_node node (data_path);
		...
		else if (vm.count ("debug_bootstrap_generate"))
		{
			...
			if (key_it != vm.end ())
			{
				...
				{
					btcnew::keypair genesis (key.to_string ());
					...
					btcnew::uint128_t balance (std::numeric_limits<btcnew::uint128_t>::max ());
					btcnew::open_block genesis_block (reinterpret_cast<const btcnew::block_hash &> (genesis.pub), genesis.pub, genesis.pub, genesis.prv, genesis.pub, *work.generate (genesis.pub));
					std::cout << genesis_block.to_json ();
					std::cout.flush ();
					btcnew::block_hash previous (genesis_block.hash ());
					for (auto i (0); i != 8; ++i)
					{
						btcnew::uint128_t yearly_distribution (btcnew::uint128_t (1) << (127 - (i == 7 ? 6 : i)));
						...
						for (auto j (0); j != 52; ++j)
						{
							...
							btcnew::send_block send (previous, landing.pub, balance, genesis.prv, genesis.pub, *work.generate (previous));
							previous = send.hash ();
		...
		else if (vm.count ("debug_dump_representatives"))
		{
			...
			btcnew::inactive_node node (data_path, 24000, node_flags);
			auto transaction (node.node->store.tx_begin_read ());
			btcnew::uint128_t total;
			auto rep_amounts = node.node->ledger.rep_weights.get_rep_amounts ();
			std::map<btcnew::account, btcnew::uint128_t> ordered_reps (rep_amounts.begin (), rep_amounts.end ());
		...
		else if (vm.count ("debug_dump_frontier_unchecked_dependents"))
		{
			...
			auto transaction (node.node->store.tx_begin_read ());
			std::unordered_set<btcnew::block_hash> frontier_hashes;
		...
		else if (vm.count ("debug_profile_generate"))
		{
			...
			btcnew::work_pool work (std::numeric_limits<unsigned>::max (), pow_rate_limiter);
			btcnew::change_block block (0, 0, btcnew::keypair ().prv, 0, 0);
		...
		{
			uint64_t difficulty{ btcnew::network_constants::publish_full_threshold };
			...
			{
				valid = btcnew::work_value (hash, i) > difficulty;
			}
		...
		else if (vm.count ("debug_opencl"))
		{
			...
			bool error (false);
			btcnew::opencl_environment environment (error);
			if (!error)
			{
				...
				{
					if (btcnew::from_string_hex (difficulty_it->second.as<std::string> (), difficulty))
					{
				...
				if (!result)
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							...
							btcnew::opencl_config config (platform, device, threads);
							auto opencl (btcnew::opencl_work::create (true, config, logger));
							btcnew::work_pool work_pool (std::numeric_limits<unsigned>::max (), std::chrono::nanoseconds (0), opencl ? [&opencl] (btcnew::root const & root_a, uint64_t difficulty_a, std::atomic<int> &) {
		...
		{
			if (boost::filesystem::exists ("btcnew_node_backtrace.dump"))
			{
				// There is a backtrace, so output the contents
				std::ifstream ifs ("btcnew_node_backtrace.dump");
		...
		{
			btcnew::keypair key;
			btcnew::uint256_union message;
			auto signature = btcnew::sign_message (key.prv, key.pub, message);
			auto begin (std::chrono::high_resolution_clock::now ());
			for (auto i (0u); i < 1000; ++i)
			{
				btcnew::validate_message (key.pub, message, signature);
			}
		...
		else if (vm.count ("debug_verify_profile_batch"))
		{
			...
			btcnew::uint256_union message;
			btcnew::uint512_union signature (btcnew::sign_message (key.prv, key.pub, message));
			...
			auto begin (std::chrono::high_resolution_clock::now ());
			btcnew::validate_message_batch (messages.data (), lengths.data (), pub_keys.data (), signatures.data (), batch_count, verifications.data ());
			auto end (std::chrono::high_resolution_clock::now ());
		...
		else if (vm.count ("debug_profile_sign"))
		{
			...
			{
				btcnew::keypair key;
				btcnew::block_hash latest (0);
				auto begin1 (std::chrono::high_resolution_clock::now ());
				for (uint64_t balance (0); balance < 1000; ++balance)
				{
					btcnew::send_block send (latest, key.pub, balance, key.prv, key.pub, 0);
					latest = send.hash ();
				}
				auto end1 (std::chrono::high_resolution_clock::now ());
				std::cerr << boost::str (boost::format ("%|1$ 12d|\n") % std::chrono::duration_cast<std::chrono::microseconds> (end1 - begin1).count ());
		...
		else if (vm.count ("debug_profile_process"))
		{
			...
			btcnew::network_params test_params;
			...
			logging.init (path);
			auto node (std::make_shared<btcnew::node> (system.io_ctx, 24001, path, system.alarm, logging, work));
			btcnew::block_hash genesis_latest (node->latest (test_params.ledger.test_genesis_key.pub));
			btcnew::uint128_t genesis_balance (std::numeric_limits<btcnew::uint128_t>::max ());
			// Generating keys
			std::vector<btcnew::keypair> keys (num_accounts);
			std::vector<btcnew::root> frontiers (num_accounts);
			std::vector<btcnew::uint128_t> balances (num_accounts, 1000000000);
			// Generating blocks
			std::deque<std::shared_ptr<btcnew::block>> blocks;
			...
			for (auto i (0); i != num_interations; ++i)
			{
				for (auto j (0); j != num_accounts; ++j)
				{
					...
					auto receive = builder.state ()
					               ...
					               .link (static_cast<btcnew::block_hash const &> (frontiers[j]))
		...
		else if (vm.count ("debug_profile_votes"))
		{
			...
			btcnew::uint128_t genesis_balance (std::numeric_limits<btcnew::uint128_t>::max ());
			// Generating keys
			std::vector<btcnew::keypair> keys (num_representatives);
			btcnew::uint128_t balance ((node->config.online_weight_minimum.number () / num_representatives) + 1);
			...
			// Generating votes
			std::deque<std::shared_ptr<btcnew::vote>> votes;
			for (auto j (0); j != num_representatives; ++j)
			{
				uint64_t sequence (1);
				for (auto & i : blocks)
				{
					auto vote (std::make_shared<btcnew::vote> (keys[j].pub, keys[j].prv, sequence, std::vector<btcnew::block_hash> (1, i->hash ())));
			...
			while (!votes.empty ())
			{
				...
				auto channel (std::make_shared<btcnew::transport::channel_udp> (node->network.udp_channels, node->network.endpoint (), node->network_params.protocol.protocol_version));
		...
		else if (vm.count ("debug_random_feed"))
		{
			 ...
			 *
			 *   ./btcnew_node --debug_random_feed | dieharder -a -g 200
			 */
			btcnew::raw_key seed;
			for (;;)
			{
		...
		else if (vm.count ("debug_rpc"))
		{
			...
			btcnew::inactive_node inactive_node_l (data_path);
			btcnew::node_rpc_config config;
			btcnew::ipc::ipc_server server (*inactive_node_l.node, config);
			btcnew::json_handler handler_l (*inactive_node_l.node, config, command_l.str (), response_handler_l);
		...
		else if (vm.count ("debug_validate_blocks"))
		{
			...
			for (auto i (node.node->store.latest_begin (transaction)), n (node.node->store.latest_end ()); i != n; ++i)
			{
				...
				while (!hash.is_zero () && block != nullptr)
				{
					...
					if (height == 0 && block->type () != btcnew::block_type::open && block->type () != btcnew::block_type::state)
					{
					...
					if (validate_message (account, hash, block->block_signature ()))
					{
						...
						{
							auto & state_block (static_cast<btcnew::state_block &> (*block.get ()));
					...
					if (btcnew::work_validate (*block.get ()))
					{
						std::cerr << boost::str (boost::format ("Invalid work for block %1% value: %2%\n") % hash.to_string () % btcnew::to_string_hex (block->block_work ()));
					}
					...
					if (block->type () == btcnew::block_type::open || block->type () == btcnew::block_type::change || block->type () == btcnew::block_type::state)
					{
			...
			for (auto i (node.node->store.pending_begin (transaction)), n (node.node->store.pending_end ()); i != n; ++i)
			{
				...
				else
				{
					...
					if (auto state = dynamic_cast<btcnew::state_block *> (block.get ()))
					{
					...
					else if (auto send = dynamic_cast<btcnew::send_block *> (block.get ()))
					{
		...
		{
			btcnew::inactive_node node2 (btcnew::unique_path (), 24001);
			btcnew::update_flags (node2.node->flags, vm);
			btcnew::genesis genesis;
			auto begin (std::chrono::high_resolution_clock::now ());
			uint64_t block_count (0);
			size_t count (0);
			{
				btcnew::inactive_node node (data_path, 24000);
				auto transaction (node.node->store.tx_begin_read ());
				...
				for (auto i (node.node->store.latest_begin (transaction)), n (node.node->store.latest_end ()); i != n; ++i)
				{
					...
					while (!hash.is_zero ())
					{
						...
						if (block != nullptr)
						{
							...
							btcnew::unchecked_info unchecked_info (block, account, 0, btcnew::signature_verification::unknown);
		...
		else if (vm.count ("debug_peers"))
		{
			...
			{
				std::cout << boost::str (boost::format ("%1%\n") % btcnew::endpoint (boost::asio::ip::address_v6 (i->first.address_bytes ()), i->first.port ()));
			}
		}
		...
		{
#ifdef BOOST_WINDOWS
			if (!btcnew::event_log_reg_entry_exists () && !btcnew::is_windows_elevated ())
			{
				std::cerr << "The event log requires the HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\EventLog\\BitcoinNew\\BitcoinNew registry entry, run again as administator to create it.\n";
				return 1;
			}
#endif
			btcnew::inactive_node node (data_path);
			node.node->logger.always_log (btcnew::severity_level::error, "Testing system logger");
		}
		else if (vm.count ("debug_account_versions"))
		{
			...
			auto transaction (node.node->store.tx_begin_read ());
			std::vector<std::unordered_set<btcnew::account>> opened_account_versions (btcnew::normalized_epoch (btcnew::epoch::max));
			...
			for (auto i (node.node->store.latest_begin (transaction)), n (node.node->store.latest_end ()); i != n; ++i)
			{
				...
				auto epoch_idx = btcnew::normalized_epoch (account_info.epoch ());
			...
			std::unordered_map<btcnew::account, std::underlying_type_t<btcnew::epoch>> unopened_highest_pending;
			for (auto i (node.node->store.pending_begin (transaction)), n (node.node->store.pending_end ()); i != n; ++i)
			{
				btcnew::pending_key const & key (i->first);
				btcnew::pending_info const & info (i->second);
				...
				if (!exists)
				{
					...
					auto epoch = btcnew::normalized_epoch (info.epoch);
			...
			std::vector<size_t> unopened_account_version_totals (btcnew::normalized_epoch (btcnew::epoch::max));
