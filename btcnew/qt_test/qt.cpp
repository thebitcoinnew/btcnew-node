...
TEST (wallet, construction)
{
	...
	auto wallet_l (system.nodes[0]->wallets.create (btcnew::random_wallet_id ()));
	auto key (wallet_l->deterministic_insert ());
	auto wallet (std::make_shared<btcnew_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key));
	wallet->start ();
...
TEST (wallet, status)
{
	...
	auto wallet (std::make_shared<btcnew_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key.pub));
	wallet->start ();
	auto wallet_has = [wallet] (btcnew_qt::status_types status_ty) {
	...
	system.nodes[0]->network.udp_channels.insert (btcnew::endpoint (boost::asio::ip::address_v6::loopback (), 10000), 0);
	...
	ASSERT_FALSE (wallet_has (btcnew_qt::status_types::synchronizing));
	system.deadline_set (25s);
	while (!wallet_has (btcnew_qt::status_types::synchronizing))
	{
	...
	ASSERT_TRUE (wallet_has (btcnew_qt::status_types::disconnected));
}
...
TEST (wallet, startup_balance)
{
	...
	ASSERT_EQ ("Balance: 0 BTCNEW", wallet->self.balance_label->text ().toStdString ());
}
...
TEST (wallet, select_account)
{
	...
	btcnew::system system (24000, 1);
	auto wallet_l (system.nodes[0]->wallets.create (btcnew::random_wallet_id ()));
	btcnew::public_key key1 (wallet_l->deterministic_insert ());
	btcnew::public_key key2 (wallet_l->deterministic_insert ());
	auto wallet (std::make_shared<btcnew_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key1));
	wallet->start ();
...
TEST (wallet, password_change)
{
	...
	system.wallet (0)->insert_adhoc (btcnew::keypair ().prv);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		account = system.account (transaction, 0);
	}
	auto wallet (std::make_shared<btcnew_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	QTest::mouseClick (wallet->settings_button, Qt::LeftButton);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
...
TEST (wallet, send)
{
	...
	btcnew::public_key key1 (system.wallet (1)->insert_adhoc (btcnew::keypair ().prv));
	auto account (btcnew::test_genesis_key.pub);
	auto wallet (std::make_shared<btcnew_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	ASSERT_NE (wallet->rendering_ratio, btcnew::raw_ratio);
	QTest::mouseClick (wallet->send_blocks, Qt::LeftButton);
	...
	btcnew::uint128_t amount (wallet->node.balance (key1));
...
TEST (wallet, process_block)
{
	...
	btcnew::system system (24000, 1);
	btcnew::account account;
	btcnew::block_hash latest (system.nodes[0]->latest (btcnew::genesis_account));
	system.wallet (0)->insert_adhoc (btcnew::keypair ().prv);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		account = system.account (transaction, 0);
	...
	btcnew::send_block send (latest, key1.pub, 0, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (latest));
	std::string previous;
...
TEST (wallet, create_send)
{
	...
	QTest::keyClicks (wallet->block_creation.account, btcnew::test_genesis_key.pub.to_account ().c_str ());
	...
	bool error (false);
	btcnew::state_block send (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (btcnew::process_result::progress, system.nodes[0]->process (send).code);
	ASSERT_EQ (btcnew::process_result::old, system.nodes[0]->process (send).code);
}
...
TEST (wallet, create_open_receive)
{
	...
	btcnew::system system (24000, 1);
	system.wallet (0)->insert_adhoc (btcnew::test_genesis_key.prv);
	system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key.pub, 100);
	btcnew::block_hash latest1 (system.nodes[0]->latest (btcnew::test_genesis_key.pub));
	system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key.pub, 100);
	btcnew::block_hash latest2 (system.nodes[0]->latest (btcnew::test_genesis_key.pub));
	...
	QTest::keyClicks (wallet->block_creation.representative, btcnew::test_genesis_key.pub.to_account ().c_str ());
	QTest::mouseClick (wallet->block_creation.create, Qt::LeftButton);
	...
	bool error (false);
	btcnew::state_block open (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (btcnew::process_result::progress, system.nodes[0]->process (open).code);
	ASSERT_EQ (btcnew::process_result::old, system.nodes[0]->process (open).code);
	...
	bool error2 (false);
	btcnew::state_block receive (error2, tree2);
	ASSERT_FALSE (error2);
	ASSERT_EQ (btcnew::process_result::progress, system.nodes[0]->process (receive).code);
	ASSERT_EQ (btcnew::process_result::old, system.nodes[0]->process (receive).code);
}
...
TEST (wallet, create_change)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	btcnew::state_block change (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (btcnew::process_result::progress, system.nodes[0]->process (change).code);
	ASSERT_EQ (btcnew::process_result::old, system.nodes[0]->process (change).code);
}
...
TEST (history, short_text)
{
	...
	auto wallet (std::make_shared<btcnew_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	btcnew::mdb_store store (system.nodes[0]->logger, btcnew::unique_path ());
	ASSERT_TRUE (!store.init_error ());
	btcnew::genesis genesis;
	btcnew::ledger ledger (store, system.nodes[0]->stats);
	{
		...
		btcnew::send_block send (latest, btcnew::test_genesis_key.pub, 0, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (latest));
		ASSERT_EQ (btcnew::process_result::progress, ledger.process (transaction, send).code);
		btcnew::receive_block receive (send.hash (), send.hash (), btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (send.hash ()));
		ASSERT_EQ (btcnew::process_result::progress, ledger.process (transaction, receive).code);
		btcnew::change_block change (receive.hash (), key.pub, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (receive.hash ()));
		ASSERT_EQ (btcnew::process_result::progress, ledger.process (transaction, change).code);
	}
	btcnew_qt::history history (ledger, btcnew::test_genesis_key.pub, *wallet);
	history.refresh ();
...
TEST (wallet, startup_work)
{
	...
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		ASSERT_TRUE (wallet->wallet_m->store.work_get (transaction, btcnew::test_genesis_key.pub, work1));
	}
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	while (again)
	{
		ASSERT_NO_ERROR (system.poll ());
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		again = wallet->wallet_m->store.work_get (transaction, btcnew::test_genesis_key.pub, work1);
	}
}
...
TEST (wallet, import)
{
	...
	auto wallet (std::make_shared<btcnew_qt::wallet> (*test_application, processor, *system.nodes[1], system.wallet (1), key2.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, republish)
{
	...
	btcnew::block_hash hash;
	{
		...
		auto latest (system.nodes[0]->ledger.latest (transaction, btcnew::test_genesis_key.pub));
		btcnew::send_block block (latest, key.pub, 0, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (latest));
		hash = block.hash ();
	...
	ASSERT_FALSE (system.nodes[1]->balance (btcnew::test_genesis_key.pub).is_zero ());
	system.deadline_set (10s);
	while (system.nodes[1]->balance (btcnew::test_genesis_key.pub).is_zero ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (wallet, ignore_empty_adhoc)
{
	...
	btcnew::keypair key1;
	system.wallet (0)->insert_adhoc (key1.prv);
	auto wallet (std::make_shared<btcnew_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	ASSERT_EQ (wallet->accounts.window, wallet->main_stack->currentWidget ());
	QTest::keyClicks (wallet->accounts.account_key_line, btcnew::test_genesis_key.prv.data.to_string ().c_str ());
	QTest::mouseClick (wallet->accounts.account_key_button, Qt::LeftButton);
	ASSERT_EQ (1, wallet->accounts.model->rowCount ());
...
TEST (wallet, change_seed)
{
	...
	auto wallet (std::make_shared<btcnew_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), wallet_key));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, seed_work_generation)
{
	...
	btcnew::system system (24000, 1);
	auto key1 (system.wallet (0)->deterministic_insert ());
	auto wallet (std::make_shared<btcnew_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	auto transaction (system.nodes[0]->store.tx_begin_read ());
	ASSERT_FALSE (btcnew::work_validate (system.nodes[0]->ledger.latest_root (transaction, pub), work));
}
...
TEST (wallet, DISABLED_synchronizing)
{
	...
	auto wallet (std::make_shared<btcnew_qt::wallet> (*test_application, processor, *system0.nodes[0], system0.wallet (0), key1));
	wallet->start ();
	{
		auto transaction (system1.nodes[0]->store.tx_begin_write ());
		auto latest (system1.nodes[0]->ledger.latest (transaction, btcnew::genesis_account));
		btcnew::send_block send (latest, key1, 0, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system1.work.generate (latest));
		system1.nodes[0]->ledger.process (transaction, send);
	}
	ASSERT_EQ (0, wallet->active_status.active.count (btcnew_qt::status_types::synchronizing));
	system0.nodes[0]->bootstrap_initiator.bootstrap (system1.nodes[0]->network.endpoint ());
	system1.deadline_set (10s);
	while (wallet->active_status.active.count (btcnew_qt::status_types::synchronizing) == 0)
	{
	...
	while (wallet->active_status.active.count (btcnew_qt::status_types::synchronizing) == 1)
	{
		ASSERT_NO_ERROR (system0.poll ());
		ASSERT_NO_ERROR (system1.poll ());
		test_application->processEvents ();
