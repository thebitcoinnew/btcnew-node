...
void output (const char * str, std::chrono::milliseconds time, Mutex & mutex)
{
	...
	auto stacktrace = btcnew::generate_stacktrace ();
...
template <typename Mutex>
void output_if_held_long_enough (btcnew::timer<std::chrono::milliseconds> & timer, Mutex & mutex)
{
	auto time_held = timer.since_start ();
	if (time_held >= std::chrono::milliseconds (BTCNEW_TIMED_LOCKS))
	{
...
template <typename Mutex>
void output_if_blocked_long_enough (btcnew::timer<std::chrono::milliseconds> & timer, Mutex & mutex)
{
	auto time_blocked = timer.since_start ();
	if (time_blocked >= std::chrono::milliseconds (BTCNEW_TIMED_LOCKS))
	{
