...
REGISTER_ERROR_CODES (btcnew, error_common);
REGISTER_ERROR_CODES (btcnew, error_blocks);
REGISTER_ERROR_CODES (btcnew, error_rpc);
REGISTER_ERROR_CODES (btcnew, error_process);
REGISTER_ERROR_CODES (btcnew, error_config);
...
inline std::error_code make_error_code (boost::system::errc::errc_t e)
{
	...
	::btcnew::error_conversion::generic_category ());
}
}
namespace btcnew
{
namespace error_conversion
{
	...
	inline std::error_code convert (const boost::system::error_code & error)
	{
		...
		return btcnew::error_common::invalid_type_conversion;
	}
}
}
...
public:
	error () = default;
	error (btcnew::error const & error_a) = default;
	error (btcnew::error && error_a) = default;
	...
	{
		code = btcnew::error_common::generic;
		message = std::move (message_a);
	}

	error (std::exception const & exception_a)
	{
		code = btcnew::error_common::exception;
		message = exception_a.what ();
	}

	error & operator= (btcnew::error const & err_a)
	{
		code = err_a.code;
	...
	error & operator= (btcnew::error && err_a)
	{
		code = err_a.code;
	...
	{
		code = btcnew::error_conversion::convert (code_a);
		message.clear ();
		return *this;
	}
	...
	{
		code = btcnew::error_conversion::convert (boost::system::errc::make_error_code (code_a));
		message.clear ();
		return *this;
	}

	/** Set the error to btcnew::error_common::generic and the error message to \p message_a */
	...
	/** Sets the error to btcnew::error_common::exception and adopts the exception error message. */
	...
	error & then (std::function<btcnew::error &()> next)
	{
	...
	error & set (std::string message_a, std::error_code code_a = btcnew::error_common::generic)
	{
		message = message_a;
		code = code_a;
		return *this;
	...
	/** Set a custom error message. If the error code is not set, it will be set to btcnew::error_common::generic. */
...
/**
 * A type that manages a btcnew::error.
 * The default return type is btcnew::error&, though shared_ptr<btcnew::error> is a good option in cases
 * where shared error state is desirable.
 */
template <typename RET_TYPE = btcnew::error &>
class error_aware
{
	static_assert (std::is_same<RET_TYPE, btcnew::error &>::value || std::is_same<RET_TYPE, std::shared_ptr<btcnew::error>>::value, "Must be btcnew::error& or shared_ptr<btcnew::error>");
