...
public:
	...
	timer (btcnew::timer_state state_a, std::string description_a = "timer") :
	desc (description_a)
	{
		if (state_a == btcnew::timer_state::started)
		{
			start ();
		}
	}
	...
	void start ()
	{
		assert (state == btcnew::timer_state::stopped);
		state = btcnew::timer_state::started;
		begin = CLOCK::now ();
	}
	...
	UNIT stop ()
	{
		assert (state == btcnew::timer_state::started);
		state = btcnew::timer_state::stopped;
	...
	btcnew::timer_state current_state () const
	{
		return state;
	}
...
private:
	...
	btcnew::timer_state state{ btcnew::timer_state::stopped };
	std::string desc;
...
class autotimer : public btcnew::timer<UNIT>
{
public:
	autotimer (std::string description_a, std::ostream & stream_a = std::cout) :
	btcnew::timer<UNIT> (description_a), stream (stream_a)
	{
		btcnew::timer<UNIT>::start ();
	}
	~autotimer ()
	{
		btcnew::timer<UNIT>::stop (stream);
	}
