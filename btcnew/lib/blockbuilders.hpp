...
inline btcnew::build_flags operator| (btcnew::build_flags a, btcnew::build_flags b)
{
	return static_cast<btcnew::build_flags> (static_cast<uint8_t> (a) | static_cast<uint8_t> (b));
}
inline uint8_t operator| (uint8_t a, btcnew::build_flags b)
{
...
public:
	...
	inline abstract_builder & sign (btcnew::raw_key const & private_key, btcnew::public_key const & public_key)
	{
		block->signature = btcnew::sign_message (private_key, public_key, block->hash ());
...
protected:
	...
	uint8_t base_fields = static_cast<uint8_t> (btcnew::build_flags::work_present | btcnew::build_flags::signature_present);
};

/** Builder for state blocks */
class state_block_builder : public abstract_builder<btcnew::state_block, state_block_builder>
{
public:
	/** Creates a state block builder by calling make_block() */
	state_block_builder ();
	/** Initialize from an existing block */
	state_block_builder & from (btcnew::state_block const & block);
	...
	/** Set link */
	state_block_builder & link (btcnew::link const & link);
	/** Set link from hex representation */
	state_block_builder & link_hex (std::string const & link_hex);
	/** Set link from an btcnew_ address */
	state_block_builder & link_address (std::string const & link_address);
	/** Provides validation for build() */
	void validate ();

private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (btcnew::build_flags::account_present | btcnew::build_flags::balance_present | btcnew::build_flags::link_present | btcnew::build_flags::previous_present | btcnew::build_flags::representative_present);
};

/** Builder for open blocks */
class open_block_builder : public abstract_builder<btcnew::open_block, open_block_builder>
{
public:
	...
	open_block_builder & source (btcnew::block_hash source);
...
private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (btcnew::build_flags::account_present | btcnew::build_flags::representative_present | btcnew::build_flags::link_present);
};

/** Builder for change blocks */
class change_block_builder : public abstract_builder<btcnew::change_block, change_block_builder>
{
public:
	...
	/** Set representative */
	change_block_builder & representative (btcnew::account account);
	/** Set representative from hex representation of public key */
	...
	/** Set previous block hash */
	change_block_builder & previous (btcnew::block_hash previous);
	/** Set previous block hash from hex representation */
...
private:
	uint8_t required_fields = base_fields | static_cast<uint8_t> (btcnew::build_flags::previous_present | btcnew::build_flags::representative_present);
};

/** Builder for send blocks */
class send_block_builder : public abstract_builder<btcnew::send_block, send_block_builder>
{
public:
...
class receive_block_builder : public abstract_builder<btcnew::receive_block, receive_block_builder>
{
public:
	...
	/** Set previous block hash */
	receive_block_builder & previous (btcnew::block_hash previous);
	/** Set previous block hash from hex representation */
	receive_block_builder & previous_hex (std::string previous_hex);
	/** Set source block hash */
	receive_block_builder & source (btcnew::block_hash source);
	/** Set source block hash from hex representation */
...
public:
	...
	inline btcnew::state_block_builder & state ()
	{
	...
	inline btcnew::open_block_builder & open ()
	{
	...
	inline btcnew::change_block_builder & change ()
	{
	...
	inline btcnew::send_block_builder & send ()
	{
	...
	inline btcnew::receive_block_builder & receive ()
	{
...
private:
	btcnew::state_block_builder state_builder;
	btcnew::open_block_builder open_builder;
	btcnew::change_block_builder change_builder;
	btcnew::send_block_builder send_builder;
	btcnew::receive_block_builder receive_builder;
};
}
