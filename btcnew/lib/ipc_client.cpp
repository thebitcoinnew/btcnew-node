...
public:
	...
	virtual void async_write (btcnew::shared_const_buffer const & buffer_a, std::function<void (boost::system::error_code const &, size_t)> callback_a) = 0;
};
...
class socket_client : public btcnew::ipc::socket_base, public channel
{
public:
	...
	void async_write (btcnew::shared_const_buffer const & buffer_a, std::function<void (boost::system::error_code const &, size_t)> callback_a) override
	{
		this->timer_start (io_timeout);
		btcnew::async_write (socket, buffer_a, boost::asio::bind_executor (this->strand, [this, callback_a] (boost::system::error_code const & ec, size_t size_a) {
			this->timer_cancel ();
			callback_a (ec, size_a);
		}));
	}
 ...
 */
class client_impl : public btcnew::ipc::ipc_client_impl
{
public:
	...
	void connect (std::string const & host_a, uint16_t port_a, std::function<void (btcnew::error)> callback_a)
	{
		...
		tcp_client->async_resolve (host_a, port_a, [this, callback_a] (boost::system::error_code const & ec_resolve_a, boost::asio::ip::tcp::endpoint endpoint_a) {
			if (!ec_resolve_a)
			{
				this->tcp_client->async_connect ([callback_a] (const boost::system::error_code & ec_connect_a) {
					callback_a (btcnew::error (ec_connect_a));
				});
			}
			else
			{
				callback_a (btcnew::error (ec_resolve_a));
			}
		});
	}

	btcnew::error connect (std::string const & path_a)
	{
		btcnew::error err;
...
#else
		err = btcnew::error ("Domain sockets are not supported by this platform");
#endif
		return err;
	}
...
btcnew::ipc::ipc_client::ipc_client (boost::asio::io_context & io_ctx_a) :
io_ctx (io_ctx_a)
{
}

btcnew::error btcnew::ipc::ipc_client::connect (std::string const & path_a)
{
	impl = std::make_unique<client_impl> (io_ctx);
	return boost::polymorphic_downcast<client_impl *> (impl.get ())->connect (path_a);
}

void btcnew::ipc::ipc_client::async_connect (std::string const & host_a, uint16_t port_a, std::function<void (btcnew::error)> callback_a)
{
	impl = std::make_unique<client_impl> (io_ctx);
...
btcnew::error btcnew::ipc::ipc_client::connect (std::string const & host, uint16_t port)
{
	std::promise<btcnew::error> result_l;
	async_connect (host, port, [&result_l] (btcnew::error err_a) {
		result_l.set_value (err_a);
	});
	return result_l.get_future ().get ();
}

void btcnew::ipc::ipc_client::async_write (btcnew::shared_const_buffer const & buffer_a, std::function<void (btcnew::error, size_t)> callback_a)
{
	auto client (boost::polymorphic_downcast<client_impl *> (impl.get ()));
	client->get_channel ().async_write (buffer_a, [callback_a] (const boost::system::error_code & ec_a, size_t bytes_written_a) {
		callback_a (btcnew::error (ec_a), bytes_written_a);
	});
}

void btcnew::ipc::ipc_client::async_read (std::shared_ptr<std::vector<uint8_t>> buffer_a, size_t size_a, std::function<void (btcnew::error, size_t)> callback_a)
{
	auto client (boost::polymorphic_downcast<client_impl *> (impl.get ()));
	client->get_channel ().async_read (buffer_a, size_a, [callback_a] (const boost::system::error_code & ec_a, size_t bytes_read_a) {
		callback_a (btcnew::error (ec_a), bytes_read_a);
	});
}

btcnew::shared_const_buffer btcnew::ipc::prepare_request (btcnew::ipc::payload_encoding encoding_a, std::string const & payload_a)
{
	std::vector<uint8_t> buffer_l;
	if (encoding_a == btcnew::ipc::payload_encoding::json_legacy)
	{
	...
	return btcnew::shared_const_buffer{ std::move (buffer_l) };
}

std::string btcnew::ipc::request (btcnew::ipc::ipc_client & ipc_client, std::string const & rpc_action_a)
{
	auto req (prepare_request (btcnew::ipc::payload_encoding::json_legacy, rpc_action_a));
	auto res (std::make_shared<std::vector<uint8_t>> ());

	std::promise<std::string> result_l;
	// clang-format off
	ipc_client.async_write (req, [&ipc_client, &res, &result_l](btcnew::error err_a, size_t size_a) {
		// Read length
		ipc_client.async_read (res, sizeof (uint32_t), [&ipc_client, &res, &result_l](btcnew::error err_read_a, size_t size_read_a) {
			uint32_t payload_size_l = boost::endian::big_to_native (*reinterpret_cast<uint32_t *> (res->data ()));
			// Read json payload
			ipc_client.async_read (res, payload_size_l, [&res, &result_l](btcnew::error err_read_a, size_t size_read_a) {
