...
public:
	...
	btcnew::error deserialize_json (btcnew::jsonconfig & json);
...
public:
	...
	{
		btcnew::lock_guard<std::mutex> lock (other_a.datapoint_mutex);
		value = other_a.value;
	...
	uint64_t get_value ()
	{
		btcnew::lock_guard<std::mutex> lock (datapoint_mutex);
		return value;
	}
...
public:
	...
	btcnew::observer_set<boost::circular_buffer<stat_datapoint> &> sample_observers;

	/** Observers for count. Called on each update. */
	btcnew::observer_set<uint64_t, uint64_t> count_observers;
};
...
public:
	 ...
	 */
	stat (btcnew::stat_config config);
...
private:
	...
	std::shared_ptr<btcnew::stat_entry> get_entry (uint32_t key);

	/** Get entry for key, creating a new entry if necessary */
	std::shared_ptr<btcnew::stat_entry> get_entry (uint32_t key, size_t sample_interval, size_t max_samples);

	/** Unlocked implementation of get_entry() */
	std::shared_ptr<btcnew::stat_entry> get_entry_impl (uint32_t key, size_t sample_interval, size_t max_samples);
	...
	btcnew::stat_config config;

	/** Stat entries are sorted by key to simplify processing of log output */
	std::map<uint32_t, std::shared_ptr<btcnew::stat_entry>> entries;
