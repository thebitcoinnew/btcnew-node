...
btcnew::error btcnew::rpc_secure_config::serialize_json (btcnew::jsonconfig & json) const
{
...
btcnew::error btcnew::rpc_secure_config::deserialize_json (btcnew::jsonconfig & json)
{
...
btcnew::error btcnew::rpc_secure_config::serialize_toml (btcnew::tomlconfig & toml) const
{
...
btcnew::error btcnew::rpc_secure_config::deserialize_toml (btcnew::tomlconfig & toml)
{
...
btcnew::rpc_config::rpc_config (bool enable_control_a) :
enable_control (enable_control_a)
{
}

btcnew::error btcnew::rpc_config::serialize_json (btcnew::jsonconfig & json) const
{
	...
	btcnew::jsonconfig rpc_process_l;
...
btcnew::error btcnew::rpc_config::deserialize_json (bool & upgraded_a, btcnew::jsonconfig & json)
{
	if (!json.empty ())
	{
...
btcnew::error btcnew::rpc_config::serialize_toml (btcnew::tomlconfig & toml) const
{
	...
	btcnew::tomlconfig rpc_process_l;
...
btcnew::error btcnew::rpc_config::deserialize_toml (btcnew::tomlconfig & toml)
{
	if (!toml.empty ())
	{
...
namespace btcnew
{
btcnew::error read_rpc_config_toml (boost::filesystem::path const & data_path_a, btcnew::rpc_config & config_a, std::vector<std::string> const & config_overrides)
{
	btcnew::error error;
	auto json_config_path = btcnew::get_rpc_config_path (data_path_a);
	auto toml_config_path = btcnew::get_rpc_toml_config_path (data_path_a);
	if (boost::filesystem::exists (json_config_path))
	{
		...
		{
			// Migrate
			btcnew::rpc_config config_json_l;
			...
			if (!error)
			{
				...
				btcnew::rpc_config config_defaults_l;
...
btcnew::error read_and_update_rpc_config (boost::filesystem::path const & data_path, btcnew::rpc_config & config_a)
{
	...
	auto config_path = btcnew::get_rpc_config_path (data_path);
...
std::string get_default_rpc_filepath ()
{
	...
	// Construct the btcnew_rpc executable file path based on where the currently running executable is found.
	auto rpc_filepath = running_executable_filepath.parent_path () / "btcnew_rpc";
