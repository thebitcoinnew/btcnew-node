...
class configbase : public btcnew::error_aware<>
{
public:
	configbase () = default;
	configbase (std::shared_ptr<btcnew::error> const & error_a) :
	error (error_a)
	{
	}

	/** Returns the current error */
	btcnew::error & get_error () override
	{
		return *error;
	}
...
protected:
	...
	/** We're a btcnew::error_aware type. Child nodes share the error state. */
	std::shared_ptr<btcnew::error> error;
