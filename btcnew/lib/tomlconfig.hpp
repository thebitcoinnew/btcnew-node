...
class tomlconfig : public btcnew::configbase
{
public:
	tomlconfig () :
	...
	tomlconfig (std::shared_ptr<cpptoml::table> const & tree_a, std::shared_ptr<btcnew::error> const & error_a = nullptr) :
	...
	btcnew::error & read (std::istream & stream_overrides, boost::filesystem::path const & path_a)
	{
		std::fstream stream;
	...
	tomlconfig & put_child (std::string const & key_a, btcnew::tomlconfig & conf_a)
	{
		tree->insert (key_a, conf_a.get_tree ());
		return *this;
	}

	tomlconfig & replace_child (std::string const & key_a, btcnew::tomlconfig & conf_a)
	{
		tree->erase (key_a);
	...
	tomlconfig & array_entries_required (std::string const & key, std::function<void (T)> callback)
	{
		...
		{
			conditionally_set_error<T> (btcnew::error_config::missing_value, false, key);
		}
		return *this;
	}
