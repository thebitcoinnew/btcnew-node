...
TEST (distributed_work, stopped)
{
	...
	ASSERT_TRUE (system.nodes[0]->distributed_work.make (btcnew::block_hash (), {}, {}, btcnew::network_constants::publish_test_threshold));
}
...
TEST (distributed_work, no_peers)
{
	...
	ASSERT_FALSE (node->distributed_work.make (hash, node->config.work_peers, callback, node->network_params.network.publish_threshold, btcnew::account ()));
	system.deadline_set (5s);
	while (!done)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_FALSE (btcnew::work_validate (hash, *work));
...
TEST (distributed_work, no_peers_disabled)
{
	...
	ASSERT_TRUE (node.distributed_work.make (btcnew::block_hash (), node.config.work_peers, {}, btcnew::network_constants::publish_test_threshold));
}
...
TEST (distributed_work, no_peers_cancel)
{
	...
	ASSERT_FALSE (node.distributed_work.make (hash, node.config.work_peers, callback_to_cancel, btcnew::difficulty::from_multiplier (1e6, node.network_params.network.publish_threshold)));
	ASSERT_EQ (1, node.distributed_work.items.size ());
...
TEST (distributed_work, no_peers_multi)
{
	...
	{
		ASSERT_FALSE (node->distributed_work.make (hash, node->config.work_peers, callback, btcnew::difficulty::from_multiplier (10, node->network_params.network.publish_threshold)));
	}
