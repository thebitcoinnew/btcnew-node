...
{
	btcnew::signature_checker checker (0);
	btcnew::signature_check_set check = { 0, nullptr, nullptr, nullptr, nullptr, nullptr };
...
TEST (signature_checker, bulk_single_thread)
{
	...
	btcnew::signature_checker checker (0);
	std::vector<btcnew::uint256_union> hashes;
	size_t size (1000);
	hashes.reserve (size);
...
{
	btcnew::signature_checker checker (4);
	...
	auto signature_checker_work_func = [&checker] () {
		...
		btcnew::state_block invalid_block (key.pub, 0, key.pub, 0, 0, key.prv, key.pub, 0);
		...
		std::vector<btcnew::signature_check_set> signature_checker_sets;
