...
{
	btcnew::system system (24000, 1);
	btcnew::message_buffer data;
	data.endpoint = system.nodes[0]->network.endpoint ();
	ASSERT_EQ (0, system.nodes[0]->stats.count (btcnew::stat::type::error, btcnew::stat::detail::bad_sender));
	system.nodes[0]->network.udp_channels.receive_action (&data);
	ASSERT_EQ (1, system.nodes[0]->stats.count (btcnew::stat::type::error, btcnew::stat::detail::bad_sender));
}
...
TEST (network, send_node_id_handshake)
{
	...
	auto initial (system.nodes[0]->stats.count (btcnew::stat::type::message, btcnew::stat::detail::node_id_handshake, btcnew::stat::dir::in));
	auto initial_node1 (node1->stats.count (btcnew::stat::type::message, btcnew::stat::detail::node_id_handshake, btcnew::stat::dir::in));
	auto channel (std::make_shared<btcnew::transport::channel_udp> (system.nodes[0]->network.udp_channels, node1->network.endpoint (), node1->network_params.protocol.protocol_version));
	system.nodes[0]->network.send_keepalive (channel);
	ASSERT_EQ (0, system.nodes[0]->network.size ());
	ASSERT_EQ (0, node1->network.size ());
	system.deadline_set (10s);
	while (node1->stats.count (btcnew::stat::type::message, btcnew::stat::detail::node_id_handshake, btcnew::stat::dir::in) == initial_node1)
	{
	...
	system.deadline_set (10s);
	while (system.nodes[0]->stats.count (btcnew::stat::type::message, btcnew::stat::detail::node_id_handshake, btcnew::stat::dir::in) < initial + 2)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
...
TEST (network, send_node_id_handshake_tcp)
{
	...
	auto initial (system.nodes[0]->stats.count (btcnew::stat::type::message, btcnew::stat::detail::node_id_handshake, btcnew::stat::dir::in));
	auto initial_node1 (node1->stats.count (btcnew::stat::type::message, btcnew::stat::detail::node_id_handshake, btcnew::stat::dir::in));
	auto initial_keepalive (system.nodes[0]->stats.count (btcnew::stat::type::message, btcnew::stat::detail::keepalive, btcnew::stat::dir::in));
	std::weak_ptr<btcnew::node> node_w (system.nodes[0]);
	system.nodes[0]->network.tcp_channels.start_tcp (node1->network.endpoint (), [node_w] (std::shared_ptr<btcnew::transport::channel> channel_a) {
	...
	while (node1->stats.count (btcnew::stat::type::message, btcnew::stat::detail::node_id_handshake, btcnew::stat::dir::in) < initial_node1 + 2)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	system.deadline_set (5s);
	while (system.nodes[0]->stats.count (btcnew::stat::type::message, btcnew::stat::detail::keepalive, btcnew::stat::dir::in) < initial_keepalive + 2)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	system.deadline_set (5s);
	while (node1->stats.count (btcnew::stat::type::message, btcnew::stat::detail::keepalive, btcnew::stat::dir::in) < initial_keepalive + 2)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_EQ (1, system.nodes[0]->network.size ());
...
TEST (network, last_contacted)
{
	...
	node1->start ();
	system.nodes.push_back (node1);
	auto channel1 (std::make_shared<btcnew::transport::channel_udp> (node1->network.udp_channels, btcnew::endpoint (boost::asio::ip::address_v6::loopback (), 24000), node1->network_params.protocol.protocol_version));
	...
	auto channel2 (system.nodes[0]->network.udp_channels.channel (btcnew::endpoint (boost::asio::ip::address_v6::loopback (), 24001)));
	...
	node1->network.send_keepalive (channel1);
	while (system.nodes[0]->stats.count (btcnew::stat::type::message, btcnew::stat::detail::keepalive, btcnew::stat::dir::in) < 2)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_EQ (system.nodes[0]->network.size (), 1);
...
TEST (network, multi_keepalive)
{
	...
	auto node2 (std::make_shared<btcnew::node> (system.io_ctx, 24002, btcnew::unique_path (), system.alarm, system.logging, system.work));
	ASSERT_FALSE (node2->init_error ());
	node2->start ();
	system.nodes.push_back (node2);
	auto channel2 (std::make_shared<btcnew::transport::channel_udp> (node2->network.udp_channels, system.nodes[0]->network.endpoint (), node2->network_params.protocol.protocol_version));
...
TEST (network, send_discarded_publish)
{
	...
	btcnew::genesis genesis;
	{
		...
		ASSERT_EQ (genesis.hash (), system.nodes[0]->ledger.latest (transaction, btcnew::test_genesis_key.pub));
		ASSERT_EQ (genesis.hash (), system.nodes[1]->latest (btcnew::test_genesis_key.pub));
	}
	system.deadline_set (10s);
	while (system.nodes[1]->stats.count (btcnew::stat::type::message, btcnew::stat::detail::publish, btcnew::stat::dir::in) == 0)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	auto transaction (system.nodes[0]->store.tx_begin_read ());
...
TEST (network, send_invalid_publish)
{
	btcnew::system system (24000, 2);
	btcnew::genesis genesis;
	auto block (std::make_shared<btcnew::send_block> (1, 1, 20, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (btcnew::root (1))));
	{
		auto transaction (system.nodes[0]->store.tx_begin_read ());
		system.nodes[0]->network.flood_block (block);
		ASSERT_EQ (genesis.hash (), system.nodes[0]->ledger.latest (transaction, btcnew::test_genesis_key.pub));
...
TEST (network, send_valid_confirm_ack)
{
	...
	for (auto & type : types)
	{
		...
		btcnew::send_block block2 (latest1, key2.pub, 50, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (latest1));
		btcnew::block_hash latest2 (system.nodes[1]->latest (btcnew::test_genesis_key.pub));
		system.nodes[0]->process_active (std::make_shared<btcnew::send_block> (block2));
		system.deadline_set (10s);
		// Keep polling until latest block changes
		while (system.nodes[1]->latest (btcnew::test_genesis_key.pub) == latest2)
		{
			ASSERT_NO_ERROR (system.poll ());
		}
		// Make sure the balance has decreased after processing the block.
		ASSERT_EQ (50, system.nodes[1]->balance (btcnew::test_genesis_key.pub));
	}
}
...
TEST (network, send_valid_publish)
{
	...
	for (auto & type : types)
	{
		...
		btcnew::block_hash latest2 (system.nodes[1]->latest (btcnew::test_genesis_key.pub));
		system.nodes[1]->process_active (std::make_shared<btcnew::send_block> (block2));
		system.deadline_set (10s);
		while (system.nodes[0]->stats.count (btcnew::stat::type::message, btcnew::stat::detail::publish, btcnew::stat::dir::in) == 0)
		{
			ASSERT_NO_ERROR (system.poll ());
		}
...
{
	btcnew::system system (24000, 2);
	auto block (std::make_shared<btcnew::send_block> (0, 1, 20, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, 0));
	btcnew::publish publish (block);
	auto node1 (system.nodes[1]->shared ());
	btcnew::transport::channel_udp channel (system.nodes[0]->network.udp_channels, system.nodes[1]->network.endpoint (), system.nodes[0]->network_params.protocol.protocol_version);
	channel.send (publish, [] (boost::system::error_code const & ec, size_t size) {});
	ASSERT_EQ (0, system.nodes[0]->stats.count (btcnew::stat::type::error, btcnew::stat::detail::insufficient_work));
	system.deadline_set (10s);
	while (system.nodes[1]->stats.count (btcnew::stat::type::error, btcnew::stat::detail::insufficient_work) == 0)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	ASSERT_EQ (1, system.nodes[1]->stats.count (btcnew::stat::type::error, btcnew::stat::detail::insufficient_work));
}
...
TEST (receivable_processor, confirm_insufficient_pos)
{
	...
	btcnew::genesis genesis;
	auto block1 (std::make_shared<btcnew::send_block> (genesis.hash (), 0, 0, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, 0));
	node1.work_generate_blocking (*block1);
	ASSERT_EQ (btcnew::process_result::progress, node1.process (*block1).code);
	auto node_l (system.nodes[0]);
	node1.active.start (block1);
	btcnew::keypair key1;
	auto vote (std::make_shared<btcnew::vote> (key1.pub, key1.prv, 0, block1));
...
TEST (receivable_processor, confirm_sufficient_pos)
{
	...
	auto node_l (system.nodes[0]);
	node1.active.start (block1);
	auto vote (std::make_shared<btcnew::vote> (btcnew::test_genesis_key.pub, btcnew::test_genesis_key.prv, 0, block1));
	btcnew::confirm_ack con1 (vote);
	node1.network.process_message (con1, node1.network.udp_channels.create (node1.network.endpoint ()));
}
...
TEST (receivable_processor, send_with_receive)
{
	...
	for (auto & type : types)
	{
		...
		btcnew::block_hash latest1 (system.nodes[0]->latest (btcnew::test_genesis_key.pub));
		system.wallet (1)->insert_adhoc (key2.prv);
		auto block1 (std::make_shared<btcnew::send_block> (latest1, key2.pub, amount - system.nodes[0]->config.receive_minimum.number (), btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (latest1)));
		ASSERT_EQ (amount, system.nodes[0]->balance (btcnew::test_genesis_key.pub));
		ASSERT_EQ (0, system.nodes[0]->balance (key2.pub));
		ASSERT_EQ (amount, system.nodes[1]->balance (btcnew::test_genesis_key.pub));
		...
		ASSERT_EQ (amount - system.nodes[0]->config.receive_minimum.number (), system.nodes[0]->balance (btcnew::test_genesis_key.pub));
		ASSERT_EQ (0, system.nodes[0]->balance (key2.pub));
		ASSERT_EQ (amount - system.nodes[0]->config.receive_minimum.number (), system.nodes[1]->balance (btcnew::test_genesis_key.pub));
		ASSERT_EQ (0, system.nodes[1]->balance (key2.pub));
		system.deadline_set (10s);
...
TEST (network, receive_weight_change)
{
	...
	system.deadline_set (10s);
	while (std::any_of (system.nodes.begin (), system.nodes.end (), [&] (std::shared_ptr<btcnew::node> const & node_a) { return node_a->weight (key2.pub) != system.nodes[0]->config.receive_minimum.number (); }))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (parse_endpoint, valid)
{
	...
	ASSERT_FALSE (btcnew::parse_endpoint (string, endpoint));
...
TEST (parse_endpoint, invalid_port)
{
	...
	btcnew::endpoint endpoint;
	ASSERT_TRUE (btcnew::parse_endpoint (string, endpoint));
}
...
TEST (network, ipv6)
{
	...
	btcnew::endpoint endpoint1 (address, 16384);
	std::vector<uint8_t> bytes1;
	{
		btcnew::vectorstream stream (bytes1);
		btcnew::write (stream, address.to_bytes ());
	}
	...
	btcnew::bufferstream stream (bytes1.data (), bytes1.size ());
	auto error (btcnew::try_read (stream, bytes2));
	ASSERT_FALSE (error);
	btcnew::endpoint endpoint2 (boost::asio::ip::address_v6 (bytes2), 16384);
	ASSERT_EQ (endpoint1, endpoint2);
}

TEST (network, ipv6_from_ipv4)
{
	btcnew::endpoint endpoint1 (boost::asio::ip::address_v4::loopback (), 16000);
	ASSERT_TRUE (endpoint1.address ().is_v4 ());
	btcnew::endpoint endpoint2 (boost::asio::ip::address_v6::v4_mapped (endpoint1.address ().to_v4 ()), 16000);
...
{
	boost::asio::io_context io_ctx;
	btcnew::endpoint endpoint1 (boost::asio::ip::address_v6::any (), 24000);
	btcnew::endpoint endpoint2 (boost::asio::ip::address_v4::any (), 24001);
	std::array<uint8_t, 16> bytes1;
	auto finish1 (false);
	btcnew::endpoint endpoint3;
	...
	btcnew::endpoint endpoint5 (boost::asio::ip::address_v4::loopback (), 24000);
	btcnew::endpoint endpoint6 (boost::asio::ip::address_v6::v4_mapped (boost::asio::ip::address_v4::loopback ()), 24001);
...
TEST (network, reserved_address)
{
	btcnew::system system (24000, 1);
	// 0 port test
	ASSERT_TRUE (btcnew::transport::reserved_address (btcnew::endpoint (boost::asio::ip::address_v6::from_string ("2001::"), 0)));
	// Valid address test
	ASSERT_FALSE (btcnew::transport::reserved_address (btcnew::endpoint (boost::asio::ip::address_v6::from_string ("2001::"), 1)));
	btcnew::endpoint loopback (boost::asio::ip::address_v6::from_string ("::1"), 1);
	ASSERT_FALSE (btcnew::transport::reserved_address (loopback));
	btcnew::endpoint private_network_peer (boost::asio::ip::address_v6::from_string ("::ffff:10.0.0.0"), 1);
	ASSERT_TRUE (btcnew::transport::reserved_address (private_network_peer, false));
	ASSERT_FALSE (btcnew::transport::reserved_address (private_network_peer, true));
}
...
{
	btcnew::stat stats;
	btcnew::message_buffer_manager buffer (stats, 512, 1);
...
{
	btcnew::stat stats;
	btcnew::message_buffer_manager buffer (stats, 512, 2);
	auto buffer1 (buffer.allocate ());
	ASSERT_NE (nullptr, buffer1);
...
{
	btcnew::stat stats;
	btcnew::message_buffer_manager buffer (stats, 512, 16);
...
TEST (message_buffer_manager, stats)
{
	...
	buffer.enqueue (buffer1);
	buffer.allocate ();
	ASSERT_EQ (1, stats.count (btcnew::stat::type::udp, btcnew::stat::detail::overflow));
}

TEST (tcp_listener, tcp_node_id_handshake)
{
	btcnew::system system (24000, 1);
	auto socket (std::make_shared<btcnew::socket> (system.nodes[0]));
	auto bootstrap_endpoint (system.nodes[0]->bootstrap.endpoint ());
	auto cookie (system.nodes[0]->network.syn_cookies.assign (btcnew::transport::map_tcp_to_endpoint (bootstrap_endpoint)));
	btcnew::node_id_handshake node_id_handshake (cookie, boost::none);
	...
	boost::optional<std::pair<btcnew::account, btcnew::signature>> response_zero (std::make_pair (btcnew::account (0), btcnew::signature (0)));
	btcnew::node_id_handshake node_id_handshake_response (boost::none, response_zero);
...
TEST (tcp_listener, tcp_listener_timeout_empty)
{
	btcnew::system system (24000, 1);
	auto node0 (system.nodes[0]);
	auto socket (std::make_shared<btcnew::socket> (node0));
	...
	{
		{
			btcnew::lock_guard<std::mutex> guard (node0->bootstrap.mutex);
...
TEST (tcp_listener, tcp_listener_timeout_node_id_handshake)
{
	...
	auto node0 (system.nodes[0]);
	auto socket (std::make_shared<btcnew::socket> (node0));
	auto cookie (node0->network.syn_cookies.assign (btcnew::transport::map_tcp_to_endpoint (node0->bootstrap.endpoint ())));
	btcnew::node_id_handshake node_id_handshake (cookie, boost::none);
	auto input (node_id_handshake.to_shared_const_buffer ());
	...
	system.deadline_set (std::chrono::seconds (5));
	while (node0->stats.count (btcnew::stat::type::message, btcnew::stat::detail::node_id_handshake) == 0)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	{
...
TEST (network, replace_port)
{
	...
	system.nodes.push_back (node1);
	{
		auto channel (system.nodes[0]->network.udp_channels.insert (btcnew::endpoint (node1->network.endpoint ().address (), 23000), node1->network_params.protocol.protocol_version));
		if (channel)
		{
	...
	system.nodes[0]->network.udp_channels.clean_node_id (btcnew::endpoint (node1->network.endpoint ().address (), 23000), node1->node_id.pub);
...
TEST (bandwidth_limiter, validate)
{
	...
	{
		btcnew::bandwidth_limiter limiter_0 (0);
		btcnew::bandwidth_limiter limiter_1 (1024);
		btcnew::bandwidth_limiter limiter_256 (1024 * 256);
		btcnew::bandwidth_limiter limiter_1024 (1024 * 1024);
		btcnew::bandwidth_limiter limiter_1536 (1024 * 1536);
