...
TEST (work, one)
{
	...
	btcnew::change_block block (1, 1, btcnew::keypair ().prv, 3, 4);
...
{
	btcnew::network_constants network_constants;
	btcnew::work_pool pool (0);
	auto result (pool.generate (btcnew::block_hash ()));
...
TEST (work, validate)
{
	btcnew::network_constants network_constants;
	btcnew::work_pool pool (std::numeric_limits<unsigned>::max ());
	btcnew::send_block send_block (1, 1, 2, btcnew::keypair ().prv, 4, 6);
	uint64_t difficulty;
	ASSERT_TRUE (btcnew::work_validate (send_block, &difficulty));
	ASSERT_LT (difficulty, network_constants.publish_threshold);
	send_block.block_work_set (*pool.generate (send_block.root ()));
	ASSERT_FALSE (btcnew::work_validate (send_block, &difficulty));
	ASSERT_LT (network_constants.publish_threshold, difficulty);
}
...
TEST (work, opencl)
{
	...
	{
		btcnew::opencl_config config (0, 0, 16 * 1024);
		auto opencl (btcnew::opencl_work::create (true, config, logger));
		if (opencl != nullptr)
		{
			// 0 threads, should add 1 for managing OpenCL
			btcnew::work_pool pool (0, std::chrono::nanoseconds (0), [&opencl] (btcnew::root const & root_a, uint64_t difficulty_a, std::atomic<int> & ticket_a) {
			...
			for (auto i (0); i < 16; ++i)
			{
				btcnew::random_pool::generate_block (root.bytes.data (), root.bytes.size ());
				auto result (*pool.generate (root, difficulty));
				uint64_t result_difficulty (0);
				ASSERT_FALSE (btcnew::work_validate (root, result, &result_difficulty));
...
{
	btcnew::opencl_config config1;
	config1.platform = 1;
	config1.device = 2;
	config1.threads = 3;
	btcnew::jsonconfig tree;
	config1.serialize_json (tree);
	btcnew::opencl_config config2;
...
TEST (work, difficulty)
{
	...
	do
	{
		...
		btcnew::work_validate (root, work1, &nonce1);
	...
	do
	{
		...
		btcnew::work_validate (root, work2, &nonce2);
...
TEST (work, eco_pow)
{
	auto work_func = [] (std::promise<std::chrono::nanoseconds> & promise, std::chrono::nanoseconds interval) {
		btcnew::work_pool pool (1, interval);
		constexpr auto num_iterations = 5;

		btcnew::timer<std::chrono::nanoseconds> timer;
		timer.start ();
		for (int i = 0; i < num_iterations; ++i)
		{
			...
			do
			{
				...
				btcnew::work_validate (root, work, &nonce);
