...
TEST (toml, diff)
{
	btcnew::tomlconfig defaults, other;
...
TEST (toml, daemon_config_update_array)
{
	...
	btcnew::daemon_config c (data_path);
...
TEST (toml, rpc_config_deserialize_defaults)
{
	...
	btcnew::tomlconfig t;
	t.read (ss);
	btcnew::rpc_config conf;
	btcnew::rpc_config defaults;
...
TEST (toml, daemon_config_deserialize_defaults)
{
	...
	btcnew::tomlconfig t;
	t.read (ss);
	btcnew::daemon_config conf;
	btcnew::daemon_config defaults;
	conf.deserialize_toml (t);
...
TEST (toml, base_override)
{
	...
	ASSERT_TRUE (t.get_error () == btcnew::error_config::missing_value);
	t.get_error ().clear ();

	// Query uint16 that's too big, make sure we have an error
	t.get_required<uint16_t> ("node.too_big", port);
	ASSERT_TRUE (t.get_error ());
	ASSERT_TRUE (t.get_error () == btcnew::error_config::invalid_value);
}

TEST (toml, put)
{
	btcnew::tomlconfig config;
	btcnew::tomlconfig config_node;
...
TEST (toml, daemon_config_deserialize_no_defaults)
{
	...
	preconfigured_representatives = ["btcnew_3arg3asgtigae3xckabaaewkx3bzsh7nwz7jkmjos79ihyaxwphhm6qgjps4"]
	receive_minimum = "999"
...
TEST (toml, daemon_config_deserialize_errors)
{
	...
	btcnew::tomlconfig toml2;
	toml2.read (ss_frontiers_confirmation);
	btcnew::daemon_config conf2;
	...
	ASSERT_EQ (conf2.node.frontiers_confirmation, btcnew::frontiers_confirmation_mode::invalid);
}
