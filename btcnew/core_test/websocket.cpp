...
TEST (websocket, subscription_edge)
{
	...
	auto node1 (std::make_shared<btcnew::node> (system.io_ctx, btcnew::unique_path (), system.alarm, config, system.work, node_flags));
	node1->start ();
	system.nodes.push_back (node1);

	ASSERT_EQ (0, node1->websocket_server->subscriber_count (btcnew::websocket::topic::confirmation));
	...
	// First subscription
	{
		...
		ASSERT_EQ (1, node1->websocket_server->subscriber_count (btcnew::websocket::topic::confirmation));
	}
...
TEST (websocket, active_difficulty)
{
	...
	ASSERT_EQ (0, node1->websocket_server->subscriber_count (btcnew::websocket::topic::active_difficulty));
	...
	ASSERT_EQ (1, node1->websocket_server->subscriber_count (btcnew::websocket::topic::active_difficulty));
	...
	btcnew::from_string_hex (message_contents.get<std::string> ("network_minimum"), network_minimum);
	ASSERT_EQ (network_minimum, node1->network_params.network.publish_threshold);

	uint64_t network_current;
	btcnew::from_string_hex (message_contents.get<std::string> ("network_current"), network_current);
	ASSERT_EQ (network_current, node1->active.active_difficulty ());

	double multiplier = message_contents.get<double> ("multiplier");
	ASSERT_NEAR (multiplier, btcnew::difficulty::to_multiplier (node1->active.active_difficulty (), node1->network_params.network.publish_threshold), 1e-6);
...
TEST (websocket, confirmation)
{
	...
	auto node1 (std::make_shared<btcnew::node> (system.io_ctx, btcnew::unique_path (), system.alarm, config, system.work, node_flags));
	node1->wallets.create (btcnew::random_wallet_id ());
	node1->start ();
	system.nodes.push_back (node1);
	...
	ASSERT_FALSE (node1->websocket_server->any_subscriber (btcnew::websocket::topic::confirmation));
	...
	ASSERT_TRUE (node1->websocket_server->any_subscriber (btcnew::websocket::topic::confirmation));

	btcnew::keypair key;
	system.wallet (1)->insert_adhoc (btcnew::test_genesis_key.prv);
	auto balance = btcnew::genesis_amount;
	...
	{
		btcnew::block_hash previous (node1->latest (btcnew::test_genesis_key.pub));
		balance -= send_amount;
		auto send (std::make_shared<btcnew::send_block> (previous, key.pub, balance, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (previous)));
	...
	// Quick confirm a state block
	{
		btcnew::block_hash previous (node1->latest (btcnew::test_genesis_key.pub));
		balance -= send_amount;
		auto send (std::make_shared<btcnew::state_block> (btcnew::test_genesis_key.pub, previous, btcnew::test_genesis_key.pub, balance, key.pub, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (previous)));
		node1->process_active (send);
	}
...
TEST (websocket, confirmation_options)
{
	...
	std::thread client_thread ([&client_thread_finished] () {
		...
		auto response = websocket_test_call ("::1", "24078",
		R"json({"action": "subscribe", "topic": "confirmation", "ack": "true", "options": {"confirmation_type": "active_quorum", "accounts": ["btcnew_invalid"]}})json", true, true, 1s);
...
TEST (websocket, vote)
{
	...
	ack_ready = false;
	std::atomic<bool> client_thread_finished{ false };
	ASSERT_FALSE (node1->websocket_server->any_subscriber (btcnew::websocket::topic::vote));
	std::thread client_thread ([&client_thread_finished] () {
		// This will expect two results: the acknowledgement of the subscription
	...
	ASSERT_TRUE (node1->websocket_server->any_subscriber (btcnew::websocket::topic::vote));

	// Quick-confirm a block
	btcnew::keypair key;
	system.wallet (1)->insert_adhoc (btcnew::test_genesis_key.prv);
	btcnew::block_hash previous (node1->latest (btcnew::test_genesis_key.pub));
	auto send (std::make_shared<btcnew::state_block> (btcnew::test_genesis_key.pub, previous, btcnew::test_genesis_key.pub, btcnew::genesis_amount - (node1->config.online_weight_minimum.number () + 1), key.pub, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *system.work.generate (previous)));
	node1->process_active (send);
...
TEST (websocket, vote_options)
{
	...
	std::thread client_thread ([&client_thread_finished] () {
		...
		data << R"json({"action": "subscribe", "topic": "vote", "ack": true, "options": {"representatives": [")json"
		     << btcnew::test_genesis_key.pub.to_account ()
		     << R"json("]}})json";
	...
	// Wait for the websocket client to receive the vote message
	system.deadline_set (5s);
	while (!client_thread_finished || node1->websocket_server->any_subscriber (btcnew::websocket::topic::vote))
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	...
	std::thread client_thread_2 ([&client_thread_2_finished] () {
		auto response = websocket_test_call ("::1", "24078",
		R"json({"action": "subscribe", "topic": "vote", "ack": true, "options": {"representatives": ["btcnew_invalid"]}})json", true, true, 1s);
...
TEST (websocket, work)
{
	...
	ASSERT_EQ (0, node1->websocket_server->subscriber_count (btcnew::websocket::topic::work));
	...
	ASSERT_EQ (1, node1->websocket_server->subscriber_count (btcnew::websocket::topic::work));
	...
	ASSERT_EQ (request.get<std::string> ("difficulty"), btcnew::to_string_hex (node1->network_params.network.publish_threshold));
	ASSERT_EQ (request.get<double> ("multiplier"), 1.0);

	ASSERT_EQ (1, contents.count ("result"));
	auto & result = contents.get_child ("result");
	uint64_t result_difficulty;
	btcnew::from_string_hex (result.get<std::string> ("difficulty"), result_difficulty);
	ASSERT_GE (result_difficulty, node1->network_params.network.publish_threshold);
	ASSERT_NEAR (result.get<double> ("multiplier"), btcnew::difficulty::to_multiplier (result_difficulty, node1->network_params.network.publish_threshold), 1e-6);
	ASSERT_EQ (result.get<std::string> ("work"), btcnew::to_string_hex (work.get ()));
