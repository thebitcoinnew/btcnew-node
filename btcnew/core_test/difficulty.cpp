...
TEST (difficulty, multipliers)
{
...

	{
		...
		ASSERT_NEAR (expected_multiplier, btcnew::difficulty::to_multiplier (difficulty, base), 1e-10);
		ASSERT_EQ (difficulty, btcnew::difficulty::from_multiplier (expected_multiplier, base));
	}
...

	{
		...
		if (!btcnew::running_within_valgrind ())
		{
			uint64_t base = 0xffffffc000000000;
			uint64_t difficulty_nil = 0;
			double multiplier_nil = 0.;

			ASSERT_DEATH_IF_SUPPORTED (btcnew::difficulty::to_multiplier (difficulty_nil, base), "");
			ASSERT_DEATH_IF_SUPPORTED (btcnew::difficulty::from_multiplier (multiplier_nil, base), "");
		}
#endif
	}
}
...
{
	ASSERT_NEAR (16., btcnew::difficulty::to_multiplier (btcnew::network_constants::publish_full_threshold, btcnew::network_constants::publish_beta_threshold), 1e-10);
}
...
TEST (difficulty, overflow)
{
	// Overflow max (attempt to overflow & receive lower difficulty)
	{
		...
		ASSERT_EQ (difficulty, btcnew::difficulty::from_multiplier (multiplier, base));
	}
