...
unsigned constexpr btcnew::wallet_store::version_current;

TEST (wallet, no_special_keys_accounts)
{
	bool init;
	btcnew::mdb_env env (init, btcnew::unique_path ());
	ASSERT_FALSE (init);
	auto transaction (env.tx_begin_write ());
	btcnew::kdf kdf;
	btcnew::wallet_store wallet (init, kdf, transaction, btcnew::genesis_account, 1, "0");
	ASSERT_FALSE (init);
	btcnew::keypair key1;
	...
	for (uint64_t account = 0; account < btcnew::wallet_store::special_count; account++)
	{
		btcnew::account account_l (account);
...
TEST (wallet, one_item_iteration)
{
	...
	{
		ASSERT_EQ (key1.pub, btcnew::uint256_union (i->first));
		btcnew::raw_key password;
		wallet.wallet_key (password, transaction);
		btcnew::raw_key key;
		key.decrypt (btcnew::wallet_value (i->second).key, password, (btcnew::uint256_union (i->first)).owords[0].number ());
...
TEST (wallet, two_item_iteration)
{
	...
	std::unordered_set<btcnew::public_key> pubs;
	std::unordered_set<btcnew::private_key> prvs;
	btcnew::kdf kdf;
	{
		auto transaction (env.tx_begin_write ());
		btcnew::wallet_store wallet (init, kdf, transaction, btcnew::genesis_account, 1, "0");
		...
		for (auto i (wallet.begin (transaction)), j (wallet.end ()); i != j; ++i)
		{
			...
			wallet.wallet_key (password, transaction);
			btcnew::raw_key key;
			key.decrypt (btcnew::wallet_value (i->second).key, password, (i->first).owords[0].number ());
...
TEST (wallet, insufficient_spend_one)
{
	...
	auto block (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key1.pub, 500));
	ASSERT_NE (nullptr, block);
	ASSERT_EQ (nullptr, system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key1.pub, btcnew::genesis_amount));
}
...
TEST (wallet, spend_all_one)
{
	...
	system.wallet (0)->insert_adhoc (btcnew::test_genesis_key.prv);
	btcnew::keypair key2;
	ASSERT_NE (nullptr, system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key2.pub, std::numeric_limits<btcnew::uint128_t>::max ()));
	btcnew::account_info info2;
	{
		auto transaction (system.nodes[0]->store.tx_begin_read ());
		system.nodes[0]->store.account_get (transaction, btcnew::test_genesis_key.pub, info2);
...
TEST (wallet, send_async)
{
	...
	boost::thread thread ([&system] () {
		...
		while (!system.nodes[0]->balance (btcnew::test_genesis_key.pub).is_zero ())
		{
			ASSERT_NO_ERROR (system.poll ());
		}
	});
	std::atomic<bool> success (false);
	system.wallet (0)->send_async (btcnew::test_genesis_key.pub, key2.pub, std::numeric_limits<btcnew::uint128_t>::max (), [&success] (std::shared_ptr<btcnew::block> block_a) { ASSERT_NE (nullptr, block_a); success = true; });
	thread.join ();
	ASSERT_TRUE (success);
}
...
TEST (wallet, change)
{
	...
	ASSERT_NE (nullptr, system.wallet (0)->change_action (btcnew::test_genesis_key.pub, key2.pub));
	auto block2 (system.nodes[0]->rep_block (btcnew::test_genesis_key.pub));
...
TEST (wallet, partial_spend)
{
	...
	system.wallet (0)->insert_adhoc (btcnew::test_genesis_key.prv);
	btcnew::keypair key2;
	ASSERT_NE (nullptr, system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key2.pub, 500));
	ASSERT_EQ (std::numeric_limits<btcnew::uint128_t>::max () - 500, system.nodes[0]->balance (btcnew::test_genesis_key.pub));
}
...
{
	btcnew::account number0 (0);
	std::string str0;
	number0.encode_account (str0);

	ASSERT_EQ ((str0.front () == 'b') ? 67 : 67, str0.size ());
	ASSERT_EQ (67, str0.size ());
	btcnew::account number1;
...
{
	btcnew::uint256_union value0 (0);
	btcnew::fan fan (value0, 1024);
	for (auto & i : fan.values)
	{
		ASSERT_NE (value0, *i);
	}
	btcnew::raw_key value1;
	fan.value (value1);
...
TEST (fan, change)
{
	btcnew::raw_key value0;
	value0.data = 0;
	btcnew::raw_key value1;
	value1.data = 1;
	ASSERT_NE (value0, value1);
	btcnew::fan fan (value0.data, 1024);
	ASSERT_EQ (1024, fan.values.size ());
	btcnew::raw_key value2;
	fan.value (value2);
...
{
	auto error (false);
	btcnew::mdb_env env (error, btcnew::unique_path ());
	ASSERT_FALSE (error);
	auto transaction (env.tx_begin_write ());
	btcnew::kdf kdf;
	btcnew::wallet_store wallet (error, kdf, transaction, btcnew::genesis_account, 1, "0");
	ASSERT_FALSE (error);
	ASSERT_FALSE (wallet.is_representative (transaction));
	ASSERT_EQ (btcnew::genesis_account, wallet.representative (transaction));
	ASSERT_FALSE (wallet.is_representative (transaction));
	btcnew::keypair key;
...
TEST (wallet, serialize_json_empty)
{
	...
	auto transaction (env.tx_begin_write ());
	btcnew::kdf kdf;
	btcnew::wallet_store wallet1 (error, kdf, transaction, btcnew::genesis_account, 1, "0");
	ASSERT_FALSE (error);
	std::string serialized;
	wallet1.serialize_json (transaction, serialized);
	btcnew::wallet_store wallet2 (error, kdf, transaction, btcnew::genesis_account, 1, "1", serialized);
	ASSERT_FALSE (error);
	btcnew::raw_key password1;
	btcnew::raw_key password2;
...
TEST (wallet_store, move)
{
	...
	btcnew::wallet_store wallet2 (error, kdf, transaction, btcnew::genesis_account, 1, "1");
	ASSERT_FALSE (error);
	btcnew::keypair key2;
	...
	std::vector<btcnew::public_key> keys;
...
TEST (wallet, work)
{
	...
	while (!done)
	{
		...
		uint64_t work (0);
		if (!wallet->store.work_get (transaction, btcnew::test_genesis_key.pub, work))
		{
			done = !btcnew::work_validate (genesis.hash (), work);
		}
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (wallet, work_generate)
{
	btcnew::system system (24000, 1);
	auto wallet (system.wallet (0));
	btcnew::uint128_t amount1 (system.nodes[0]->balance (btcnew::test_genesis_key.pub));
	uint64_t work1;
	wallet->insert_adhoc (btcnew::test_genesis_key.prv);
	...
	btcnew::keypair key;
	wallet->send_action (btcnew::test_genesis_key.pub, key.pub, 100);
	system.deadline_set (10s);
	auto transaction (system.nodes[0]->store.tx_begin_read ());
	while (system.nodes[0]->ledger.account_balance (transaction, btcnew::test_genesis_key.pub) == amount1)
	{
		ASSERT_NO_ERROR (system.poll ());
	}
	system.deadline_set (10s);
	...
	while (again)
	{
		...
		auto transaction (system.wallet (0)->wallets.tx_begin_read ());
		again = wallet->store.work_get (transaction, account1, work1) || btcnew::work_validate (system.nodes[0]->ledger.latest_root (block_transaction, account1), work1);
	}
}
...
TEST (wallet, insert_locked)
{
	...
	ASSERT_TRUE (wallet->insert_adhoc (btcnew::keypair ().prv).is_zero ());
}
...
TEST (wallet, version_1_upgrade)
{
	...
	btcnew::wallet_value value (wallet->store.entry_get_raw (transaction, btcnew::wallet_store::wallet_key_special));
	btcnew::raw_key kdf;
	kdf.data.clear ();
	...
	wallet->store.entry_put_raw (transaction, key.pub, btcnew::wallet_value (ciphertext, 0));
	wallet->store.version_put (transaction, 1);
	wallet->enter_password (transaction, "1");
	ASSERT_TRUE (wallet->store.valid_password (transaction));
	ASSERT_EQ (btcnew::wallet_store::version_current, wallet->store.version (transaction));
	btcnew::raw_key prv;
	ASSERT_FALSE (wallet->store.fetch (transaction, key.pub, prv));
	ASSERT_EQ (key.prv, prv);
	value = wallet->store.entry_get_raw (transaction, btcnew::wallet_store::wallet_key_special);
...
TEST (wallet, reseed)
{
	...
	btcnew::kdf kdf;
	btcnew::wallet_store wallet (init, kdf, transaction, btcnew::genesis_account, 1, "0");
	btcnew::raw_key seed1;
	seed1.data = 1;
	btcnew::raw_key seed2;
	seed2.data = 2;
	wallet.seed_set (transaction, seed1);
	btcnew::raw_key seed3;
...
TEST (wallet, version_2_upgrade)
{
	...
	wallet->store.erase (transaction, btcnew::wallet_store::deterministic_index_special);
	wallet->store.erase (transaction, btcnew::wallet_store::seed_special);
	wallet->store.version_put (transaction, 2);
	ASSERT_EQ (2, wallet->store.version (transaction));
	ASSERT_EQ (wallet->store.find (transaction, btcnew::wallet_store::deterministic_index_special), wallet->store.end ());
	ASSERT_EQ (wallet->store.find (transaction, btcnew::wallet_store::seed_special), wallet->store.end ());
	wallet->store.attempt_password (transaction, "1");
	ASSERT_EQ (btcnew::wallet_store::version_current, wallet->store.version (transaction));
	ASSERT_NE (wallet->store.find (transaction, btcnew::wallet_store::deterministic_index_special), wallet->store.end ());
	ASSERT_NE (wallet->store.find (transaction, btcnew::wallet_store::seed_special), wallet->store.end ());
...
TEST (wallet, version_3_upgrade)
{
	...
	btcnew::keypair key;
	btcnew::raw_key seed;
	btcnew::uint256_union seed_ciphertext;
	...
	wallet->store.entry_put_raw (transaction, btcnew::wallet_store::seed_special, btcnew::wallet_value (seed_ciphertext, 0));
	...
	ASSERT_FALSE (wallet->store.fetch (transaction, key.pub, prv));
	ASSERT_EQ (key.prv, prv);
	btcnew::raw_key seed_compare;
	wallet->store.seed (seed_compare, transaction);
	ASSERT_EQ (seed, seed_compare);
	ASSERT_NE (seed_ciphertext, wallet->store.entry_get_raw (transaction, btcnew::wallet_store::seed_special).key);
}
...
TEST (wallet, upgrade_backup)
{
	...
	{
		auto node1 (std::make_shared<btcnew::node> (system.io_ctx, 24001, dir, system.alarm, system.logging, system.work));
	...
	// Now do the upgrade and confirm that backup is saved
	{
		...
		auto node1 (std::make_shared<btcnew::node> (system.io_ctx, dir, system.alarm, node_config, system.work));
		ASSERT_FALSE (node1->init_error ());
		auto wallet (node1->wallets.open (wallet_id));
		ASSERT_NE (nullptr, wallet);
...
{
	btcnew::system system (24000, 1);
	system.wallet (0)->insert_adhoc (btcnew::test_genesis_key.prv, false);
	btcnew::keypair key2;
	auto block (system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key2.pub, std::numeric_limits<btcnew::uint128_t>::max (), false));
	ASSERT_NE (nullptr, block);
	ASSERT_NE (0, block->block_work ());
	ASSERT_FALSE (btcnew::work_validate (block->root (), block->block_work ()));
	auto transaction (system.wallet (0)->wallets.tx_begin_read ());
	uint64_t cached_work (0);
	system.wallet (0)->store.work_get (transaction, btcnew::test_genesis_key.pub, cached_work);
...
TEST (wallet, send_race)
{
	...
	for (auto i (1); i < 60; ++i)
	{
		ASSERT_NE (nullptr, system.wallet (0)->send_action (btcnew::test_genesis_key.pub, key2.pub, btcnew::Gbtcnew_ratio));
		ASSERT_EQ (btcnew::genesis_amount - btcnew::Gbtcnew_ratio * i, system.nodes[0]->balance (btcnew::test_genesis_key.pub));
	}
}
...
TEST (wallet, change_seed)
{
	...
	uint32_t index (4);
	auto prv = btcnew::deterministic_key (seed1, index);
	pub = btcnew::pub_key (prv);
	wallet->insert_adhoc (btcnew::test_genesis_key.prv, false);
	auto block (wallet->send_action (btcnew::test_genesis_key.pub, pub, 100));
	ASSERT_NE (nullptr, block);
...
TEST (wallet, work_watcher_update)
{
	...
	wallet.insert_adhoc (btcnew::test_genesis_key.prv);
	btcnew::keypair key;
	auto const block1 (wallet.send_action (btcnew::test_genesis_key.pub, key.pub, 100));
	uint64_t difficulty1 (0);
	btcnew::work_validate (*block1, &difficulty1);
	auto const block2 (wallet.send_action (btcnew::test_genesis_key.pub, key.pub, 200));
	uint64_t difficulty2 (0);
	btcnew::work_validate (*block2, &difficulty2);
	auto multiplier = btcnew::difficulty::to_multiplier (std::max (difficulty1, difficulty2), node.network_params.network.publish_threshold);
...
TEST (wallet, work_watcher_generation_disabled)
{
	...
	btcnew::genesis genesis;
	btcnew::keypair key;
	auto block (std::make_shared<btcnew::state_block> (btcnew::test_genesis_key.pub, genesis.hash (), btcnew::test_genesis_key.pub, btcnew::genesis_amount - btcnew::Mbtcnew_ratio, key.pub, btcnew::test_genesis_key.prv, btcnew::test_genesis_key.pub, *pool.generate (genesis.hash ())));
	uint64_t difficulty (0);
	ASSERT_FALSE (btcnew::work_validate (*block, &difficulty));
	node.wallets.watcher->add (block);
	ASSERT_FALSE (node.process_local (block).code != btcnew::process_result::progress);
	ASSERT_TRUE (node.wallets.watcher->is_watched (block->qualified_root ()));
	auto multiplier = btcnew::difficulty::to_multiplier (difficulty, node.network_params.network.publish_threshold);
...
TEST (wallet, work_watcher_removed)
{
	...
	auto const block (wallet.send_action (btcnew::test_genesis_key.pub, key.pub, 100));
...
TEST (wallet, work_watcher_cancel)
{
	...
	node_config.max_work_generate_difficulty = btcnew::difficulty::from_multiplier (node_config.max_work_generate_multiplier, btcnew::network_constants::publish_test_threshold);
	node_config.enable_voting = false;
	auto & node = *system.add_node (node_config);
	auto & wallet (*system.wallet (0));
	wallet.insert_adhoc (btcnew::test_genesis_key.prv, false);
	btcnew::keypair key;
	auto work1 (node.work_generate_blocking (btcnew::test_genesis_key.pub));
	auto const block1 (wallet.send_action (btcnew::test_genesis_key.pub, key.pub, 100, *work1, false));
	uint64_t difficulty1 (0);
	btcnew::work_validate (*block1, &difficulty1);
	{
		btcnew::unique_lock<std::mutex> lock (node.active.mutex);
	...
	{
		btcnew::unique_lock<std::mutex> lock (wallet.wallets.watcher->mutex);
