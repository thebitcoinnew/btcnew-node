...
{
	btcnew::daemon_config daemon_config (data_path);
	...
	toml.write (btcnew::get_node_toml_config_path (data_path));
	...
	// Write rpc config
	btcnew::tomlconfig toml_rpc;
	rpc_config.serialize_toml (toml_rpc);
	toml_rpc.write (btcnew::get_rpc_toml_config_path (data_path));
}
...
int main (int argc, char * const * argv)
{
	...
	description.add_options ()
		...
		("node_path", boost::program_options::value<std::string> (), "The path to the btcnew_node to test")
		("rpc_path", boost::program_options::value<std::string> (), "The path to the btcnew_rpc to test");
	// clang-format on
	...
	{
		auto node_filepath = running_executable_filepath.parent_path () / "btcnew_node";
	...
	btcnew::signal_handler_impl = [&ioc] () {
		ioc.stop ();
	};
	...
	wallet_add_rpc (ioc, primary_node_results, wallet, btcnew::test_genesis_key.prv.data.to_string ());
	...
	std::thread t ([send_count, &destination_accounts, &ioc, &primary_node_results, &wallet, &resolver, &node_count] () {
		...
		for (auto i = 0; i < send_count; ++i)
		{
			...
			std::make_shared<send_session> (ioc, send_calls_remaining, wallet, btcnew::genesis_account.to_account (), destination_account->as_string, primary_node_results)->run ();
		}
	...
	btcnew::thread_runner runner (ioc, simultaneous_process_calls);
	t.join ();
	runner.join ();
