...
btcnew::rpc_connection::rpc_connection (btcnew::rpc_config const & rpc_config, boost::asio::io_context & io_ctx, btcnew::logger_mt & logger, btcnew::rpc_handler_interface & rpc_handler_interface) :
socket (io_ctx),
...
void btcnew::rpc_connection::parse_connection ()
{
	read ();
}

void btcnew::rpc_connection::prepare_head (unsigned version, boost::beast::http::status status)
{
	res.version (version);
	res.result (status);
...
void btcnew::rpc_connection::write_result (std::string body, unsigned version, boost::beast::http::status status)
{
...
void btcnew::rpc_connection::read ()
{
	...
	boost::beast::http::async_read_header (socket, buffer, *header_parser, boost::asio::bind_executor (strand, [this_l, header_parser] (boost::system::error_code const & ec, size_t bytes_transferred) {
		if (!ec)
		{
			if (boost::iequals (header_parser->get ()[boost::beast::http::field::expect], "100-continue"))
			{
				...
				continue_response->set (boost::beast::http::field::server, "btcnew");
...
void btcnew::rpc_connection::parse_request (std::shared_ptr<boost::beast::http::request_parser<boost::beast::http::empty_body>> header_parser)
{
	auto this_l (shared_from_this ());
			...
			this_l->io_ctx.post ([this_l, body_parser] () {
				...
				switch (method)
				{
					case boost::beast::http::verb::post: {
						auto handler (std::make_shared<btcnew::rpc_handler> (this_l->rpc_config, req.body (), request_id, response_handler, this_l->rpc_handler_interface, this_l->logger));
...
void btcnew::rpc_connection::write_completion_handler (std::shared_ptr<btcnew::rpc_connection> rpc_connection)
{
	// Intentional no-op
}
