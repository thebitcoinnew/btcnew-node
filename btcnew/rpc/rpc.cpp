...
#ifdef BTCNEW_SECURE_RPC
#include <btcnew/rpc/rpc_secure.hpp>
#endif

btcnew::rpc::rpc (boost::asio::io_context & io_ctx_a, btcnew::rpc_config const & config_a, btcnew::rpc_handler_interface & rpc_handler_interface_a) :
config (config_a),
acceptor (io_ctx_a),
...
void btcnew::rpc::accept ()
{
	auto connection (std::make_shared<btcnew::rpc_connection> (config, io_ctx, logger, rpc_handler_interface));
...
void btcnew::rpc::stop ()
{
	stopped = true;
	acceptor.close ();
}

std::unique_ptr<btcnew::rpc> btcnew::get_rpc (boost::asio::io_context & io_ctx_a, btcnew::rpc_config const & config_a, btcnew::rpc_handler_interface & rpc_handler_interface_a)
{
