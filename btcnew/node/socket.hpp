...
class socket : public std::enable_shared_from_this<btcnew::socket>
{
...
public:
	 ...
	 */
	explicit socket (std::shared_ptr<btcnew::node> node, boost::optional<std::chrono::seconds> io_timeout = boost::none, concurrency = concurrency::single_writer);
	virtual ~socket ();
	...
	void async_write (btcnew::shared_const_buffer const &, std::function<void (boost::system::error_code const &, size_t)> = nullptr);
...
protected:
	...
	{
	public:
		btcnew::shared_const_buffer buffer;
		std::function<void (boost::system::error_code const &, size_t)> callback;
	};

	boost::asio::strand<boost::asio::io_context::executor_type> strand;
	boost::asio::ip::tcp::socket tcp_socket;
	std::weak_ptr<btcnew::node> node;
...
public:
	 ...
	 */
	explicit server_socket (std::shared_ptr<btcnew::node> node_a, boost::asio::ip::tcp::endpoint local_a, size_t max_connections_a, concurrency concurrency_a = concurrency::single_writer);
	...
	void on_connection (std::function<bool (std::shared_ptr<btcnew::socket> new_connection, boost::system::error_code const &)>);
...
private:
	std::vector<std::weak_ptr<btcnew::socket>> connections;
