...
namespace
{
void construct_json (btcnew::seq_con_info_component * component, boost::property_tree::ptree & parent);
using ipc_json_handler_no_arg_func_map = std::unordered_map<std::string, std::function<void (btcnew::json_handler *)>>;
...
bool block_confirmed (btcnew::node & node, btcnew::transaction & transaction, btcnew::block_hash const & hash, bool include_active, bool include_only_confirmed);
const char * epoch_as_string (btcnew::epoch);
}

btcnew::json_handler::json_handler (btcnew::node & node_a, btcnew::node_rpc_config const & node_rpc_config_a, std::string const & body_a, std::function<void (std::string const &)> const & response_a, std::function<void ()> stop_callback_a) :
body (body_a),
node (node_a),
response (response_a),
...
void btcnew::json_handler::process_request (bool unsafe_a)
{
	try
	{
		...
		else
		{
			...
			else if (action == "kbtcnew_from_raw" || action == "krai_from_raw")
			{
				mbtcnew_from_raw (btcnew::kbtcnew_ratio);
			}
			else if (action == "kbtcnew_to_raw" || action == "krai_to_raw")
			{
				mbtcnew_to_raw (btcnew::kbtcnew_ratio);
			}
			else if (action == "btcnew_from_raw" || action == "rai_from_raw")
			{
				mbtcnew_from_raw (btcnew::btcnew_ratio);
			}
			else if (action == "btcnew_to_raw" || action == "rai_to_raw")
			{
				mbtcnew_to_raw (btcnew::btcnew_ratio);
			}
			else if (action == "mbtcnew_from_raw" || action == "mrai_from_raw")
			{
				mbtcnew_from_raw ();
			}
			else if (action == "mbtcnew_to_raw" || action == "mrai_to_raw")
			{
				mbtcnew_to_raw ();
			}
...
void btcnew::json_handler::response_errors ()
{
...
std::shared_ptr<btcnew::wallet> btcnew::json_handler::wallet_impl ()
{
	if (!ec)
	{
		...
		if (!wallet.decode_hex (wallet_text))
		{
			...
			{
				ec = btcnew::error_common::wallet_not_found;
			}
		}
		else
		{
			ec = btcnew::error_common::bad_wallet_number;
		}
	}
	return nullptr;
}

bool btcnew::json_handler::wallet_locked_impl (btcnew::transaction const & transaction_a, std::shared_ptr<btcnew::wallet> wallet_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = btcnew::error_common::wallet_locked;
			result = true;
		}
	}
	return result;
...
bool btcnew::json_handler::wallet_account_impl (btcnew::transaction const & transaction_a, std::shared_ptr<btcnew::wallet> wallet_a, btcnew::account const & account_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = btcnew::error_common::account_not_found_wallet;
		}
	}
	return result;
}

btcnew::account btcnew::json_handler::account_impl (std::string account_text, std::error_code ec_a)
{
...
btcnew::account_info btcnew::json_handler::account_info_impl (btcnew::transaction const & transaction_a, btcnew::account const & account_a)
{
	...
	if (!ec)
	{
		...
		{
			ec = btcnew::error_common::account_not_found;
...
btcnew::amount btcnew::json_handler::amount_impl ()
{
	btcnew::amount result (0);
	if (!ec)
	{
		std::string amount_text (request.get<std::string> ("amount"));
		if (result.decode_dec (amount_text))
		{
			ec = btcnew::error_common::invalid_amount;
		}
	}
	return result;
}

std::shared_ptr<btcnew::block> btcnew::json_handler::block_impl (bool signature_work_required)
{
	std::shared_ptr<btcnew::block> result{ nullptr };
	if (!ec)
	{
		...
		{
			ec = btcnew::error_blocks::invalid_block;
		}
		if (!ec)
		{
			...
			result = btcnew::deserialize_block_json (block_l);
			if (result == nullptr)
			{
				ec = btcnew::error_blocks::invalid_block;
			}
...
std::shared_ptr<btcnew::block> btcnew::json_handler::block_json_impl (bool signature_work_required)
{
...
btcnew::block_hash btcnew::json_handler::hash_impl (std::string search_text)
{
	...
	if (!ec)
	{
		...
		{
			ec = btcnew::error_blocks::invalid_block_hash;
		}
	}
	return result;
}

btcnew::amount btcnew::json_handler::threshold_optional_impl ()
{
	btcnew::amount result (0);
	...
	if (!ec && threshold_text.is_initialized ())
	{
		...
		{
			ec = btcnew::error_common::bad_threshold;
		}
	}
	return result;
}

uint64_t btcnew::json_handler::work_optional_impl ()
{
	uint64_t result (0);
	boost::optional<std::string> work_text (request.get_optional<std::string> ("work"));
	if (!ec && work_text.is_initialized ())
	{
		if (btcnew::from_string_hex (work_text.get (), result))
		{
			ec = btcnew::error_common::bad_work_format;
		}
	}
	return result;
}

uint64_t btcnew::json_handler::difficulty_optional_impl ()
{
	...
	{
		if (btcnew::from_string_hex (difficulty_text.get (), difficulty))
		{
			ec = btcnew::error_rpc::bad_difficulty_format;
		}
	}
	return difficulty;
}

double btcnew::json_handler::multiplier_optional_impl (uint64_t & difficulty)
{
	double multiplier (1.);
	...
	if (!ec && multiplier_text.is_initialized ())
	{
		...
		{
			difficulty = btcnew::difficulty::from_multiplier (multiplier, node.network_params.network.publish_threshold);
		}
		else
		{
			ec = btcnew::error_rpc::bad_multiplier_format;
		}
	}
	return multiplier;
}
...
uint64_t btcnew::json_handler::count_impl ()
{
	uint64_t result (0);
	if (!ec)
	{
		...
		{
			ec = btcnew::error_common::invalid_count;
		}
	}
	return result;
}

uint64_t btcnew::json_handler::count_optional_impl (uint64_t result)
{
...
uint64_t btcnew::json_handler::offset_optional_impl (uint64_t result)
{
	...
	if (!ec && offset_text.is_initialized ())
	{
		...
		{
			ec = btcnew::error_rpc::invalid_offset;
		}
	}
	return result;
}

void btcnew::json_handler::account_balance ()
{
...
void btcnew::json_handler::account_block_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::account_create ()
{
	...
	node.worker.push_task ([rpc_l] () {
		...
		if (!rpc_l->ec)
		{
			...
			if (index_text.is_initialized ())
			{
				...
				{
					rpc_l->ec = btcnew::error_common::invalid_index;
				}
				else
				{
			...
			if (!rpc_l->ec)
			{
				...
				{
					rpc_l->ec = btcnew::error_common::wallet_locked;
				}
			}
		}
...
void btcnew::json_handler::account_get ()
{
	...
	{
		ec = btcnew::error_common::bad_public_key;
	}
	response_errors ();
}

void btcnew::json_handler::account_info ()
{
	auto account (account_impl ());
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			std::string balance;
			btcnew::uint128_union (info.balance).encode_dec (balance);
...
void btcnew::json_handler::account_key ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::account_list ()
{
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), j (wallet->store.end ()); i != j; ++i)
		{
			...
			entry.put ("", btcnew::account (i->first).to_account ());
...
void btcnew::json_handler::account_move ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			if (!source.decode_hex (source_text))
			{
				...
				if (existing != rpc_l->node.wallets.items.end ())
				{
					...
					std::vector<btcnew::public_key> accounts;
				...
				{
					rpc_l->ec = btcnew::error_rpc::source_not_found;
				}
			}
			else
			{
				rpc_l->ec = btcnew::error_rpc::bad_source;
			}
		}
		rpc_l->response_errors ();
	});
...
void btcnew::json_handler::account_remove ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
		auto wallet (rpc_l->wallet_impl ());
...
void btcnew::json_handler::account_representative ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::account_representative_set ()
{
	auto rpc_l (shared_from_this ());
	// clang-format off
	node.worker.push_task ([ rpc_l, work_generation_enabled = node.work_generation_enabled () ]() {
		...
		auto representative (rpc_l->account_impl (representative_text, btcnew::error_rpc::bad_representative_number));
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec && work)
			{
				...
				if (!rpc_l->ec)
				{
					...
					if (!rpc_l->ec)
					{
						if (btcnew::work_validate (info.head, work))
						{
							rpc_l->ec = btcnew::error_common::invalid_work;
						}
					}
				}
			}
			else if (!rpc_l->ec) // work == 0
			{
				...
				{
					rpc_l->ec = btcnew::error_common::disabled_work_generation;
				}
			}
			if (!rpc_l->ec)
			{
				...
				// clang-format off
				wallet->change_async(account, representative, [response_a, response_data](std::shared_ptr<btcnew::block> block) {
					if (block != nullptr)
					{
...
void btcnew::json_handler::account_weight ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::accounts_balances ()
{
...
void btcnew::json_handler::accounts_create ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
		auto wallet (rpc_l->wallet_impl ());
		...
		if (!rpc_l->ec)
		{
			...
			{
				btcnew::account new_key (wallet->deterministic_insert (generate_work));
				if (!new_key.is_zero ())
				{
					boost::property_tree::ptree entry;
...
void btcnew::json_handler::accounts_frontiers ()
{
...
void btcnew::json_handler::accounts_pending ()
{
	...
	for (auto & accounts : request.get_child ("accounts"))
	{
		...
		if (!ec)
		{
			...
			for (auto i (node.store.pending_begin (transaction, btcnew::pending_key (account, 0))); btcnew::pending_key (i->first).account == account && peers_l.size () < count; ++i)
			{
				btcnew::pending_key const & key (i->first);
			...
			if (sorting && !simple)
			{
				if (source)
				{
					peers_l.sort ([] (const auto & child1, const auto & child2) -> bool {
						return child1.second.template get<btcnew::uint128_t> ("amount") > child2.second.template get<btcnew::uint128_t> ("amount");
					});
				}
				else
				{
					peers_l.sort ([] (const auto & child1, const auto & child2) -> bool {
						return child1.second.template get<btcnew::uint128_t> ("") > child2.second.template get<btcnew::uint128_t> ("");
					});
				}
			}
...
void btcnew::json_handler::active_difficulty ()
{
	auto include_trend (request.get<bool> ("include_trend", false));
	response_l.put ("network_minimum", btcnew::to_string_hex (node.network_params.network.publish_threshold));
	auto difficulty_active = node.active.active_difficulty ();
	response_l.put ("network_current", btcnew::to_string_hex (difficulty_active));
	auto multiplier = btcnew::difficulty::to_multiplier (difficulty_active, node.network_params.network.publish_threshold);
	response_l.put ("multiplier", btcnew::to_string (multiplier));
	if (include_trend)
	{
		...
		{
			boost::property_tree::ptree entry;
			entry.put ("", btcnew::to_string (multiplier_l));
...
void btcnew::json_handler::available_supply ()
{
	auto genesis_balance (node.balance (node.network_params.ledger.genesis_account)); // Cold storage genesis
	auto landing_balance (node.balance (btcnew::account ("059F68AAB29DE0D3A27443625C7EA9CDDB6517A8B76FE37727EF6A4D76832AD5"))); // Active unavailable account
	auto faucet_balance (node.balance (btcnew::account ("8E319CE6F3025E5B2DF66DA7AB1467FE48F1679C13DD43BFDB29FA2E9FC40D3B"))); // Faucet account
	auto burned_balance ((node.balance_pending (btcnew::account (0))).second); // Burning 0 account
...
void state_subtype (btcnew::transaction const & transaction_a, btcnew::node & node_a, std::shared_ptr<btcnew::block> block_a, btcnew::uint128_t const & balance_a, boost::property_tree::ptree & tree_a)
{
	// Subtype check
...
void btcnew::json_handler::block_info ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			btcnew::account account (block->account ().is_zero () ? sideband.account : block->account ());
			...
			if (block->type () == btcnew::block_type::state)
			{
		...
		{
			ec = btcnew::error_blocks::not_found;
		}
	}
	response_errors ();
}

void btcnew::json_handler::block_confirm ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block_l != nullptr)
		{
			...
			else
			{
				...
				btcnew::election_status status{ block_l, 0, std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::system_clock::now ().time_since_epoch ()), std::chrono::duration_values<std::chrono::milliseconds>::zero (), 0, btcnew::election_status_type::active_confirmation_height };
				{
					btcnew::lock_guard<std::mutex> lock (node.active.mutex);
				...
				if (auto state = dynamic_cast<btcnew::state_block *> (block_l.get ()))
				{
...
void btcnew::json_handler::blocks ()
{
	...
	for (boost::property_tree::ptree::value_type & hashes : request.get_child ("hashes"))
	{
		if (!ec)
		{
			...
			{
				ec = btcnew::error_blocks::bad_hash_number;
			}
		}
	}
...
void btcnew::json_handler::blocks_info ()
{
	const bool pending = request.get<bool> ("pending", false);
	const bool source = request.get<bool> ("source", false);
	const bool json_block_l = request.get<bool> ("json_block", false);
			...
			if (!hash.decode_hex (hash_text))
			{
				...
				if (block != nullptr)
				{
					...
					if (pending)
					{
						...
						{
							exists = node.store.pending_exists (transaction, btcnew::pending_key (destination, hash));
						}
						entry.put ("pending", exists ? "1" : "0");
					}
					if (source)
					{
						btcnew::block_hash source_hash (node.ledger.block_source (transaction, *block));
...
void btcnew::json_handler::block_account ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::block_count ()
{
	auto transaction (node.store.tx_begin_read ());
...
void btcnew::json_handler::block_count_type ()
{
	auto transaction (node.store.tx_begin_read ());
	btcnew::block_counts count (node.store.block_count (transaction));
...
void btcnew::json_handler::block_create ()
{
	std::string type (request.get<std::string> ("type"));
	btcnew::wallet_id wallet (0);
	...
	{
		representative = account_impl (representative_text.get (), btcnew::error_rpc::bad_representative_number);
	}
	...
	{
		destination = account_impl (destination_text.get (), btcnew::error_rpc::bad_destination);
	}
	btcnew::block_hash source (0);
	...
	if (!ec && source_text.is_initialized ())
	{
		...
		{
			ec = btcnew::error_rpc::bad_source;
		}
	}
	...
	{
		ec = btcnew::error_common::disabled_work_generation;
	}
	...
	if (!ec && key_text.is_initialized ())
	{
		...
		{
			ec = btcnew::error_common::bad_private_key;
		}
	}
	...
	if (!ec && previous_text.is_initialized ())
	{
		...
		{
			ec = btcnew::error_rpc::bad_previous;
		}
	}
	...
	if (!ec && balance_text.is_initialized ())
	{
		...
		{
			ec = btcnew::error_rpc::invalid_balance;
		}
	}
	btcnew::link link (0);
	...
	if (!ec)
	{
		...
		auto block_response_put_l = [rpc_l, this] (btcnew::block const & block_a) {
		...
		auto get_callback_l = [rpc_l, this, block_response_put_l] (std::shared_ptr<btcnew::block> block_a) {
			...
			return [block_a, rpc_l, this, block_response_put_l] (boost::optional<uint64_t> const & work_a) {
				if (block_a != nullptr)
				{
					...
					{
						rpc_l->ec = btcnew::error_common::failure_work_generation;
					}
				}
				else
				{
					rpc_l->ec = btcnew::error_common::generic;
				}
				if (rpc_l->ec)
				{
					rpc_l->response_errors ();
		...
		if (prv.data != 0)
		{
			btcnew::account pub (btcnew::pub_key (prv.as_private_key ()));
			...
			else if (previous_text.is_initialized () && balance_text.is_initialized () && type == "send")
			{
				...
				{
					ec = btcnew::error_rpc::block_create_balance_mismatch;
				}
			}
			...
			if (!ec && account_text.is_initialized ())
			{
				if (account != pub)
				{
					ec = btcnew::error_rpc::block_create_public_key_mismatch;
				}
			}
			btcnew::block_builder builder_l;
			std::shared_ptr<btcnew::block> block_l{ nullptr };
			btcnew::root root_l;
			...
			if (type == "state")
			{
				...
				{
					ec = btcnew::error_rpc::block_create_requirements_state;
				}
			}
			else if (type == "open")
			{
				...
				{
					ec = btcnew::error_rpc::block_create_requirements_open;
				}
			}
			else if (type == "receive")
			{
				...
				{
					ec = btcnew::error_rpc::block_create_requirements_receive;
				}
			}
			else if (type == "change")
			{
				...
				{
					ec = btcnew::error_rpc::block_create_requirements_change;
				}
			}
			else if (type == "send")
			{
				if (destination != 0 && previous != 0 && balance != 0 && amount != 0)
				{
					...
					{
						ec = btcnew::error_common::insufficient_balance;
					}
				}
				else
				{
					ec = btcnew::error_rpc::block_create_requirements_send;
				}
			}
			else
			{
				ec = btcnew::error_blocks::invalid_type;
			}
			if (!ec && (!ec_build || ec_build == btcnew::error_common::missing_work))
			{
				if (work == 0)
				{
					node.work_generate (root_l, get_callback_l (block_l), btcnew::account (pub));
				}
				else
				{
		...
		{
			ec = btcnew::error_rpc::block_create_key_required;
		}
	}
	// Because of callback
	if (ec)
...
void btcnew::json_handler::block_hash ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
...
void btcnew::json_handler::bootstrap ()
{
	...
	{
		uint16_t port;
		if (!btcnew::parse_port (port_text, port))
		{
			if (!node.flags.disable_legacy_bootstrap)
			{
				node.bootstrap_initiator.bootstrap (btcnew::endpoint (address, port), true, bypass_frontier_confirmation);
				response_l.put ("success", "");
			}
			else
			{
				ec = btcnew::error_rpc::disabled_bootstrap_legacy;
			}
		}
		else
		{
			ec = btcnew::error_common::invalid_port;
		}
	}
	else
	{
		ec = btcnew::error_common::invalid_ip_address;
	}
	response_errors ();
}

void btcnew::json_handler::bootstrap_any ()
{
...
void btcnew::json_handler::bootstrap_lazy ()
{
	auto hash (hash_impl ());
	const bool force = request.get<bool> ("force", false);
	if (!ec)
	{
		...
		{
			ec = btcnew::error_rpc::disabled_bootstrap_lazy;
		}
	}
	response_errors ();
}
 ...
 */
void btcnew::json_handler::bootstrap_status ()
{
	auto attempt (node.bootstrap_initiator.current_attempt ());
	if (attempt != nullptr)
	{
		btcnew::lock_guard<std::mutex> lock (attempt->mutex);
		btcnew::lock_guard<std::mutex> lazy_lock (attempt->lazy_mutex);
		...
		if (attempt->mode == btcnew::bootstrap_mode::legacy)
		{
			mode_text = "legacy";
		}
		else if (attempt->mode == btcnew::bootstrap_mode::lazy)
		{
			mode_text = "lazy";
		}
		else if (attempt->mode == btcnew::bootstrap_mode::wallet_lazy)
		{
...
void btcnew::json_handler::chain (bool successors)
{
...
void btcnew::json_handler::confirmation_active ()
{
...
void btcnew::json_handler::confirmation_height_currently_processing ()
{
	...
	{
		ec = btcnew::error_rpc::confirmation_height_not_processing;
	}
	response_errors ();
}

void btcnew::json_handler::confirmation_history ()
{
	boost::property_tree::ptree elections;
...
void btcnew::json_handler::confirmation_info ()
{
	...
	if (!root.decode_hex (root_text))
	{
		...
		if (conflict_info != node.active.roots.end ())
		{
			...
			for (auto i (tally_l.begin ()), n (tally_l.end ()); i != n; ++i)
			{
				...
				if (representatives)
				{
					std::multimap<btcnew::uint128_t, btcnew::account, std::greater<btcnew::uint128_t>> representatives;
					for (auto ii (election->last_votes.begin ()), nn (election->last_votes.end ()); ii != nn; ++ii)
					{
						if (i->second->hash () == ii->second.hash)
						{
							btcnew::account const & representative (ii->first);
		...
		{
			ec = btcnew::error_rpc::confirmation_not_found;
		}
	}
	else
	{
		ec = btcnew::error_rpc::invalid_root;
	}
	response_errors ();
}

void btcnew::json_handler::confirmation_quorum ()
{
...
void btcnew::json_handler::database_txn_tracker ()
{
	...
	{
		ec = btcnew::error_common::tracking_not_enabled;
	}

	response_errors ();
}

void btcnew::json_handler::delegators ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::delegators_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::deterministic_key ()
{
	...
	if (!seed.data.decode_hex (seed_text))
	{
		try
		{
			...
			btcnew::private_key prv = btcnew::deterministic_key (seed, index);
			btcnew::public_key pub (btcnew::pub_key (prv));
		...
		{
			ec = btcnew::error_common::invalid_index;
		}
	}
	else
	{
		ec = btcnew::error_common::bad_seed;
	}
	response_errors ();
}

void epoch_upgrader (std::shared_ptr<btcnew::node> node_a, btcnew::private_key const & prv_a, btcnew::epoch epoch_a, uint64_t count_limit)
{
	...
	raw_key.data = prv_a;
	auto signer (btcnew::pub_key (prv_a));
	...
	boost::multi_index::hashed_unique<boost::multi_index::tag<account_tag>, boost::multi_index::member<account_upgrade_item, btcnew::account, &account_upgrade_item::account>>>>
	accounts_list;
	...
	while (!finished_upgrade && !node_a->stopped)
	{
		...
		{
			{
				...
				for (auto i (node_a->store.latest_begin (transaction)), n (node_a->store.latest_end ()); i != n; ++i)
				{
					...
					{
						release_assert (btcnew::epochs::is_sequential (info.epoch (), epoch_a));
			...
			for (auto i (accounts_list.get<modified_tag> ().begin ()), n (accounts_list.get<modified_tag> ().end ()); i != n && upgraded_accounts < upgrade_batch_size && upgraded_accounts < count_limit && !node_a->stopped; ++i)
			{
				...
				if (!node_a->store.account_get (transaction, i->account, info) && info.epoch () < epoch_a)
				{
					auto epoch = builder.state ()
					             ...
					             .build ();
					bool valid_signature (!btcnew::validate_message (signer, epoch->hash (), epoch->block_signature ()));
					bool valid_work (!btcnew::work_validate (*epoch.get ()));
					btcnew::process_result result (btcnew::process_result::old);
					if (valid_signature && valid_work)
					{
						result = node_a->process_local (std::move (epoch)).code;
					}
					if (result == btcnew::process_result::progress)
					{
						++upgraded_accounts;
					}
					else
					{
						bool fork (result == btcnew::process_result::fork);
		...
		while (!finished_pending && count_limit != 0 && !node_a->stopped)
		{
			...
			auto transaction (node_a->store.tx_begin_read ());
			for (auto i (node_a->store.pending_begin (transaction, btcnew::pending_key (1, 0))), n (node_a->store.pending_end ()); i != n && upgraded_pending < upgrade_batch_size && upgraded_pending < count_limit && !node_a->stopped;)
			{
				...
				if (to_next_account)
				{
					...
					if (key.account.number () == std::numeric_limits<btcnew::uint256_t>::max ())
					{
						break;
					}
					else
					{
						i = node_a->store.pending_begin (transaction, btcnew::pending_key (key.account.number () + 1, 0));
					}
				}
				else
				{
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void btcnew::json_handler::epoch_upgrade ()
{
	btcnew::epoch epoch (btcnew::epoch::invalid);
	uint8_t epoch_int (request.get<uint8_t> ("epoch"));
	switch (epoch_int)
	{
		case 1:
			epoch = btcnew::epoch::epoch_1;
			break;
		case 2:
			epoch = btcnew::epoch::epoch_2;
			break;
		default:
			break;
	}
	if (epoch != btcnew::epoch::invalid)
	{
		uint64_t count_limit (count_optional_impl ());
		std::string key_text (request.get<std::string> ("key"));
		btcnew::private_key prv;
		if (!prv.decode_hex (key_text))
		{
			if (btcnew::pub_key (prv) == node.ledger.epoch_signer (node.ledger.epoch_link (epoch)))
			{
			...
			{
				ec = btcnew::error_rpc::invalid_epoch_signer;
			}
		}
		else
		{
	...
	{
		ec = btcnew::error_rpc::invalid_epoch;
	}
	response_errors ();
}

void btcnew::json_handler::frontiers ()
{
...
void btcnew::json_handler::account_count ()
{
	auto transaction (node.store.tx_begin_read ());
...
namespace
{
class history_visitor : public btcnew::block_visitor
{
public:
	history_visitor (btcnew::json_handler & handler_a, bool raw_a, btcnew::transaction & transaction_a, boost::property_tree::ptree & tree_a, btcnew::block_hash const & hash_a, std::vector<btcnew::public_key> const & accounts_filter_a) :
	handler (handler_a),
	raw (raw_a),
	...
	void send_block (btcnew::send_block const & block_a)
	{
	...
	void receive_block (btcnew::receive_block const & block_a)
	{
	...
	void open_block (btcnew::open_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "open");
	...
	void change_block (btcnew::change_block const & block_a)
	{
	...
	void state_block (btcnew::state_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "state");
	...
	bool should_ignore_account (btcnew::public_key const & account)
	{
		bool ignore (false);
	...
	btcnew::json_handler & handler;
	bool raw;
	btcnew::transaction & transaction;
	boost::property_tree::ptree & tree;
	btcnew::block_hash const & hash;
	btcnew::network_params network_params;
	std::vector<btcnew::public_key> const & accounts_filter;
};
}

void btcnew::json_handler::account_history ()
{
	std::vector<btcnew::public_key> accounts_to_filter;
		...
		while (block != nullptr && count > 0)
		{
			...
			else
			{
				...
				if (!entry.empty ())
				{
					...
					if (output_raw)
					{
						entry.put ("work", btcnew::to_string_hex (block->block_work ()));
...
void btcnew::json_handler::keepalive ()
{
	if (!ec)
	{
		std::string address_text (request.get<std::string> ("address"));
...
void btcnew::json_handler::key_create ()
{
	btcnew::keypair pair;
...
void btcnew::json_handler::key_expand ()
{
	std::string key_text (request.get<std::string> ("key"));
	btcnew::private_key prv;
	if (!prv.decode_hex (key_text))
...
void btcnew::json_handler::ledger ()
{
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	if (!ec)
	{
		btcnew::account start (0);
		boost::optional<std::string> account_text (request.get_optional<std::string> ("account"));
		...
		if (modified_since_text.is_initialized ())
		{
			...
			{
				ec = btcnew::error_rpc::invalid_timestamp;
			}
		}
		const bool sorting = request.get<bool> ("sorting", false);
		const bool representative = request.get<bool> ("representative", false);
		...
		{
			std::vector<std::pair<btcnew::uint128_union, btcnew::account>> ledger_l;
			for (auto i (node.store.latest_begin (transaction, start)), n (node.store.latest_end ()); i != n; ++i)
			{
				btcnew::account_info const & info (i->second);
				btcnew::uint128_union balance (info.balance);
			...
			for (auto i (ledger_l.begin ()), n (ledger_l.end ()); i != n && accounts.size () < count; ++i)
			{
				...
				{
					btcnew::account const & account (i->second);
					boost::property_tree::ptree response_a;
					if (pending)
					{
						auto account_pending (node.ledger.account_pending (transaction, account));
...
void btcnew::json_handler::mbtcnew_from_raw (btcnew::uint128_t ratio)
{
...
void btcnew::json_handler::mbtcnew_to_raw (btcnew::uint128_t ratio)
{
	auto amount (amount_impl ());
	if (!ec)
	{
		...
		{
			ec = btcnew::error_common::invalid_amount_big;
		}
	}
	response_errors ();
}
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void btcnew::json_handler::node_id ()
{
	if (!ec)
	{
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void btcnew::json_handler::node_id_delete ()
{
	response_l.put ("deprecated", "1");
	response_errors ();
}

void btcnew::json_handler::password_change ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
		auto wallet (rpc_l->wallet_impl ());
...
void btcnew::json_handler::password_enter ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
		auto wallet (rpc_l->wallet_impl ());
...
void btcnew::json_handler::password_valid (bool wallet_locked)
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::peers ()
{
	boost::property_tree::ptree peers_l;
	...
	for (auto i (peers_list.begin ()), n (peers_list.end ()); i != n; ++i)
	{
		...
		if (peer_details)
		{
			...
			pending_tree.put ("type", channel->get_type () == btcnew::transport::transport_type::tcp ? "tcp" : "udp");
...
void btcnew::json_handler::pending ()
{
	auto account (account_impl ());
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
...
void btcnew::json_handler::pending_exists ()
{
	auto hash (hash_impl ());
	const bool include_active = request.get<bool> ("include_active", false);
	const bool include_only_confirmed = request.get<bool> ("include_only_confirmed", false);
...
void btcnew::json_handler::payment_begin ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
		...
		if (!id.decode_hex (id_text))
		{
			...
			{
				auto transaction (rpc_l->node.wallets.tx_begin_write ());
				std::shared_ptr<btcnew::wallet> wallet (existing->second);
				if (wallet->store.valid_password (transaction))
				{
					...
					{
						rpc_l->ec = btcnew::error_rpc::payment_unable_create_account;
					}
				}
				else
				{
			...
			{
				rpc_l->ec = btcnew::error_common::wallet_not_found;
			}
		}
		else
		{
			rpc_l->ec = btcnew::error_common::bad_wallet_number;
		}
		rpc_l->response_errors ();
	});
}

void btcnew::json_handler::payment_init ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
		auto wallet (rpc_l->wallet_impl ());
...
void btcnew::json_handler::payment_end ()
{
	auto account (account_impl ());
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			{
				ec = btcnew::error_rpc::payment_account_balance;
			}
		}
	}
	response_errors ();
...
void btcnew::json_handler::payment_wait ()
{
	...
	if (!ec)
	{
		...
		{
			{
				auto observer (std::make_shared<btcnew::json_payment_observer> (node, response, account, amount));
		...
		{
			ec = btcnew::error_rpc::bad_timeout;
		}
	}
	if (ec)
	{
...
void btcnew::json_handler::process ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
		...
		if (!rpc_l->ec && block->type () == btcnew::block_type::state)
		{
			std::string subtype_text (rpc_l->request.get<std::string> ("subtype", ""));
			if (!subtype_text.empty ())
			{
				std::shared_ptr<btcnew::state_block> block_state (std::static_pointer_cast<btcnew::state_block> (block));
				...
				{
					rpc_l->ec = btcnew::error_process::gap_previous;
				}
				else
				{
					...
					if (subtype_text == "send")
					{
						...
						{
							rpc_l->ec = btcnew::error_rpc::invalid_subtype_balance;
						}
					...
					else if (subtype_text == "open")
					{
						...
						{
							rpc_l->ec = btcnew::error_rpc::invalid_subtype_previous;
						}
					}
					...
					else if (subtype_text == "epoch")
					{
						...
						{
							rpc_l->ec = btcnew::error_rpc::invalid_subtype_epoch_link;
						}
					}
					else
					{
						rpc_l->ec = btcnew::error_rpc::invalid_subtype;
					}
				}
			}
		}
		if (!rpc_l->ec)
		{
			if (!btcnew::work_validate (*block))
			{
				auto result (rpc_l->node.process_local (block, watch_work_l));
				switch (result.code)
				{
					case btcnew::process_result::progress: {
						rpc_l->response_l.put ("hash", block->hash ().to_string ());
						break;
					}
					case btcnew::process_result::gap_previous: {
						rpc_l->ec = btcnew::error_process::gap_previous;
						break;
					}
					case btcnew::process_result::gap_source: {
						rpc_l->ec = btcnew::error_process::gap_source;
						break;
					}
					case btcnew::process_result::old: {
						rpc_l->ec = btcnew::error_process::old;
						break;
					}
					case btcnew::process_result::bad_signature: {
						rpc_l->ec = btcnew::error_process::bad_signature;
						break;
					}
					case btcnew::process_result::negative_spend: {
						// TODO once we get RPC versioning, this should be changed to "negative spend"
						rpc_l->ec = btcnew::error_process::negative_spend;
						break;
					}
					case btcnew::process_result::balance_mismatch: {
						rpc_l->ec = btcnew::error_process::balance_mismatch;
						break;
					}
					case btcnew::process_result::unreceivable: {
						rpc_l->ec = btcnew::error_process::unreceivable;
						break;
					}
					case btcnew::process_result::block_position: {
						rpc_l->ec = btcnew::error_process::block_position;
						break;
					}
					case btcnew::process_result::fork: {
						...
						{
							rpc_l->ec = btcnew::error_process::fork;
						}
						break;
					}
					default: {
						rpc_l->ec = btcnew::error_process::other;
						break;
					}
				}
			}
			...
			{
				rpc_l->ec = btcnew::error_blocks::work_low;
			}
		}
		rpc_l->response_errors ();
	});
...
void btcnew::json_handler::receive ()
{
	auto wallet (wallet_impl ());
	auto account (account_impl ());
	auto hash (hash_impl ("block"));
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			if (block != nullptr)
			{
				if (node.store.pending_exists (block_transaction, btcnew::pending_key (account, hash)))
				{
					auto work (work_optional_impl ());
					if (!ec && work)
					{
						...
						if (btcnew::work_validate (head, work))
						{
							ec = btcnew::error_common::invalid_work;
						}
					}
					else if (!ec) // && work == 0
					{
					...
					if (!ec)
					{
						...
						// clang-format off
						wallet->receive_async(std::move(block), account, node.network_params.ledger.genesis_amount, [response_a](std::shared_ptr<btcnew::block> block_a) {
							if (block_a != nullptr)
							{
								boost::property_tree::ptree response_l;
				...
				{
					ec = btcnew::error_process::unreceivable;
				}
			}
			else
			{
...
void btcnew::json_handler::receive_minimum ()
{
	if (!ec)
	{
...
void btcnew::json_handler::receive_minimum_set ()
{
	auto amount (amount_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::representatives ()
{
	auto count (count_optional_impl ());
	if (!ec)
	{
		...
		if (!sorting) // Simple
		{
			std::map<btcnew::account, btcnew::uint128_t> ordered (rep_amounts.begin (), rep_amounts.end ());
		...
		{
			std::vector<std::pair<btcnew::uint128_t, std::string>> representation;
...
void btcnew::json_handler::representatives_online ()
{
...
void btcnew::json_handler::republish ()
{
	...
	if (!ec && sources_text.is_initialized ())
	{
		...
		{
			ec = btcnew::error_rpc::invalid_sources;
		}
	}
	...
	if (!ec && destinations_text.is_initialized ())
	{
		...
		{
			ec = btcnew::error_rpc::invalid_destinations;
		}
	}
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			std::deque<std::shared_ptr<btcnew::block>> republish_bundle;
			for (auto i (0); !hash.is_zero () && i < count; ++i)
			{
				block = node.store.block_get (transaction, hash);
				if (sources != 0) // Republish source chain
				{
					btcnew::block_hash source (node.ledger.block_source (transaction, *block));
				...
				if (destinations != 0) // Republish destination chain
				{
					...
					if (!destination.is_zero ())
					{
						if (!node.store.pending_exists (transaction, btcnew::pending_key (destination, hash)))
						{
							btcnew::block_hash previous (node.ledger.latest (transaction, destination));
...
void btcnew::json_handler::search_pending ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::search_pending_all ()
{
	if (!ec)
	{
...
void btcnew::json_handler::send ()
{
	...
	auto source (account_impl (source_text, btcnew::error_rpc::bad_source));
	std::string destination_text (request.get<std::string> ("destination"));
	auto destination (account_impl (destination_text, btcnew::error_rpc::bad_destination));
	if (!ec)
	{
		auto work (work_optional_impl ());
		...
		if (!ec)
		{
			...
			// clang-format off
			wallet->send_async(source, destination, amount.number(), [balance, amount, response_a, response_data](std::shared_ptr<btcnew::block> block_a) {
				if (block_a != nullptr)
				{
				...
				else
				{
					...
					{
						std::error_code ec(btcnew::error_common::insufficient_balance);
...
void btcnew::json_handler::sign ()
{
	...
	{
		ec = btcnew::error_rpc::sign_hash_disabled;
	}
	if (!ec)
	{
		btcnew::raw_key prv;
		...
		if (prv.data != 0)
		{
			...
			btcnew::signature signature (btcnew::sign_message (prv, pub, hash));
...
void btcnew::json_handler::stats ()
{
	...
	{
		ec = btcnew::error_rpc::invalid_missing_type;
	}
	if (!ec && use_sink)
	{
...
void btcnew::json_handler::stats_clear ()
{
	node.stats.clear ();
	response_l.put ("success", "");
	std::stringstream ostream;
...
void btcnew::json_handler::unchecked ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	auto count (count_optional_impl ());
	if (!ec)
...
void btcnew::json_handler::unchecked_clear ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
...
void btcnew::json_handler::unchecked_get ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	auto hash (hash_impl ());
	if (!ec)
...
void btcnew::json_handler::unchecked_keys ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	auto count (count_optional_impl ());
	btcnew::block_hash key (0);
	...
	if (!ec)
	{
		boost::property_tree::ptree unchecked;
		auto transaction (node.store.tx_begin_read ());
		for (auto i (node.store.unchecked_begin (transaction, btcnew::unchecked_key (key, 0))), n (node.store.unchecked_end ()); i != n && unchecked.size () < count; ++i)
		{
			boost::property_tree::ptree entry;
			btcnew::unchecked_info const & info (i->second);
...
void btcnew::json_handler::unopened ()
{
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	btcnew::account start (1); // exclude burn account by default
	boost::optional<std::string> account_text (request.get_optional<std::string> ("account"));
	if (account_text.is_initialized ())
	{
		start = account_impl (account_text.get ());
	...
	{
		auto transaction (node.store.tx_begin_read ());
		auto iterator (node.store.pending_begin (transaction, btcnew::pending_key (start, 0)));
		auto end (node.store.pending_end ());
		btcnew::account current_account (start);
		btcnew::uint128_t current_account_sum{ 0 };
		boost::property_tree::ptree accounts;
		while (iterator != end && accounts.size () < count)
		{
			btcnew::pending_key key (iterator->first);
			btcnew::account account (key.account);
			btcnew::pending_info info (iterator->second);
			if (node.store.account_exists (transaction, account))
			{
				if (account.number () == std::numeric_limits<btcnew::uint256_t>::max ())
				{
					break;
				}
				// Skip existing accounts
				iterator = node.store.pending_begin (transaction, btcnew::pending_key (account.number () + 1, 0));
			}
			else
			{
...
void btcnew::json_handler::wallet_add ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			{
				rpc_l->ec = btcnew::error_common::bad_private_key;
			}
		}
		rpc_l->response_errors ();
	});
...
void btcnew::json_handler::wallet_add_watch ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
		auto wallet (rpc_l->wallet_impl ());
			...
			if (wallet->store.valid_password (transaction))
			{
				for (auto & accounts : rpc_l->request.get_child ("accounts"))
				{
					...
					if (!rpc_l->ec)
					{
						...
						{
							rpc_l->ec = btcnew::error_common::bad_public_key;
						}
					}
				}
				if (!rpc_l->ec)
...
void btcnew::json_handler::wallet_info ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		btcnew::uint128_t balance (0);
		btcnew::uint128_t pending (0);
		uint64_t count (0);
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			btcnew::key_type key_type (wallet->store.key_type (i->second));
			if (key_type == btcnew::key_type::deterministic)
			{
				deterministic_count++;
			}
			else if (key_type == btcnew::key_type::adhoc)
			{
				adhoc_count++;
			}
			count++;
...
void btcnew::json_handler::wallet_balances ()
{
	auto wallet (wallet_impl ());
	auto threshold (threshold_optional_impl ());
	if (!ec)
	{
		...
		{
			btcnew::account const & account (i->first);
			btcnew::uint128_t balance = node.ledger.account_balance (block_transaction, account);
			if (balance >= threshold.number ())
			{
				boost::property_tree::ptree entry;
				btcnew::uint128_t pending = node.ledger.account_pending (block_transaction, account);
				entry.put ("balance", balance.convert_to<std::string> ());
...
void btcnew::json_handler::wallet_change_seed ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			if (!seed.data.decode_hex (seed_text))
			{
				...
				if (wallet->store.valid_password (transaction))
				{
					btcnew::public_key account (wallet->change_seed (transaction, seed, count));
					rpc_l->response_l.put ("success", "");
			...
			{
				rpc_l->ec = btcnew::error_common::bad_seed;
			}
		}
		rpc_l->response_errors ();
	});
...
void btcnew::json_handler::wallet_contains ()
{
	auto account (account_impl ());
	auto wallet (wallet_impl ());
	if (!ec)
...
void btcnew::json_handler::wallet_create ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
		btcnew::raw_key seed;
		...
		if (!rpc_l->ec)
		{
			...
			{
				rpc_l->ec = btcnew::error_common::wallet_lmdb_max_dbs;
			}
			if (!rpc_l->ec && seed_text.is_initialized ())
			{
				auto transaction (rpc_l->node.wallets.tx_begin_write ());
				btcnew::public_key account (wallet->change_seed (transaction, seed));
				rpc_l->response_l.put ("last_restored_account", account.to_account ());
				auto index (wallet->store.deterministic_index_get (transaction));
				assert (index > 0);
				rpc_l->response_l.put ("restored_count", std::to_string (index));
...
void btcnew::json_handler::wallet_destroy ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
...
void btcnew::json_handler::wallet_export ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::wallet_frontiers ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::wallet_history ()
{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			if (!node.store.account_get (block_transaction, account, info))
			{
				...
				while (timestamp >= modified_since && !hash.is_zero ())
				{
					...
					{
						boost::property_tree::ptree entry;
						std::vector<btcnew::public_key> no_filter;
...
void btcnew::json_handler::wallet_key_valid ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::wallet_ledger ()
{
	const bool representative = request.get<bool> ("representative", false);
	const bool weight = request.get<bool> ("weight", false);
	const bool pending = request.get<bool> ("pending", false);
...
void btcnew::json_handler::wallet_lock ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		btcnew::raw_key empty;
		empty.data.clear ();
...
void btcnew::json_handler::wallet_pending ()
{
	auto wallet (wallet_impl ());
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			btcnew::account const & account (i->first);
			boost::property_tree::ptree peers_l;
			for (auto ii (node.store.pending_begin (block_transaction, btcnew::pending_key (account, 0))); btcnew::pending_key (ii->first).account == account && peers_l.size () < count; ++ii)
			{
				btcnew::pending_key key (ii->first);
				if (block_confirmed (node, block_transaction, key.hash, include_active, include_only_confirmed))
				{
					...
					{
						btcnew::pending_info info (ii->second);
						if (info.amount.number () >= threshold.number ())
						{
							if (source || min_version)
							{
...
void btcnew::json_handler::wallet_representative ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::wallet_representative_set ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
		auto wallet (rpc_l->wallet_impl ());
		...
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec && update_existing_accounts)
			{
				...
				{
					// clang-format off
					wallet->change_async(account, representative, [](std::shared_ptr<btcnew::block>) {}, 0, false);
					// clang-format on
				}
			}
		}
...
void btcnew::json_handler::wallet_republish ()
{
	auto wallet (wallet_impl ());
	auto count (count_impl ());
	if (!ec)
...
void btcnew::json_handler::wallet_seed ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::wallet_work_get ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			(void)error_work;
			works.put (account.to_account (), btcnew::to_string_hex (work));
		}
...
void btcnew::json_handler::work_generate ()
{
	boost::optional<btcnew::account> account;
	...
	if (!ec)
	{
		...
		{
			ec = btcnew::error_rpc::difficulty_limit;
		}
		if (!ec)
		{
			...
			auto callback = [rpc_l, hash, this] (boost::optional<uint64_t> const & work_a) {
				if (work_a)
				{
					...
					response_l.put ("work", btcnew::to_string_hex (work));
					std::stringstream ostream;
					uint64_t result_difficulty;
					btcnew::work_validate (hash, work, &result_difficulty);
					response_l.put ("difficulty", btcnew::to_string_hex (result_difficulty));
					auto result_multiplier = btcnew::difficulty::to_multiplier (result_difficulty, this->node.network_params.network.publish_threshold);
					response_l.put ("multiplier", btcnew::to_string (result_multiplier));
					boost::property_tree::write_json (ostream, response_l);
					rpc_l->response (ostream.str ());
				}
				else
			...
			if (!use_peers)
			{
				...
				{
					ec = btcnew::error_common::disabled_local_work_generation;
				}
			}
			else
			{
...
void btcnew::json_handler::work_cancel ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void btcnew::json_handler::work_get ()
{
	auto wallet (wallet_impl ());
	auto account (account_impl ());
	if (!ec)
...
void btcnew::json_handler::work_set ()
{
	auto rpc_l (shared_from_this ());
	node.worker.push_task ([rpc_l] () {
		auto wallet (rpc_l->wallet_impl ());
...
void btcnew::json_handler::work_validate ()
{
	auto hash (hash_impl ());
	auto work (work_optional_impl ());
	auto difficulty (difficulty_optional_impl ());
	...
	if (!ec)
	{
		...
		response_l.put ("difficulty", btcnew::to_string_hex (result_difficulty));
		auto result_multiplier = btcnew::difficulty::to_multiplier (result_difficulty, node.network_params.network.publish_threshold);
		response_l.put ("multiplier", btcnew::to_string (result_multiplier));
	}
	response_errors ();
}

void btcnew::json_handler::work_peer_add ()
{
	std::string address_text = request.get<std::string> ("address");
	std::string port_text = request.get<std::string> ("port");
	uint16_t port;
...
void btcnew::json_handler::work_peers ()
{
...
void btcnew::json_handler::work_peers_clear ()
{
...
void construct_json (btcnew::seq_con_info_component * component, boost::property_tree::ptree & parent)
{
	...
	{
		auto & leaf_info = static_cast<btcnew::seq_con_info_leaf *> (component)->get_info ();
	...
	auto composite = static_cast<btcnew::seq_con_info_composite *> (component);
...
ipc_json_handler_no_arg_func_map create_ipc_json_handler_no_arg_func_map ()
{
	...
	no_arg_funcs.emplace ("account_balance", &btcnew::json_handler::account_balance);
	no_arg_funcs.emplace ("account_block_count", &btcnew::json_handler::account_block_count);
	no_arg_funcs.emplace ("account_count", &btcnew::json_handler::account_count);
	no_arg_funcs.emplace ("account_create", &btcnew::json_handler::account_create);
	no_arg_funcs.emplace ("account_get", &btcnew::json_handler::account_get);
	no_arg_funcs.emplace ("account_history", &btcnew::json_handler::account_history);
	no_arg_funcs.emplace ("account_info", &btcnew::json_handler::account_info);
	no_arg_funcs.emplace ("account_key", &btcnew::json_handler::account_key);
	no_arg_funcs.emplace ("account_list", &btcnew::json_handler::account_list);
	no_arg_funcs.emplace ("account_move", &btcnew::json_handler::account_move);
	no_arg_funcs.emplace ("account_remove", &btcnew::json_handler::account_remove);
	no_arg_funcs.emplace ("account_representative", &btcnew::json_handler::account_representative);
	no_arg_funcs.emplace ("account_representative_set", &btcnew::json_handler::account_representative_set);
	no_arg_funcs.emplace ("account_weight", &btcnew::json_handler::account_weight);
	no_arg_funcs.emplace ("accounts_balances", &btcnew::json_handler::accounts_balances);
	no_arg_funcs.emplace ("accounts_create", &btcnew::json_handler::accounts_create);
	no_arg_funcs.emplace ("accounts_frontiers", &btcnew::json_handler::accounts_frontiers);
	no_arg_funcs.emplace ("accounts_pending", &btcnew::json_handler::accounts_pending);
	no_arg_funcs.emplace ("active_difficulty", &btcnew::json_handler::active_difficulty);
	no_arg_funcs.emplace ("available_supply", &btcnew::json_handler::available_supply);
	no_arg_funcs.emplace ("block_info", &btcnew::json_handler::block_info);
	no_arg_funcs.emplace ("block", &btcnew::json_handler::block_info);
	no_arg_funcs.emplace ("block_confirm", &btcnew::json_handler::block_confirm);
	no_arg_funcs.emplace ("blocks", &btcnew::json_handler::blocks);
	no_arg_funcs.emplace ("blocks_info", &btcnew::json_handler::blocks_info);
	no_arg_funcs.emplace ("block_account", &btcnew::json_handler::block_account);
	no_arg_funcs.emplace ("block_count", &btcnew::json_handler::block_count);
	no_arg_funcs.emplace ("block_count_type", &btcnew::json_handler::block_count_type);
	no_arg_funcs.emplace ("block_create", &btcnew::json_handler::block_create);
	no_arg_funcs.emplace ("block_hash", &btcnew::json_handler::block_hash);
	no_arg_funcs.emplace ("bootstrap", &btcnew::json_handler::bootstrap);
	no_arg_funcs.emplace ("bootstrap_any", &btcnew::json_handler::bootstrap_any);
	no_arg_funcs.emplace ("bootstrap_lazy", &btcnew::json_handler::bootstrap_lazy);
	no_arg_funcs.emplace ("bootstrap_status", &btcnew::json_handler::bootstrap_status);
	no_arg_funcs.emplace ("confirmation_active", &btcnew::json_handler::confirmation_active);
	no_arg_funcs.emplace ("confirmation_height_currently_processing", &btcnew::json_handler::confirmation_height_currently_processing);
	no_arg_funcs.emplace ("confirmation_history", &btcnew::json_handler::confirmation_history);
	no_arg_funcs.emplace ("confirmation_info", &btcnew::json_handler::confirmation_info);
	no_arg_funcs.emplace ("confirmation_quorum", &btcnew::json_handler::confirmation_quorum);
	no_arg_funcs.emplace ("database_txn_tracker", &btcnew::json_handler::database_txn_tracker);
	no_arg_funcs.emplace ("delegators", &btcnew::json_handler::delegators);
	no_arg_funcs.emplace ("delegators_count", &btcnew::json_handler::delegators_count);
	no_arg_funcs.emplace ("deterministic_key", &btcnew::json_handler::deterministic_key);
	no_arg_funcs.emplace ("epoch_upgrade", &btcnew::json_handler::epoch_upgrade);
	no_arg_funcs.emplace ("frontiers", &btcnew::json_handler::frontiers);
	no_arg_funcs.emplace ("frontier_count", &btcnew::json_handler::account_count);
	no_arg_funcs.emplace ("keepalive", &btcnew::json_handler::keepalive);
	no_arg_funcs.emplace ("key_create", &btcnew::json_handler::key_create);
	no_arg_funcs.emplace ("key_expand", &btcnew::json_handler::key_expand);
	no_arg_funcs.emplace ("ledger", &btcnew::json_handler::ledger);
	no_arg_funcs.emplace ("node_id", &btcnew::json_handler::node_id);
	no_arg_funcs.emplace ("node_id_delete", &btcnew::json_handler::node_id_delete);
	no_arg_funcs.emplace ("password_change", &btcnew::json_handler::password_change);
	no_arg_funcs.emplace ("password_enter", &btcnew::json_handler::password_enter);
	no_arg_funcs.emplace ("wallet_unlock", &btcnew::json_handler::password_enter);
	no_arg_funcs.emplace ("payment_begin", &btcnew::json_handler::payment_begin);
	no_arg_funcs.emplace ("payment_init", &btcnew::json_handler::payment_init);
	no_arg_funcs.emplace ("payment_end", &btcnew::json_handler::payment_end);
	no_arg_funcs.emplace ("payment_wait", &btcnew::json_handler::payment_wait);
	no_arg_funcs.emplace ("peers", &btcnew::json_handler::peers);
	no_arg_funcs.emplace ("pending", &btcnew::json_handler::pending);
	no_arg_funcs.emplace ("pending_exists", &btcnew::json_handler::pending_exists);
	no_arg_funcs.emplace ("process", &btcnew::json_handler::process);
	no_arg_funcs.emplace ("receive", &btcnew::json_handler::receive);
	no_arg_funcs.emplace ("receive_minimum", &btcnew::json_handler::receive_minimum);
	no_arg_funcs.emplace ("receive_minimum_set", &btcnew::json_handler::receive_minimum_set);
	no_arg_funcs.emplace ("representatives", &btcnew::json_handler::representatives);
	no_arg_funcs.emplace ("representatives_online", &btcnew::json_handler::representatives_online);
	no_arg_funcs.emplace ("republish", &btcnew::json_handler::republish);
	no_arg_funcs.emplace ("search_pending", &btcnew::json_handler::search_pending);
	no_arg_funcs.emplace ("search_pending_all", &btcnew::json_handler::search_pending_all);
	no_arg_funcs.emplace ("send", &btcnew::json_handler::send);
	no_arg_funcs.emplace ("sign", &btcnew::json_handler::sign);
	no_arg_funcs.emplace ("stats", &btcnew::json_handler::stats);
	no_arg_funcs.emplace ("stats_clear", &btcnew::json_handler::stats_clear);
	no_arg_funcs.emplace ("stop", &btcnew::json_handler::stop);
	no_arg_funcs.emplace ("unchecked", &btcnew::json_handler::unchecked);
	no_arg_funcs.emplace ("unchecked_clear", &btcnew::json_handler::unchecked_clear);
	no_arg_funcs.emplace ("unchecked_get", &btcnew::json_handler::unchecked_get);
	no_arg_funcs.emplace ("unchecked_keys", &btcnew::json_handler::unchecked_keys);
	no_arg_funcs.emplace ("unopened", &btcnew::json_handler::unopened);
	no_arg_funcs.emplace ("uptime", &btcnew::json_handler::uptime);
	no_arg_funcs.emplace ("validate_account_number", &btcnew::json_handler::validate_account_number);
	no_arg_funcs.emplace ("version", &btcnew::json_handler::version);
	no_arg_funcs.emplace ("wallet_add", &btcnew::json_handler::wallet_add);
	no_arg_funcs.emplace ("wallet_add_watch", &btcnew::json_handler::wallet_add_watch);
	no_arg_funcs.emplace ("wallet_balances", &btcnew::json_handler::wallet_balances);
	no_arg_funcs.emplace ("wallet_change_seed", &btcnew::json_handler::wallet_change_seed);
	no_arg_funcs.emplace ("wallet_contains", &btcnew::json_handler::wallet_contains);
	no_arg_funcs.emplace ("wallet_create", &btcnew::json_handler::wallet_create);
	no_arg_funcs.emplace ("wallet_destroy", &btcnew::json_handler::wallet_destroy);
	no_arg_funcs.emplace ("wallet_export", &btcnew::json_handler::wallet_export);
	no_arg_funcs.emplace ("wallet_frontiers", &btcnew::json_handler::wallet_frontiers);
	no_arg_funcs.emplace ("wallet_history", &btcnew::json_handler::wallet_history);
	no_arg_funcs.emplace ("wallet_info", &btcnew::json_handler::wallet_info);
	no_arg_funcs.emplace ("wallet_balance_total", &btcnew::json_handler::wallet_info);
	no_arg_funcs.emplace ("wallet_key_valid", &btcnew::json_handler::wallet_key_valid);
	no_arg_funcs.emplace ("wallet_ledger", &btcnew::json_handler::wallet_ledger);
	no_arg_funcs.emplace ("wallet_lock", &btcnew::json_handler::wallet_lock);
	no_arg_funcs.emplace ("wallet_pending", &btcnew::json_handler::wallet_pending);
	no_arg_funcs.emplace ("wallet_representative", &btcnew::json_handler::wallet_representative);
	no_arg_funcs.emplace ("wallet_representative_set", &btcnew::json_handler::wallet_representative_set);
	no_arg_funcs.emplace ("wallet_republish", &btcnew::json_handler::wallet_republish);
	no_arg_funcs.emplace ("wallet_work_get", &btcnew::json_handler::wallet_work_get);
	no_arg_funcs.emplace ("work_generate", &btcnew::json_handler::work_generate);
	no_arg_funcs.emplace ("work_cancel", &btcnew::json_handler::work_cancel);
	no_arg_funcs.emplace ("work_get", &btcnew::json_handler::work_get);
	no_arg_funcs.emplace ("work_set", &btcnew::json_handler::work_set);
	no_arg_funcs.emplace ("work_validate", &btcnew::json_handler::work_validate);
	no_arg_funcs.emplace ("work_peer_add", &btcnew::json_handler::work_peer_add);
	no_arg_funcs.emplace ("work_peers", &btcnew::json_handler::work_peers);
	no_arg_funcs.emplace ("work_peers_clear", &btcnew::json_handler::work_peers_clear);
	return no_arg_funcs;
}
...
const char * epoch_as_string (btcnew::epoch epoch)
{
	switch (epoch)
	{
		case btcnew::epoch::epoch_2:
			return "2";
		case btcnew::epoch::epoch_1:
			return "1";
		default:
			return "0";
	}
