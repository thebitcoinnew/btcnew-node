...
__kernel void btcnew_work (__global ulong const * attempt, __global ulong * result_a, __global uchar const * item_a, __global ulong const * difficulty_a)
{
...
btcnew::opencl_environment::opencl_environment (bool & error_a)
{
	...
	{
		btcnew::opencl_platform platform;
		platform.platform = *i;
...
void btcnew::opencl_environment::dump (std::ostream & stream)
{
	auto index (0);
...
btcnew::opencl_work::opencl_work (bool & error_a, btcnew::opencl_config const & config_a, btcnew::opencl_environment & environment_a, btcnew::logger_mt & logger_a) :
config (config_a),
context (0),
attempt_buffer (0),
result_buffer (0),
...
logger (logger_a)
{
	...
	if (!error_a)
	{
		...
		if (!error_a)
		{
			btcnew::random_pool::generate_block (reinterpret_cast<uint8_t *> (rand.s.data ()), rand.s.size () * sizeof (decltype (rand.s)::value_type));
			...
			if (!error_a)
			{
				...
				if (!error_a)
				{
					...
					if (!error_a)
					{
						...
						{
							cl_int item_error (0);
							size_t item_size (sizeof (btcnew::uint256_union));
								...
								if (!error_a)
								{
									...
									if (!error_a)
									{
										...
										if (!error_a)
										{
											...
											kernel = clCreateKernel (program, "btcnew_work", &kernel_error);
...
btcnew::opencl_work::~opencl_work ()
{
	if (kernel != 0)
	{
...
boost::optional<uint64_t> btcnew::opencl_work::generate_work (btcnew::root const & root_a, uint64_t const difficulty_a)
{
	std::atomic<int> ticket_l{ 0 };
	return generate_work (root_a, difficulty_a, ticket_l);
}

boost::optional<uint64_t> btcnew::opencl_work::generate_work (btcnew::root const & root_a, uint64_t const difficulty_a, std::atomic<int> & ticket_a)
{
	...
	while ((btcnew::work_validate (root_a, result, &computed_difficulty) || computed_difficulty < difficulty_a) && !error && ticket_a == ticket_l)
	{
		result = rand.next ();
		...
		{
			cl_int write_error2 = clEnqueueWriteBuffer (queue, item_buffer, false, 0, sizeof (btcnew::root), root_a.bytes.data (), 0, nullptr, nullptr);
...
std::unique_ptr<btcnew::opencl_work> btcnew::opencl_work::create (bool create_a, btcnew::opencl_config const & config_a, btcnew::logger_mt & logger_a)
{
	std::unique_ptr<btcnew::opencl_work> result;
	if (create_a)
	{
		auto error (false);
		...
		if (!error)
		{
			result.reset (new btcnew::opencl_work (error, config_a, environment, logger_a));
			if (error)
			{
				result.reset ();
			}
