...
std::chrono::milliseconds constexpr btcnew::block_processor::confirmation_request_delay;

btcnew::block_processor::block_processor (btcnew::node & node_a, btcnew::write_database_queue & write_database_queue_a) :
generator (node_a),
stopped (false),
active (false),
...
void btcnew::block_processor::stop ()
{
	generator.stop ();
	{
...
void btcnew::block_processor::flush ()
{
	node.checker.flush ();
...
size_t btcnew::block_processor::size ()
{
	btcnew::unique_lock<std::mutex> lock (mutex);
	return (blocks.size () + state_blocks.size () + forced.size ());
}

bool btcnew::block_processor::full ()
{
	return size () > node.flags.block_processor_full_size;
}

bool btcnew::block_processor::half_full ()
{
	return size () > node.flags.block_processor_full_size / 2;
}

void btcnew::block_processor::add (std::shared_ptr<btcnew::block> block_a, uint64_t origination)
{
	btcnew::unchecked_info info (block_a, 0, origination, btcnew::signature_verification::unknown);
	add (info);
}

void btcnew::block_processor::add (btcnew::unchecked_info const & info_a)
{
	if (!btcnew::work_validate (info_a.block->root (), info_a.block->block_work ()))
	{
		{
			...
			{
				if (info_a.verified == btcnew::signature_verification::unknown && (info_a.block->type () == btcnew::block_type::state || info_a.block->type () == btcnew::block_type::open || !info_a.account.is_zero ()))
				{
	...
	{
		node.logger.try_log ("btcnew::block_processor::add called for hash ", info_a.block->hash ().to_string (), " with invalid work ", btcnew::to_string_hex (info_a.block->block_work ()));
		assert (false && "btcnew::block_processor::add called with invalid work");
	}
}

void btcnew::block_processor::force (std::shared_ptr<btcnew::block> block_a)
{
	{
		btcnew::lock_guard<std::mutex> lock (mutex);
...
void btcnew::block_processor::wait_write ()
{
	btcnew::lock_guard<std::mutex> lock (mutex);
	awaiting_write = true;
}

void btcnew::block_processor::process_blocks ()
{
	btcnew::unique_lock<std::mutex> lock (mutex);
	while (!stopped)
	{
...
bool btcnew::block_processor::should_log (bool first_time)
{
	auto result (false);
...
bool btcnew::block_processor::have_blocks ()
{
	assert (!mutex.try_lock ());
	return !blocks.empty () || !forced.empty () || !state_blocks.empty ();
}

void btcnew::block_processor::verify_state_blocks (btcnew::unique_lock<std::mutex> & lock_a, size_t max_count)
{
	assert (!mutex.try_lock ());
	btcnew::timer<std::chrono::milliseconds> timer_l (btcnew::timer_state::started);
	std::deque<btcnew::unchecked_info> items;
	...
	if (!items.empty ())
	{
		...
		std::vector<btcnew::signature> blocks_signatures;
		...
		for (auto i (0); i < size; ++i)
		{
			...
			btcnew::account account (item.block->account ());
		...
		for (auto i (0); i < size; ++i)
		{
			...
			if (!item.block->link ().is_zero () && node.ledger.is_epoch_link (item.block->link ()))
			{
				...
				{
					item.verified = btcnew::signature_verification::valid_epoch;
					blocks.push_back (std::move (item));
				}
				else
				{
					// Possible regular state blocks with epoch link (send subtype)
					item.verified = btcnew::signature_verification::unknown;
					blocks.push_back (std::move (item));
				}
			}
			...
			{
				// Non epoch blocks
				item.verified = btcnew::signature_verification::valid;
				blocks.push_back (std::move (item));
			}
			else
			{
...
void btcnew::block_processor::process_batch (btcnew::unique_lock<std::mutex> & lock_a)
{
	...
	lock_a.unlock ();
	auto scoped_write_guard = write_database_queue.wait (btcnew::writer::process_batch);
	auto transaction (node.store.tx_begin_write ({ btcnew::tables::accounts, btcnew::tables::cached_counts, btcnew::tables::change_blocks, btcnew::tables::frontiers, btcnew::tables::open_blocks, btcnew::tables::pending, btcnew::tables::receive_blocks, btcnew::tables::representation, btcnew::tables::send_blocks, btcnew::tables::state_blocks, btcnew::tables::unchecked }, { btcnew::tables::confirmation_height }));
	timer_l.restart ();
	lock_a.lock ();
	// Processing blocks
	...
	while ((!blocks.empty () || !forced.empty ()) && (timer_l.before_deadline (node.config.block_processor_batch_max_time) || (number_of_blocks_processed < node.flags.block_processor_batch_size)) && !awaiting_write)
	{
		...
		{
			info = btcnew::unchecked_info (forced.front (), 0, btcnew::seconds_since_epoch (), btcnew::signature_verification::unknown);
			forced.pop_front ();
			hash = info.block->hash ();
			force = true;
		...
		if (force)
		{
			...
			if (successor != nullptr && successor->hash () != hash)
			{
				...
				{
					node.logger.always_log (btcnew::severity_level::error, boost::str (boost::format ("Failed to roll back %1% because it or a successor was confirmed") % successor->hash ().to_string ()));
				}
				else
				{
				...
				auto inserted (rolled_back.insert (btcnew::rolled_hash{ std::chrono::steady_clock::now (), successor->hash () }));
				if (inserted.second)
				{
...
void btcnew::block_processor::process_live (btcnew::block_hash const & hash_a, std::shared_ptr<btcnew::block> block_a, const bool watch_work_a)
{
...
btcnew::process_return btcnew::block_processor::process_one (btcnew::write_transaction const & transaction_a, btcnew::unchecked_info info_a, const bool watch_work_a)
{
	...
	switch (result.code)
	{
		case btcnew::process_result::progress: {
			...
			if (info_a.modified > btcnew::seconds_since_epoch () - 300 && node.block_arrival.recent (hash))
			{
		...
		case btcnew::process_result::gap_previous: {
			...
			{
				info_a.modified = btcnew::seconds_since_epoch ();
			}
			node.store.unchecked_put (transaction_a, btcnew::unchecked_key (info_a.block->previous (), hash), info_a);
		...
		case btcnew::process_result::gap_source: {
			...
			node.store.unchecked_put (transaction_a, btcnew::unchecked_key (node.ledger.block_source (transaction_a, *(info_a.block)), hash), info_a);
			node.gap_cache.add (hash);
			break;
		}
		...
		case btcnew::process_result::fork: {
			...
			node.stats.inc (btcnew::stat::type::ledger, btcnew::stat::detail::fork, btcnew::stat::dir::in);
			if (node.config.logging.ledger_logging ())
			{
		...
		case btcnew::process_result::opened_burn_account: {
		...
		case btcnew::process_result::representative_mismatch: {
			if (node.config.logging.ledger_logging ())
			{
...
btcnew::process_return btcnew::block_processor::process_one (btcnew::write_transaction const & transaction_a, std::shared_ptr<btcnew::block> block_a, const bool watch_work_a)
{
	btcnew::unchecked_info info (block_a, block_a->account (), 0, btcnew::signature_verification::unknown);
	auto result (process_one (transaction_a, info, watch_work_a));
	return result;
}

void btcnew::block_processor::queue_unchecked (btcnew::write_transaction const & transaction_a, btcnew::block_hash const & hash_a)
{
	...
	for (auto & info : unchecked_blocks)
	{
		if (!node.flags.fast_bootstrap)
		{
			node.store.unchecked_del (transaction_a, btcnew::unchecked_key (hash_a, info.block->hash ()));
		}
		add (info);
	}
...
btcnew::block_hash btcnew::block_processor::filter_item (btcnew::block_hash const & hash_a, btcnew::signature const & signature_a)
{
...
void btcnew::block_processor::requeue_invalid (btcnew::block_hash const & hash_a, btcnew::unchecked_info const & info_a)
{
	assert (hash_a == info_a.block->hash ());
	auto attempt (node.bootstrap_initiator.current_attempt ());
	if (attempt != nullptr && attempt->mode == btcnew::bootstrap_mode::lazy)
	{
