...
namespace
{
void reset_confirmation_heights (btcnew::block_store & store);
bool is_using_rocksdb (boost::filesystem::path const & data_path, std::error_code & ec);
}

std::string btcnew::error_cli_messages::message (int ev) const
{
	switch (static_cast<btcnew::error_cli> (ev))
	{
		case btcnew::error_cli::generic:
			return "Unknown error";
		case btcnew::error_cli::parse_error:
			return "Coud not parse command line";
		case btcnew::error_cli::invalid_arguments:
			return "Invalid arguments";
		case btcnew::error_cli::unknown_command:
			return "Unknown command";
		case btcnew::error_cli::database_write_error:
			return "Database write error";
		case btcnew::error_cli::reading_config:
			return "Config file read error";
		case btcnew::error_cli::disable_all_network:
...
void btcnew::add_node_options (boost::program_options::options_description & description_a)
{
	// clang-format off
...
void btcnew::add_node_flag_options (boost::program_options::options_description & description_a)
{
	// clang-format off
	description_a.add_options()
...
std::error_code btcnew::update_flags (btcnew::node_flags & flags_a, boost::program_options::variables_map const & vm)
{
	std::error_code ec;
	...
	{
		ec = btcnew::error_cli::disable_all_network;
	}
...
void database_write_lock_error (std::error_code & ec)
{
	...
	ec = btcnew::error_cli::database_write_error;
}
...
std::error_code btcnew::handle_node_options (boost::program_options::variables_map & vm)
{
	std::error_code ec;
	boost::filesystem::path data_path = vm.count ("data_path") ? boost::filesystem::path (vm["data_path"].as<std::string> ()) : btcnew::working_path ();
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (wallet != nullptr)
				{
					...
					else
					{
						...
						ec = btcnew::error_cli::invalid_arguments;
					}
				}
				else
				{
		...
		try
		{
			...
			if (!ec)
			{
				...
				if (success)
				{
					...
					if (using_rocksdb)
					{
						btcnew::remove_all_files_in_dir (backup_path);
						btcnew::move_all_files_to_dir (source_path, backup_path);
						btcnew::move_all_files_to_dir (vacuum_path, source_path);
	...
	else if (vm.count ("generate_config"))
	{
		...
		{
			valid_type = true;
			btcnew::rpc_config config (false);
			config.serialize_toml (toml);
		}
		else
		{
		...
		if (valid_type)
		{
			std::cout << "# This is an example configuration file for Bitcoin New. Visit https://docs.btcnew.org/running-a-node/configuration/ for more information.\n#\n"
			          ...
			          << "# Additional information for notable configuration options is available in https://docs.btcnew.org/running-a-node/configuration/#notable-configuration-options\n";
	...
	else if (vm.count ("diagnostics"))
	{
		...
		btcnew::raw_key key;
		key.data.clear ();
		btcnew::send_block send (0, 0, 0, key, 0, 0);
		std::cout << "Testing key derivation function" << std::endl;
		btcnew::raw_key junk1;
		junk1.data.clear ();
		btcnew::uint256_union junk2 (0);
		btcnew::kdf kdf;
		kdf.phs (junk1, "", junk2);
	...
	else if (vm.count ("wallet_create"))
	{
		...
		{
			inactive_node node (data_path);
			auto wallet_key = btcnew::random_wallet_id ();
				...
				if (existing != node.node->wallets.items.end ())
				{
					...
					if (!existing->second->enter_password (transaction, password))
					{
						...
						for (auto i (existing->second->store.begin (transaction)), m (existing->second->store.end ()); i != m; ++i)
						{
							...
							if (btcnew::pub_key (key.as_private_key ()) != account)
							{
					...
					if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
					{
						...
						else
						{
							...
							else
							{
								bool error (true);
								{
									btcnew::lock_guard<std::mutex> lock (node.node->wallets.mutex);
									auto transaction (node.node->wallets.tx_begin_write ());
									btcnew::wallet wallet (error, transaction, node.node->wallets, wallet_id.to_string (), contents.str ());
								}
								if (error)
								{
									std::cerr << "Unable to import wallet\n";
	...
	else if (vm.count ("wallet_list"))
	{
		...
		for (auto i (node.node->wallets.items.begin ()), n (node.node->wallets.items.end ()); i != n; ++i)
		{
			...
			{
				std::cout << btcnew::account (j->first).to_account () << '\n';
			}
		}
	}
	...
	{
		ec = btcnew::error_cli::unknown_command;
	}
...
{
	btcnew::daemon_config config (data_path);
	auto error = btcnew::read_node_config_toml (data_path, config);
	if (!error)
	{
		...
		if (use_rocksdb)
		{
			...
			ec = btcnew::error_cli::database_write_error;
#endif
			return (BTCNEW_ROCKSDB == 1);
		}
	}
	else
	{
		ec = btcnew::error_cli::reading_config;
	}
