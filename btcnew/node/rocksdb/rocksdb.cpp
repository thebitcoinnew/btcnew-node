...
#include <btcnew/node/rocksdb/rocksdb_txn.hpp>
...
btcnew::rocksdb_store::rocksdb_store (btcnew::logger_mt & logger_a, boost::filesystem::path const & path_a, btcnew::rocksdb_config const & rocksdb_config_a, bool open_read_only_a) :
logger (logger_a),
rocksdb_config (rocksdb_config_a)
{
	boost::system::error_code error_mkdir, error_chmod;
	boost::filesystem::create_directories (path_a, error_mkdir);
	btcnew::set_secure_perm_directory (path_a, error_chmod);
...
btcnew::rocksdb_store::~rocksdb_store ()
{
...
void btcnew::rocksdb_store::open (bool & error_a, boost::filesystem::path const & path_a, bool open_read_only_a)
{
...
btcnew::write_transaction btcnew::rocksdb_store::tx_begin_write (std::vector<btcnew::tables> const & tables_requiring_locks_a, std::vector<btcnew::tables> const & tables_no_locks_a)
{
	std::unique_ptr<btcnew::write_rocksdb_txn> txn;
	...
	if (tables_requiring_locks_a.empty () && tables_no_locks_a.empty ())
	{
		...
		txn = std::make_unique<btcnew::write_rocksdb_txn> (optimistic_db, all_tables (), tables_no_locks_a, write_lock_mutexes);
	}
	else
	{
		txn = std::make_unique<btcnew::write_rocksdb_txn> (optimistic_db, tables_requiring_locks_a, tables_no_locks_a, write_lock_mutexes);
	}
	...
	return btcnew::write_transaction{ std::move (txn) };
}

btcnew::read_transaction btcnew::rocksdb_store::tx_begin_read ()
{
	return btcnew::read_transaction{ std::make_unique<btcnew::read_rocksdb_txn> (db) };
}

rocksdb::ColumnFamilyHandle * btcnew::rocksdb_store::table_to_column_family (tables table_a) const
{
...
bool btcnew::rocksdb_store::exists (btcnew::transaction const & transaction_a, tables table_a, btcnew::rocksdb_val const & key_a) const
{
...
int btcnew::rocksdb_store::del (btcnew::write_transaction const & transaction_a, tables table_a, btcnew::rocksdb_val const & key_a)
{
...
bool btcnew::rocksdb_store::block_info_get (btcnew::transaction const &, btcnew::block_hash const &, btcnew::block_info &) const
{
...
void btcnew::rocksdb_store::version_put (btcnew::write_transaction const & transaction_a, int version_a)
{
	assert (transaction_a.contains (tables::meta));
	btcnew::uint256_union version_key (1);
	btcnew::uint256_union version_value (version_a);
	auto status (put (transaction_a, tables::meta, version_key, btcnew::rocksdb_val (version_value)));
	release_assert (success (status));
}

rocksdb::Transaction * btcnew::rocksdb_store::tx (btcnew::transaction const & transaction_a) const
{
	assert (!is_read (transaction_a));
	return static_cast<rocksdb::Transaction *> (transaction_a.get_handle ());
}

int btcnew::rocksdb_store::get (btcnew::transaction const & transaction_a, tables table_a, btcnew::rocksdb_val const & key_a, btcnew::rocksdb_val & value_a) const
{
	rocksdb::ReadOptions options;
	rocksdb::PinnableSlice slice;
...
bool btcnew::rocksdb_store::is_caching_counts (btcnew::tables table_a) const
{
	switch (table_a)
	{
		case tables::accounts:
...
int btcnew::rocksdb_store::increment (btcnew::write_transaction const & transaction_a, tables table_a, btcnew::rocksdb_val const & key_a, uint64_t amount_a)
{
	release_assert (transaction_a.contains (table_a));
	uint64_t base;
	btcnew::rocksdb_val value;
	...
	return put (transaction_a, table_a, key_a, btcnew::rocksdb_val (base + amount_a));
}

int btcnew::rocksdb_store::decrement (btcnew::write_transaction const & transaction_a, tables table_a, btcnew::rocksdb_val const & key_a, uint64_t amount_a)
{
	release_assert (transaction_a.contains (table_a));
	btcnew::rocksdb_val value;
...
int btcnew::rocksdb_store::put (btcnew::write_transaction const & transaction_a, tables table_a, btcnew::rocksdb_val const & key_a, btcnew::rocksdb_val const & value_a)
{
	assert (transaction_a.contains (table_a));
...
bool btcnew::rocksdb_store::not_found (int status) const
{
	return (status_code_not_found () == status);
}

bool btcnew::rocksdb_store::success (int status) const
{
	return (static_cast<int> (rocksdb::Status::Code::kOk) == status);
}

int btcnew::rocksdb_store::status_code_not_found () const
{
	return static_cast<int> (rocksdb::Status::Code::kNotFound);
}

uint64_t btcnew::rocksdb_store::count (btcnew::transaction const & transaction_a, rocksdb::ColumnFamilyHandle * handle) const
{
	uint64_t count = 0;
	btcnew::rocksdb_val val;
	auto const & key = handle->GetName ();
	auto status = get (transaction_a, tables::cached_counts, btcnew::rocksdb_val (key.size (), (void *)key.data ()), val);
	if (success (status))
	{
...
size_t btcnew::rocksdb_store::count (btcnew::transaction const & transaction_a, tables table_a) const
{
	size_t sum = 0;
...
int btcnew::rocksdb_store::drop (btcnew::write_transaction const & transaction_a, tables table_a)
{
	assert (transaction_a.contains (table_a));
	...
	{
		// Reset counter to 0
		status = put (transaction_a, tables::cached_counts, btcnew::rocksdb_val (rocksdb::Slice (col->GetName ())), btcnew::rocksdb_val (uint64_t{ 0 }));
	}
	...
	if (success (status))
	{
		...
		if (table_a == tables::peers)
		{
			...
			for (auto i = peers_begin (transaction_a), n = peers_end (); i != n; ++i)
			{
				status = del (transaction_a, tables::peers, btcnew::rocksdb_val (i->first));
				release_assert (success (status));
			}
			return status;
		}
...
int btcnew::rocksdb_store::clear (rocksdb::ColumnFamilyHandle * column_family)
{
...
void btcnew::rocksdb_store::construct_column_family_mutexes ()
{
...
rocksdb::Options btcnew::rocksdb_store::get_db_options () const
{
...
rocksdb::BlockBasedTableOptions btcnew::rocksdb_store::get_table_options () const
{
...
rocksdb::ColumnFamilyOptions btcnew::rocksdb_store::get_cf_options () const
{
...
std::vector<btcnew::tables> btcnew::rocksdb_store::all_tables () const
{
	return std::vector<btcnew::tables>{ tables::accounts, tables::cached_counts, tables::change_blocks, tables::confirmation_height, tables::frontiers, tables::meta, tables::online_weight, tables::open_blocks, tables::peers, tables::pending, tables::receive_blocks, tables::representation, tables::send_blocks, tables::state_blocks, tables::unchecked, tables::vote };
}

bool btcnew::rocksdb_store::copy_db (boost::filesystem::path const & destination_path)
{
	...
	if (status.ok ())
	{
		btcnew::rocksdb_store rocksdb_store (logger, destination_path.string (), rocksdb_config, false);
		return !rocksdb_store.init_error ();
	}
	return false;
}

bool btcnew::rocksdb_store::init_error () const
{
	return error;
}