...
namespace websocket
{
	...
	{
	public:
		message (btcnew::websocket::topic topic_a) :
		topic (topic_a)
		{
		}
		message (btcnew::websocket::topic topic_a, boost::property_tree::ptree & tree_a) :
		topic (topic_a), contents (tree_a)
		{
		}

		std::string to_string () const;
		btcnew::websocket::topic topic;
	...
	{
	public:
		message block_confirmed (std::shared_ptr<btcnew::block> block_a, btcnew::account const & account_a, btcnew::amount const & amount_a, std::string subtype, bool include_block, btcnew::election_status const & election_status_a, btcnew::websocket::confirmation_options const & options_a);
		message stopped_election (btcnew::block_hash const & hash_a);
		message vote_received (std::shared_ptr<btcnew::vote> vote_a);
		message difficulty_changed (uint64_t publish_threshold_a, uint64_t difficulty_active_a);
		message work_generation (btcnew::block_hash const & root_a, uint64_t const work_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::string const & peer_a, std::vector<std::string> const & bad_peers_a, bool const completed_a = true, bool const cancelled_a = false);
		message work_cancelled (btcnew::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a);
		message work_failed (btcnew::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a);
	...
	{
	public:
		confirmation_options (btcnew::node & node_a);
		confirmation_options (boost::property_tree::ptree const & options_a, btcnew::node & node_a);
	...
	public:
		vote_options ();
		vote_options (boost::property_tree::ptree const & options_a, btcnew::node & node_a);
	...
	public:
		...
		explicit session (btcnew::websocket::listener & listener_a, socket_type socket_a);
		~session ();
		...
		void write (btcnew::websocket::message message_a);

	private:
		/** The owning listener */
		btcnew::websocket::listener & ws_listener;
		/** Websocket */
	...
	{
	public:
		listener (btcnew::node & node_a, boost::asio::ip::tcp::endpoint endpoint_a);
		...
		void broadcast_confirmation (std::shared_ptr<btcnew::block> block_a, btcnew::account const & account_a, btcnew::amount const & amount_a, std::string subtype, btcnew::election_status const & election_status_a);

		/** Broadcast \p message to all session subscribing to the message topic. */
		void broadcast (btcnew::websocket::message message_a);

		btcnew::node & get_node () const
		{
			return node;
		}
		 ...
		 */
		bool any_subscriber (btcnew::websocket::topic const & topic_a) const
		{
			return subscriber_count (topic_a) > 0;
		}
		/** Getter for subscriber count of a specific topic*/
		size_t subscriber_count (btcnew::websocket::topic const & topic_a) const
		{
	...
	private:
		...
		friend btcnew::websocket::session;

		/** Adds to subscription count of a specific topic*/
		void increase_subscriber_count (btcnew::websocket::topic const & topic_a);
		/** Removes from subscription count of a specific topic*/
		void decrease_subscriber_count (btcnew::websocket::topic const & topic_a);
