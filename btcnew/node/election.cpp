...
btcnew::election_vote_result::election_vote_result (bool replay_a, bool processed_a)
{
	replay = replay_a;
	processed = processed_a;
}

btcnew::election::election (btcnew::node & node_a, std::shared_ptr<btcnew::block> block_a, bool const skip_delay_a, std::function<void (std::shared_ptr<btcnew::block>)> const & confirmation_action_a) :
confirmation_action (confirmation_action_a),
node (node_a),
election_start (std::chrono::steady_clock::now ()),
status ({ block_a, 0, std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::system_clock::now ().time_since_epoch ()), std::chrono::duration_values<std::chrono::milliseconds>::zero (), 0, btcnew::election_status_type::ongoing }),
skip_delay (skip_delay_a),
confirmed (false),
stopped (false)
{
	last_votes.insert (std::make_pair (node.network_params.random.not_an_account, btcnew::vote_info{ std::chrono::steady_clock::now (), 0, block_a->hash () }));
	blocks.insert (std::make_pair (block_a->hash (), block_a));
	update_dependent ();
}

void btcnew::election::compute_rep_votes (btcnew::transaction const & transaction_a)
{
	if (node.config.enable_voting)
	{
		node.wallets.foreach_representative ([this, &transaction_a] (btcnew::public_key const & pub_a, btcnew::raw_key const & prv_a) {
...
void btcnew::election::confirm_once (btcnew::election_status_type type_a)
{
...
void btcnew::election::stop ()
{
	if (!stopped && !confirmed)
	{
		...
		status.election_duration = std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::steady_clock::now () - election_start);
		status.confirmation_request_count = confirmation_request_count;
		status.type = btcnew::election_status_type::stopped;
	}
}

bool btcnew::election::have_quorum (btcnew::tally_t const & tally_a, btcnew::uint128_t tally_sum) const
{
	bool result = false;
...
btcnew::tally_t btcnew::election::tally ()
{
	std::unordered_map<btcnew::block_hash, btcnew::uint128_t> block_weights;
...
void btcnew::election::confirm_if_quorum ()
{
	auto tally_l (tally ());
	...
	if (have_quorum (tally_l, sum))
	{
		...
		confirm_once (btcnew::election_status_type::active_confirmed_quorum);
	}
}

void btcnew::election::log_votes (btcnew::tally_t const & tally_a) const
{
...
btcnew::election_vote_result btcnew::election::vote (btcnew::account rep, uint64_t sequence, btcnew::block_hash block_hash)
{
	...
	if (node.network_params.network.is_test_network () || weight > node.minimum_principal_weight (online_stake))
	{
		...
		if (should_process)
		{
			node.stats.inc (btcnew::stat::type::election, btcnew::stat::detail::vote_new);
	...
	return btcnew::election_vote_result (replay, should_process);
}

bool btcnew::election::publish (std::shared_ptr<btcnew::block> block_a)
{
	auto result (false);
	if (blocks.size () >= 10)
	{
...
size_t btcnew::election::last_votes_size ()
{
	btcnew::lock_guard<std::mutex> lock (node.active.mutex);
	return last_votes.size ();
}

void btcnew::election::update_dependent ()
{
	assert (!node.active.mutex.try_lock ());
	std::vector<btcnew::block_hash> blocks_search;
...
void btcnew::election::clear_dependent ()
{
...
void btcnew::election::clear_blocks ()
{
...
void btcnew::election::insert_inactive_votes_cache ()
{
	auto winner_hash (status.winner->hash ());
	auto cache (node.active.find_inactive_votes_cache (winner_hash));
	for (auto & rep : cache.voters)
	{
		auto inserted (last_votes.emplace (rep, btcnew::vote_info{ std::chrono::steady_clock::time_point::min (), 0, winner_hash }));
		if (inserted.second)
		{
			node.stats.inc (btcnew::stat::type::election, btcnew::stat::detail::vote_cached);
		}
	}
	if (!confirmed && !cache.voters.empty ())
	{
		...
		{
			node.stats.inc (btcnew::stat::type::election, btcnew::stat::detail::late_block);
			node.stats.add (btcnew::stat::type::election, btcnew::stat::detail::late_block_seconds, btcnew::stat::dir::in, delay.count (), true);
		}
		confirm_if_quorum ();
	}
}
