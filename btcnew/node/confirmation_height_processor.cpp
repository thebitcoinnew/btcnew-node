...
btcnew::confirmation_height_processor::confirmation_height_processor (btcnew::pending_confirmation_height & pending_confirmation_height_a, btcnew::ledger & ledger_a, btcnew::active_transactions & active_a, btcnew::write_database_queue & write_database_queue_a, std::chrono::milliseconds batch_separate_pending_min_time_a, btcnew::logger_mt & logger_a) :
...
thread ([this] () {
	btcnew::thread_role::set (btcnew::thread_role::name::confirmation_height_processing);
	this->run ();
})
{
}
...
void btcnew::confirmation_height_processor::stop ()
{
	stopped = true;
...
void btcnew::confirmation_height_processor::run ()
{
	btcnew::unique_lock<std::mutex> lk (pending_confirmations.mutex);
	while (!stopped)
	{
		...
		else
		{
			...
			{
				lk.unlock ();
				auto scoped_write_guard = write_database_queue.wait (btcnew::writer::confirmation_height);
...
void btcnew::confirmation_height_processor::pause ()
{
	paused = true;
}

void btcnew::confirmation_height_processor::unpause ()
{
	paused = false;
	condition.notify_one ();
}

void btcnew::confirmation_height_processor::add (btcnew::block_hash const & hash_a)
{
	{
		btcnew::lock_guard<std::mutex> lk (pending_confirmations.mutex);
 ...
 */
void btcnew::confirmation_height_processor::add_confirmation_height (btcnew::block_hash const & hash_a)
{
	...
	do
	{
		...
		btcnew::account account (ledger.store.block_account (read_transaction, current));
		...
		{
			if (write_database_queue.process (btcnew::writer::confirmation_height))
			{
 ...
 */
bool btcnew::confirmation_height_processor::write_pending (std::deque<conf_height_details> & all_pending_a)
{
	...
	while (total_pending_write_block_count > 0)
	{
		...
		auto transaction (ledger.store.tx_begin_write ({}, { btcnew::tables::confirmation_height }));
		while (!all_pending_a.empty ())
		{
			...
			if (pending.height > confirmation_height)
			{
				...
				if (!block)
				{
					...
					ledger.stats.inc (btcnew::stat::type::confirmation_height, btcnew::stat::detail::invalid_block);
				...
				ledger.stats.add (btcnew::stat::type::confirmation_height, btcnew::stat::detail::blocks_confirmed, btcnew::stat::dir::in, pending.height - confirmation_height);
...
void btcnew::confirmation_height_processor::collect_unconfirmed_receive_and_sources_for_account (uint64_t block_height_a, uint64_t confirmation_height_a, btcnew::block_hash const & hash_a, btcnew::account const & account_a, btcnew::read_transaction const & transaction_a, std::vector<callback_data> & block_callbacks_required)
{
	auto hash (hash_a);
	...
	while ((num_to_confirm > 0) && !hash.is_zero () && !stopped)
	{
		...
		if (block)
		{
			...
			else
			{
				...
				block_callbacks_required.emplace_back (block, sideband, btcnew::election_status_type::active_confirmed_quorum);
			}
...
namespace btcnew
{
confirmation_height_processor::conf_height_details::conf_height_details (btcnew::account const & account_a, btcnew::block_hash const & hash_a, uint64_t height_a, uint64_t num_blocks_confirmed_a, std::vector<callback_data> const & block_callbacks_required_a) :
account (account_a),
hash (hash_a),
height (height_a),
...
confirmation_height_processor::callback_data::callback_data (std::shared_ptr<btcnew::block> const & block_a, btcnew::block_sideband const & sideband_a, btcnew::election_status_type election_status_type_a) :
block (block_a),
sideband (sideband_a),
...
size_t btcnew::pending_confirmation_height::size ()
{
	btcnew::lock_guard<std::mutex> lk (mutex);
	return pending.size ();
}

bool btcnew::pending_confirmation_height::is_processing_block (btcnew::block_hash const & hash_a)
{
...
btcnew::block_hash btcnew::pending_confirmation_height::current ()
{
	btcnew::lock_guard<std::mutex> lk (mutex);
	return current_hash;
}
...
std::unique_ptr<seq_con_info_component> collect_seq_con_info (pending_confirmation_height & pending_confirmation_height_a, const std::string & name_a)
{
	...
	auto composite = std::make_unique<seq_con_info_composite> (name_a);
	composite->add_component (std::make_unique<seq_con_info_leaf> (seq_con_info{ "pending", pending_count, sizeof (btcnew::block_hash) }));
	return composite;
}
}
