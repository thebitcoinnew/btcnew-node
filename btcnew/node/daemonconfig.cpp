...
btcnew::daemon_config::daemon_config (boost::filesystem::path const & data_path_a) :
data_path (data_path_a)
{
}

btcnew::error btcnew::daemon_config::serialize_toml (btcnew::tomlconfig & toml)
{
	btcnew::tomlconfig rpc_l;
	rpc.serialize_toml (rpc_l);
	rpc_l.doc ("enable", "Enable or disable RPC\ntype:bool");
	rpc_l.put ("enable", rpc_enable);
	toml.put_child ("rpc", rpc_l);

	btcnew::tomlconfig node_l;
	node.serialize_toml (node_l);
	btcnew::tomlconfig node (node_l);
	toml.put_child ("node", node);

	btcnew::tomlconfig opencl_l;
	...
	btcnew::tomlconfig pow_server_l;
	pow_server.serialize_toml (pow_server_l);
	btcnew::tomlconfig pow_server (pow_server_l);
	toml.put_child ("btcnew_pow_server", pow_server);

	return toml.get_error ();
}

btcnew::error btcnew::daemon_config::deserialize_toml (btcnew::tomlconfig & toml)
{
	...
	auto pow_l (toml.get_optional_child ("btcnew_pow_server"));
...
btcnew::error btcnew::daemon_config::serialize_json (btcnew::jsonconfig & json)
{
	json.put ("version", json_version ());
	json.put ("rpc_enable", rpc_enable);

	btcnew::jsonconfig rpc_l;
	rpc.serialize_json (rpc_l);
	json.put_child ("rpc", rpc_l);

	btcnew::jsonconfig node_l;
	node.serialize_json (node_l);
	btcnew::jsonconfig node (node_l);
	json.put_child ("node", node);

	json.put ("opencl_enable", opencl_enable);
	btcnew::jsonconfig opencl_l;
	opencl.serialize_json (opencl_l);
	json.put_child ("opencl", opencl_l);
	return json.get_error ();
}

btcnew::error btcnew::daemon_config::deserialize_json (bool & upgraded_a, btcnew::jsonconfig & json)
{
	try
	{
		if (!json.empty ())
...
namespace btcnew
{
btcnew::error read_node_config_toml (boost::filesystem::path const & data_path_a, btcnew::daemon_config & config_a, std::vector<std::string> const & config_overrides)
{
	btcnew::error error;
	auto json_config_path = btcnew::get_config_path (data_path_a);
	auto toml_config_path = btcnew::get_node_toml_config_path (data_path_a);
	auto toml_qt_config_path = btcnew::get_qtwallet_toml_config_path (data_path_a);
	if (boost::filesystem::exists (json_config_path))
	{
		...
		{
			// Migrate
			btcnew::daemon_config config_old_l;
			...
			if (!error)
			{
				btcnew::tomlconfig toml_l;
				config_old_l.serialize_toml (toml_l);

				// Only write out non-default values
				btcnew::daemon_config config_defaults_l;
				btcnew::tomlconfig toml_defaults_l;
				...
				if (!toml_l.empty ())
				{
					...
					boost::system::error_code error_chmod;
					btcnew::set_secure_perm_file (toml_config_path, error_chmod);
				}
...
btcnew::error read_and_update_daemon_config (boost::filesystem::path const & data_path, btcnew::daemon_config & config_a, btcnew::jsonconfig & json_a)
{
	boost::system::error_code error_chmod;
	auto config_path = btcnew::get_config_path (data_path);
	auto error (json_a.read_and_update (config_a, config_path));
	btcnew::set_secure_perm_file (config_path, error_chmod);
	return error;
}
}
