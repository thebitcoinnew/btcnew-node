...
constexpr double btcnew::bootstrap_limits::bootstrap_connection_scale_target_blocks;
constexpr double btcnew::bootstrap_limits::bootstrap_connection_scale_target_blocks_lazy;
constexpr double btcnew::bootstrap_limits::bootstrap_minimum_blocks_per_sec;
constexpr double btcnew::bootstrap_limits::bootstrap_minimum_termination_time_sec;
constexpr unsigned btcnew::bootstrap_limits::bootstrap_max_new_connections;
constexpr size_t btcnew::bootstrap_limits::bootstrap_max_confirm_frontiers;
constexpr double btcnew::bootstrap_limits::required_frontier_confirmation_ratio;
constexpr unsigned btcnew::bootstrap_limits::frontier_confirmation_blocks_limit;
constexpr unsigned btcnew::bootstrap_limits::requeued_pulls_limit;
constexpr unsigned btcnew::bootstrap_limits::requeued_pulls_limit_test;
constexpr std::chrono::seconds btcnew::bootstrap_limits::lazy_flush_delay_sec;
constexpr unsigned btcnew::bootstrap_limits::lazy_destinations_request_limit;
constexpr uint64_t btcnew::bootstrap_limits::lazy_batch_pull_count_resize_blocks_limit;
constexpr double btcnew::bootstrap_limits::lazy_batch_pull_count_resize_ratio;
constexpr size_t btcnew::bootstrap_limits::lazy_blocks_restart_limit;
constexpr std::chrono::hours btcnew::bootstrap_excluded_peers::exclude_time_hours;
constexpr std::chrono::hours btcnew::bootstrap_excluded_peers::exclude_remove_hours;

btcnew::bootstrap_client::bootstrap_client (std::shared_ptr<btcnew::node> node_a, std::shared_ptr<btcnew::bootstrap_attempt> attempt_a, std::shared_ptr<btcnew::transport::channel_tcp> channel_a, std::shared_ptr<btcnew::socket> socket_a) :
node (node_a),
attempt (attempt_a),
channel (channel_a),
socket (socket_a),
...
btcnew::bootstrap_client::~bootstrap_client ()
{
	--attempt->connections;
}

double btcnew::bootstrap_client::block_rate () const
{
	auto elapsed = std::max (elapsed_seconds (), btcnew::bootstrap_limits::bootstrap_minimum_elapsed_seconds_blockrate);
	return static_cast<double> (block_count.load () / elapsed);
}

double btcnew::bootstrap_client::elapsed_seconds () const
{
	return std::chrono::duration_cast<std::chrono::duration<double>> (std::chrono::steady_clock::now () - start_time).count ();
}

void btcnew::bootstrap_client::stop (bool force)
{
	pending_stop = true;
	if (force)
	{
...
std::shared_ptr<btcnew::bootstrap_client> btcnew::bootstrap_client::shared ()
{
	return shared_from_this ();
}

btcnew::bootstrap_attempt::bootstrap_attempt (std::shared_ptr<btcnew::node> node_a, btcnew::bootstrap_mode mode_a) :
...
btcnew::bootstrap_attempt::~bootstrap_attempt ()
{
	node->logger.always_log ("Exiting bootstrap attempt");
	node->bootstrap_initiator.notify_listeners (false);
}

bool btcnew::bootstrap_attempt::should_log ()
{
	btcnew::lock_guard<std::mutex> guard (next_log_mutex);
	auto result (false);
...
bool btcnew::bootstrap_attempt::request_frontier (btcnew::unique_lock<std::mutex> & lock_a, bool first_attempt)
{
	auto result (true);
	...
	if (connection_l)
	{
		...
		{
			auto client (std::make_shared<btcnew::frontier_req_client> (connection_l));
			client->run ();
			frontiers = client;
		...
		if (node->config.logging.network_logging ())
		{
			...
			{
				node->stats.inc (btcnew::stat::type::error, btcnew::stat::detail::frontier_req, btcnew::stat::dir::out);
			}
		}
	}
	return result;
...
void btcnew::bootstrap_attempt::request_pull (btcnew::unique_lock<std::mutex> & lock_a)
{
	...
	if (connection_l)
	{
		...
		pulls.pop_front ();
		if (mode != btcnew::bootstrap_mode::legacy)
		{
		...
		if (recent_pulls_head.size () > btcnew::bootstrap_limits::bootstrap_max_confirm_frontiers)
		{
		...
		node->background ([connection_l, pull] () {
			auto client (std::make_shared<btcnew::bulk_pull_client> (connection_l, pull));
			client->request ();
		});
	}
}

void btcnew::bootstrap_attempt::request_push (btcnew::unique_lock<std::mutex> & lock_a)
{
	bool error (false);
	if (auto connection_shared = connection_frontier_request.lock ())
	{
		std::future<bool> future;
		{
			auto client (std::make_shared<btcnew::bulk_push_client> (connection_shared));
			client->start ();
			push = client;
			future = client->promise.get_future ();
		}
...
bool btcnew::bootstrap_attempt::still_pulling ()
{
...
void btcnew::bootstrap_attempt::run_start (btcnew::unique_lock<std::mutex> & lock_a)
{
	...
	if (!pulls.empty ())
	{
		...
		{
			auto k = btcnew::random_pool::generate_word32 (0, i);
			std::swap (pulls[i], pulls[k]);
		}
	}
}

void btcnew::bootstrap_attempt::run ()
{
	...
	if (!stopped)
	{
		...
		{
			lock.unlock ();
			mode = btcnew::bootstrap_mode::wallet_lazy;
			total_blocks = 0;
			wallet_run ();
			lock.lock ();
		}
		...
		{
			lock.unlock ();
			mode = btcnew::bootstrap_mode::lazy;
			total_blocks = 0;
			lazy_run ();
			lock.lock ();
		}
...
std::shared_ptr<btcnew::bootstrap_client> btcnew::bootstrap_attempt::connection (btcnew::unique_lock<std::mutex> & lock_a, bool use_front_connection)
{
	// clang-format off
	condition.wait (lock_a, [& stopped = stopped, &idle = idle] { return stopped || !idle.empty (); });
	// clang-format on
	std::shared_ptr<btcnew::bootstrap_client> result;
	if (!idle.empty ())
	{
...
bool btcnew::bootstrap_attempt::consume_future (std::future<bool> & future_a)
{
	bool result;
	try
	{
...
{
	bool operator() (const std::shared_ptr<btcnew::bootstrap_client> & lhs, const std::shared_ptr<btcnew::bootstrap_client> & rhs) const
	{
		return lhs->block_rate () > rhs->block_rate ();
	}
};

unsigned btcnew::bootstrap_attempt::target_connections (size_t pulls_remaining)
{
	...
	double target_blocks = (mode == btcnew::bootstrap_mode::lazy) ? btcnew::bootstrap_limits::bootstrap_connection_scale_target_blocks_lazy : btcnew::bootstrap_limits::bootstrap_connection_scale_target_blocks;
	double step_scale = std::min (1.0, std::max (0.0, (double)pulls_remaining / target_blocks));
	double lazy_term = (mode == btcnew::bootstrap_mode::lazy) ? (double)node->config.bootstrap_connections : 0.0;
...
void btcnew::bootstrap_attempt::populate_connections ()
{
	double rate_sum = 0.0;
	size_t num_pulls = 0;
	std::priority_queue<std::shared_ptr<btcnew::bootstrap_client>, std::vector<std::shared_ptr<btcnew::bootstrap_client>>, block_rate_cmp> sorted_connections;
	std::unordered_set<btcnew::tcp_endpoint> endpoints;
	{
		btcnew::unique_lock<std::mutex> lock (mutex);
		num_pulls = pulls.size ();
		std::deque<std::weak_ptr<btcnew::bootstrap_client>> new_clients;
		for (auto & c : clients)
		{
			if (auto client = c.lock ())
			{
				if (auto socket_l = client->channel->socket.lock ())
				{
					...
					if (client->elapsed_seconds () > btcnew::bootstrap_limits::bootstrap_connection_warmup_time_sec && client->block_count > 0)
					{
					...
					if (elapsed_sec > btcnew::bootstrap_limits::bootstrap_minimum_termination_time_sec && blocks_per_sec < btcnew::bootstrap_limits::bootstrap_minimum_blocks_per_sec)
					{
						if (node->config.logging.bulk_pull_logging ())
						{
							node->logger.try_log (boost::str (boost::format ("Stopping slow peer %1% (elapsed sec %2%s > %3%s and %4% blocks per second < %5%)") % client->channel->to_string () % elapsed_sec % btcnew::bootstrap_limits::bootstrap_minimum_termination_time_sec % blocks_per_sec % btcnew::bootstrap_limits::bootstrap_minimum_blocks_per_sec));
						}
	...
	{
		auto delta = std::min ((target - connections) * 2, btcnew::bootstrap_limits::bootstrap_max_new_connections);
		// TODO - tune this better
		// Not many peers respond, need to try to make more connections than we need.
		for (auto i = 0u; i < delta; i++)
		{
			auto endpoint (node->network.bootstrap_peer (mode == btcnew::bootstrap_mode::lazy));
			if (endpoint != btcnew::tcp_endpoint (boost::asio::ip::address_v6::any (), 0) && endpoints.find (endpoint) == endpoints.end () && !node->bootstrap_initiator.excluded_peers.check (endpoint))
			{
	...
	if (!stopped)
	{
		std::weak_ptr<btcnew::bootstrap_attempt> this_w (shared_from_this ());
...
void btcnew::bootstrap_attempt::start_populate_connections ()
{
...
void btcnew::bootstrap_attempt::add_connection (btcnew::endpoint const & endpoint_a)
{
	connect_client (btcnew::tcp_endpoint (endpoint_a.address (), endpoint_a.port ()));
}

void btcnew::bootstrap_attempt::connect_client (btcnew::tcp_endpoint const & endpoint_a)
{
	++connections;
	auto socket (std::make_shared<btcnew::socket> (node));
	...
	[this_l, socket, endpoint_a] (boost::system::error_code const & ec) {
		if (!ec)
		{
			...
			auto client (std::make_shared<btcnew::bootstrap_client> (this_l->node, this_l, std::make_shared<btcnew::transport::channel_tcp> (*this_l->node, socket), socket));
...
void btcnew::bootstrap_attempt::pool_connection (std::shared_ptr<btcnew::bootstrap_client> client_a)
{
	btcnew::lock_guard<std::mutex> lock (mutex);
...
void btcnew::bootstrap_attempt::stop ()
{
	btcnew::lock_guard<std::mutex> lock (mutex);
	stopped = true;
	condition.notify_all ();
...
void btcnew::bootstrap_attempt::add_pull (btcnew::pull_info const & pull_a)
{
	btcnew::pull_info pull (pull_a);
...
void btcnew::bootstrap_attempt::requeue_pull (btcnew::pull_info const & pull_a, bool network_error)
{
	auto pull (pull_a);
	if (!network_error)
	{
	...
	++requeued_pulls;
	if (mode != btcnew::bootstrap_mode::lazy && pull.attempts < pull.retry_limit + (pull.processed / 10000))
	{
		btcnew::lock_guard<std::mutex> lock (mutex);
		pulls.push_front (pull);
		condition.notify_all ();
	}
	else if (mode == btcnew::bootstrap_mode::lazy && (pull.retry_limit == std::numeric_limits<unsigned>::max () || pull.attempts <= pull.retry_limit + (pull.processed / node->network_params.bootstrap.lazy_max_pull_blocks)))
	{
	...
	else
	{
		...
		node->stats.inc (btcnew::stat::type::bootstrap, btcnew::stat::detail::bulk_pull_failed_account, btcnew::stat::dir::in);

		node->bootstrap_initiator.cache.add (pull);
		if (mode == btcnew::bootstrap_mode::lazy && pull.processed > 0)
		{
			assert (pull.account_or_head == pull.head);
			btcnew::lock_guard<std::mutex> lazy_lock (lazy_mutex);
			lazy_add (pull.account_or_head, pull.retry_limit);
		}
	}
}

void btcnew::bootstrap_attempt::add_bulk_push_target (btcnew::block_hash const & head, btcnew::block_hash const & end)
{
	btcnew::lock_guard<std::mutex> lock (mutex);
	bulk_push_targets.emplace_back (head, end);
}

void btcnew::bootstrap_attempt::attempt_restart_check (btcnew::unique_lock<std::mutex> & lock_a)
{
	...
	if (!frontiers_confirmed && (requeued_pulls > (!node->network_params.network.is_test_network () ? btcnew::bootstrap_limits::requeued_pulls_limit : btcnew::bootstrap_limits::requeued_pulls_limit_test) || total_blocks > btcnew::bootstrap_limits::frontier_confirmation_blocks_limit))
	{
		auto confirmed (confirm_frontiers (lock_a));
		assert (lock_a.owns_lock ());
		if (!confirmed)
		{
			node->stats.inc (btcnew::stat::type::bootstrap, btcnew::stat::detail::frontier_confirmation_failed, btcnew::stat::dir::in);
			auto score (node->bootstrap_initiator.excluded_peers.add (endpoint_frontier_request, node->network.size ()));
			if (score >= btcnew::bootstrap_excluded_peers::score_limit)
			{
		...
		{
			node->stats.inc (btcnew::stat::type::bootstrap, btcnew::stat::detail::frontier_confirmation_successful, btcnew::stat::dir::in);
		}
		frontiers_confirmed = confirmed;
	}
}

bool btcnew::bootstrap_attempt::confirm_frontiers (btcnew::unique_lock<std::mutex> & lock_a)
{
	...
	// clang-format on
	std::vector<btcnew::block_hash> frontiers;
	for (auto i (pulls.begin ()), end (pulls.end ()); i != end && frontiers.size () != btcnew::bootstrap_limits::bootstrap_max_confirm_frontiers; ++i)
	{
	...
	for (auto i (recent_pulls_head.begin ()), end (recent_pulls_head.end ()); i != end && frontiers.size () != btcnew::bootstrap_limits::bootstrap_max_confirm_frontiers; ++i)
	{
	...
	if (frontiers_count > 0)
	{
		...
		btcnew::uint128_t total_weight (0);
		...
		{
			auto k = btcnew::random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (representatives_copy.size () - 1));
		...
		{
			std::unordered_map<std::shared_ptr<btcnew::transport::channel>, std::deque<std::pair<btcnew::block_hash, btcnew::root>>> batched_confirm_req_bundle;
			std::deque<std::pair<btcnew::block_hash, btcnew::root>> request;
			...
			for (auto ii (frontiers.begin ()); ii != frontiers.end ();)
			{
				...
				{
					btcnew::lock_guard<std::mutex> active_lock (node->active.mutex);
					auto existing (node->active.find_inactive_votes_cache (*ii));
					btcnew::uint128_t tally;
					...
					else
					{
						for (auto const & rep : representatives)
						{
							if (std::find (existing.voters.begin (), existing.voters.end (), rep.account) == existing.voters.end ())
							{
								...
								{
									std::deque<std::pair<btcnew::block_hash, btcnew::root>> insert_root_hash = { std::make_pair (*ii, *ii) };
			...
			if (confirmed_count >= frontiers_count * btcnew::bootstrap_limits::required_frontier_confirmation_ratio) // 80% of frontiers confirmed
			{
				confirmed = true;
			}
...
void btcnew::bootstrap_attempt::lazy_start (btcnew::hash_or_account const & hash_or_account_a, bool confirmed)
{
	btcnew::lock_guard<std::mutex> lazy_lock (lazy_mutex);
...
void btcnew::bootstrap_attempt::lazy_add (btcnew::hash_or_account const & hash_or_account_a, unsigned retry_limit)
{
...
void btcnew::bootstrap_attempt::lazy_requeue (btcnew::block_hash const & hash_a, btcnew::block_hash const & previous_a, bool confirmed_a)
{
	btcnew::unique_lock<std::mutex> lazy_lock (lazy_mutex);
	...
	if (existing != lazy_blocks.end ())
	{
		...
		lazy_lock.unlock ();
		requeue_pull (btcnew::pull_info (hash_a, hash_a, previous_a, static_cast<btcnew::pull_info::count_t> (1), confirmed_a ? std::numeric_limits<unsigned>::max () : node->network_params.bootstrap.lazy_destinations_retry_limit));
	}
}

void btcnew::bootstrap_attempt::lazy_pull_flush ()
{
	assert (!mutex.try_lock ());
	static size_t const max_pulls (btcnew::bootstrap_limits::bootstrap_connection_scale_target_blocks_lazy * 3);
	if (pulls.size () < max_pulls)
	{
		last_lazy_flush = std::chrono::steady_clock::now ();
		btcnew::lock_guard<std::mutex> lazy_lock (lazy_mutex);
		assert (node->network_params.bootstrap.lazy_max_pull_blocks <= std::numeric_limits<btcnew::pull_info::count_t>::max ());
		btcnew::pull_info::count_t batch_count (node->network_params.bootstrap.lazy_max_pull_blocks);
		if (total_blocks > btcnew::bootstrap_limits::lazy_batch_pull_count_resize_blocks_limit && !lazy_blocks.empty ())
		{
			double lazy_blocks_ratio (total_blocks / lazy_blocks.size ());
			if (lazy_blocks_ratio > btcnew::bootstrap_limits::lazy_batch_pull_count_resize_ratio)
			{
				// Increasing blocks ratio weight as more important (^3). Small batch count should lower blocks ratio below target
				double lazy_blocks_factor (std::pow (lazy_blocks_ratio / btcnew::bootstrap_limits::lazy_batch_pull_count_resize_ratio, 3.0));
				// Decreasing total block count weight as less important (sqrt)
				double total_blocks_factor (std::sqrt (total_blocks / btcnew::bootstrap_limits::lazy_batch_pull_count_resize_blocks_limit));
		...
		while (!lazy_pulls.empty () && count < max_pulls)
		{
			...
			{
				pulls.emplace_back (pull_start.first, pull_start.first, btcnew::block_hash (0), batch_count, pull_start.second);
				++count;
			}
...
bool btcnew::bootstrap_attempt::lazy_finished ()
{
	if (stopped)
	{
		return true;
...
bool btcnew::bootstrap_attempt::lazy_has_expired () const
{
	bool result (false);
	...
	else if (!node->flags.disable_legacy_bootstrap && lazy_blocks_count > btcnew::bootstrap_limits::lazy_blocks_restart_limit)
	{
		result = true;
	}
	return result;
...
void btcnew::bootstrap_attempt::lazy_clear ()
{
	assert (!lazy_mutex.try_lock ());
	lazy_blocks.clear ();
	lazy_blocks_count = 0;
...
void btcnew::bootstrap_attempt::lazy_run ()
{
	...
	while ((still_pulling () || !lazy_finished ()) && !lazy_has_expired ())
	{
		...
		while (still_pulling () && !lazy_has_expired ())
		{
			...
			++iterations;
			// Flushing lazy pulls
			if (iterations % 100 == 0 || last_lazy_flush + btcnew::bootstrap_limits::lazy_flush_delay_sec < std::chrono::steady_clock::now ())
			{
				lazy_pull_flush ();
			}
	...
	if (!stopped)
	{
		...
		else if (runs_count < 3 && !lazy_keys.empty () && !node->flags.disable_legacy_bootstrap)
		{
			pulls.clear ();
			lazy_clear ();
			mode = btcnew::bootstrap_mode::legacy;
			lock.unlock ();
			lazy_lock.unlock ();
			run ();
			lock.lock ();
...
bool btcnew::bootstrap_attempt::process_block (std::shared_ptr<btcnew::block> block_a, btcnew::account const & known_account_a, uint64_t pull_blocks, btcnew::bulk_pull::count_t max_blocks, bool block_expected, unsigned retry_limit)
{
	bool stop_pull (false);
	if (mode != btcnew::bootstrap_mode::legacy && block_expected)
	{
		stop_pull = process_block_lazy (block_a, known_account_a, pull_blocks, max_blocks, retry_limit);
	}
	else if (mode != btcnew::bootstrap_mode::legacy)
	{
	...
	{
		btcnew::unchecked_info info (block_a, known_account_a, 0, btcnew::signature_verification::unknown);
		node->block_processor.add (info);
	}
	return stop_pull;
}

bool btcnew::bootstrap_attempt::process_block_lazy (std::shared_ptr<btcnew::block> block_a, btcnew::account const & known_account_a, uint64_t pull_blocks, btcnew::bulk_pull::count_t max_blocks, unsigned retry_limit)
{
	bool stop_pull (false);
	...
	if (lazy_blocks.find (hash) == lazy_blocks.end ())
	{
		...
		else if (block_a->type () == btcnew::block_type::send)
		{
			std::shared_ptr<btcnew::send_block> block_l (std::static_pointer_cast<btcnew::send_block> (block_a));
		...
		if (pull_blocks == 0 && (block_a->type () == btcnew::block_type::state || block_a->type () == btcnew::block_type::send))
		{
		...
		lazy_lock.unlock ();
		btcnew::unchecked_info info (block_a, known_account_a, 0, btcnew::signature_verification::unknown, retry_limit == std::numeric_limits<unsigned>::max ());
		node->block_processor.add (info);
	}
...
void btcnew::bootstrap_attempt::lazy_block_state (std::shared_ptr<btcnew::block> block_a, unsigned retry_limit)
{
	std::shared_ptr<btcnew::state_block> block_l (std::static_pointer_cast<btcnew::state_block> (block_a));
	if (block_l != nullptr)
	{
		auto transaction (node->store.tx_begin_read ());
		btcnew::uint128_t balance (block_l->hashables.balance.number ());
		...
		if (!link.is_zero () && !node->ledger.is_epoch_link (link) && lazy_blocks.find (link) == lazy_blocks.end () && !node->store.block_exists (transaction, link))
		{
			...
			{
				lazy_state_backlog.emplace (previous, btcnew::lazy_state_backlog_item{ link, balance, retry_limit });
			}
		}
	}
}

void btcnew::bootstrap_attempt::lazy_block_state_backlog_check (std::shared_ptr<btcnew::block> block_a, btcnew::block_hash const & hash_a)
{
	...
	if (find_state != lazy_state_backlog.end ())
	{
		...
		if (block_a->type () == btcnew::block_type::state || block_a->type () == btcnew::block_type::send)
		{
...
void btcnew::bootstrap_attempt::lazy_backlog_cleanup ()
{
	auto transaction (node->store.tx_begin_read ());
	btcnew::lock_guard<std::mutex> lazy_lock (lazy_mutex);
...
void btcnew::bootstrap_attempt::lazy_destinations_increment (btcnew::account const & destination_a)
{
	...
	if (node->flags.disable_legacy_bootstrap)
	{
		...
		{
			lazy_destinations.get<account_tag> ().modify (existing, [] (btcnew::lazy_destinations_item & item_a) {
				++item_a.count;
			});
		}
		else
		{
			lazy_destinations.insert (btcnew::lazy_destinations_item{ destination_a, 1 });
		}
	}
}

void btcnew::bootstrap_attempt::lazy_destinations_flush ()
{
	lazy_destinations_flushed = true;
	size_t count (0);
	btcnew::lock_guard<std::mutex> lazy_lock (lazy_mutex);
	for (auto it (lazy_destinations.get<count_tag> ().begin ()), end (lazy_destinations.get<count_tag> ().end ()); it != end && count < btcnew::bootstrap_limits::lazy_destinations_request_limit && !stopped;)
	{
...
bool btcnew::bootstrap_attempt::lazy_processed_or_exists (btcnew::block_hash const & hash_a)
{
	bool result (false);
	btcnew::unique_lock<std::mutex> lazy_lock (lazy_mutex);
...
void btcnew::bootstrap_attempt::request_pending (btcnew::unique_lock<std::mutex> & lock_a)
{
	auto connection_l (connection (lock_a));
	if (connection_l)
	{
		...
		node->background ([connection_l, account] () {
			auto client (std::make_shared<btcnew::bulk_pull_account_client> (connection_l, account));
			client->request ();
		});
	}
}

void btcnew::bootstrap_attempt::requeue_pending (btcnew::account const & account_a)
{
...
void btcnew::bootstrap_attempt::wallet_start (std::deque<btcnew::account> & accounts_a)
{
	btcnew::lock_guard<std::mutex> lock (mutex);
	wallet_accounts.swap (accounts_a);
}

bool btcnew::bootstrap_attempt::wallet_finished ()
{
	assert (!mutex.try_lock ());
	auto running (!stopped);
...
void btcnew::bootstrap_attempt::wallet_run ()
{
...
btcnew::bootstrap_initiator::bootstrap_initiator (btcnew::node & node_a) :
node (node_a),
stopped (false),
thread ([this] () {
	btcnew::thread_role::set (btcnew::thread_role::name::bootstrap_initiator);
	run_bootstrap ();
})
{
}
...
void btcnew::bootstrap_initiator::bootstrap (bool force)
{
	btcnew::unique_lock<std::mutex> lock (mutex);
	...
	{
		node.stats.inc (btcnew::stat::type::bootstrap, btcnew::stat::detail::initiate, btcnew::stat::dir::out);
		attempt = std::make_shared<btcnew::bootstrap_attempt> (node.shared ());
		condition.notify_all ();
	}
}

void btcnew::bootstrap_initiator::bootstrap (btcnew::endpoint const & endpoint_a, bool add_to_peers, bool frontiers_confirmed)
{
	if (add_to_peers)
	{
		node.network.udp_channels.insert (btcnew::transport::map_endpoint_to_v6 (endpoint_a), node.network_params.protocol.protocol_version);
	}
	btcnew::unique_lock<std::mutex> lock (mutex);
	if (!stopped)
	{
		...
		{
			excluded_peers.remove (btcnew::transport::map_endpoint_to_tcp (endpoint_a));
		}
		if (!excluded_peers.check (btcnew::transport::map_endpoint_to_tcp (endpoint_a)))
		{
...
void btcnew::bootstrap_initiator::bootstrap_lazy (btcnew::hash_or_account const & hash_or_account_a, bool force, bool confirmed)
{
	{
		btcnew::unique_lock<std::mutex> lock (mutex);
		if (force && attempt != nullptr)
		...
		node.stats.inc (btcnew::stat::type::bootstrap, btcnew::stat::detail::initiate_lazy, btcnew::stat::dir::out);
		if (attempt == nullptr)
		{
			attempt = std::make_shared<btcnew::bootstrap_attempt> (node.shared (), btcnew::bootstrap_mode::lazy);
		}
...
void btcnew::bootstrap_initiator::bootstrap_wallet (std::deque<btcnew::account> & accounts_a)
{
	{
		btcnew::unique_lock<std::mutex> lock (mutex);
		node.stats.inc (btcnew::stat::type::bootstrap, btcnew::stat::detail::initiate_wallet_lazy, btcnew::stat::dir::out);
		if (attempt == nullptr)
		{
			attempt = std::make_shared<btcnew::bootstrap_attempt> (node.shared (), btcnew::bootstrap_mode::wallet_lazy);
		}
...
void btcnew::bootstrap_initiator::run_bootstrap ()
{
	btcnew::unique_lock<std::mutex> lock (mutex);
	while (!stopped)
	{
...
void btcnew::bootstrap_initiator::add_observer (std::function<void (bool)> const & observer_a)
{
	btcnew::lock_guard<std::mutex> lock (observers_mutex);
	observers.push_back (observer_a);
}

bool btcnew::bootstrap_initiator::in_progress ()
{
	return current_attempt () != nullptr;
}

std::shared_ptr<btcnew::bootstrap_attempt> btcnew::bootstrap_initiator::current_attempt ()
{
	btcnew::lock_guard<std::mutex> lock (mutex);
	return attempt;
}

void btcnew::bootstrap_initiator::stop ()
{
...
void btcnew::bootstrap_initiator::notify_listeners (bool in_progress_a)
{
	btcnew::lock_guard<std::mutex> lock (observers_mutex);
...
std::unique_ptr<seq_con_info_component> collect_seq_con_info (bootstrap_initiator & bootstrap_initiator, const std::string & name)
{
	...
	{
		btcnew::lock_guard<std::mutex> guard (bootstrap_initiator.observers_mutex);
		count = bootstrap_initiator.observers.size ();
	}
	{
		btcnew::lock_guard<std::mutex> guard (bootstrap_initiator.cache.pulls_cache_mutex);
		cache_count = bootstrap_initiator.cache.cache.size ();
	}
	{
		btcnew::lock_guard<std::mutex> guard (bootstrap_initiator.excluded_peers.excluded_peers_mutex);
...
void btcnew::pulls_cache::add (btcnew::pull_info const & pull_a)
{
	if (pull_a.processed > 500)
	{
		btcnew::lock_guard<std::mutex> guard (pulls_cache_mutex);
		// Clean old pull
		...
		btcnew::uint512_union head_512 (pull_a.account_or_head, pull_a.head_original);
		auto existing (cache.get<account_head_tag> ().find (head_512));
		if (existing == cache.get<account_head_tag> ().end ())
		{
			// Insert new pull
			auto inserted (cache.insert (btcnew::cached_pulls{ std::chrono::steady_clock::now (), head_512, pull_a.head }));
			(void)inserted;
		...
		else
		{
			...
			cache.get<account_head_tag> ().modify (existing, [pull_a] (btcnew::cached_pulls & cache_a) {
...
void btcnew::pulls_cache::update_pull (btcnew::pull_info & pull_a)
{
	btcnew::lock_guard<std::mutex> guard (pulls_cache_mutex);
	btcnew::uint512_union head_512 (pull_a.account_or_head, pull_a.head_original);
	auto existing (cache.get<account_head_tag> ().find (head_512));
...
void btcnew::pulls_cache::remove (btcnew::pull_info const & pull_a)
{
	btcnew::lock_guard<std::mutex> guard (pulls_cache_mutex);
	btcnew::uint512_union head_512 (pull_a.account_or_head, pull_a.head_original);
...
uint64_t btcnew::bootstrap_excluded_peers::add (btcnew::tcp_endpoint const & endpoint_a, size_t network_peers_count)
{
	uint64_t result (0);
	btcnew::lock_guard<std::mutex> guard (excluded_peers_mutex);
	...
	{
		// Insert new endpoint
		auto inserted (peers.insert (btcnew::excluded_peers_item{ std::chrono::steady_clock::steady_clock::now () + exclude_time_hours, endpoint_a, 1 }));
		(void)inserted;
		assert (inserted.second);
		result = 1;
	}
	else
	{
		...
		peers.get<endpoint_tag> ().modify (existing, [&result] (btcnew::excluded_peers_item & item_a) {
			++item_a.score;
			result = item_a.score;
			if (item_a.score == btcnew::bootstrap_excluded_peers::score_limit)
			{
				item_a.exclude_until = std::chrono::steady_clock::now () + btcnew::bootstrap_excluded_peers::exclude_time_hours;
			}
			else if (item_a.score > btcnew::bootstrap_excluded_peers::score_limit)
			{
				item_a.exclude_until = std::chrono::steady_clock::now () + btcnew::bootstrap_excluded_peers::exclude_time_hours * item_a.score * 2;
			}
		});
	}
	return result;
...
bool btcnew::bootstrap_excluded_peers::check (btcnew::tcp_endpoint const & endpoint_a)
{
	bool excluded (false);
	btcnew::lock_guard<std::mutex> guard (excluded_peers_mutex);
	auto existing (peers.get<endpoint_tag> ().find (endpoint_a));
...
void btcnew::bootstrap_excluded_peers::remove (btcnew::tcp_endpoint const & endpoint_a)
{
	btcnew::lock_guard<std::mutex> guard (excluded_peers_mutex);
