...
btcnew::pull_info::pull_info (btcnew::hash_or_account const & account_or_head_a, btcnew::block_hash const & head_a, btcnew::block_hash const & end_a, count_t count_a, unsigned retry_limit_a) :
...
btcnew::bulk_pull_client::bulk_pull_client (std::shared_ptr<btcnew::bootstrap_client> connection_a, btcnew::pull_info const & pull_a) :
...
btcnew::bulk_pull_client::~bulk_pull_client ()
{
	// If received end block is not expected end block
	if (expected != pull.end)
	{
		pull.head = expected;
		if (connection->attempt->mode != btcnew::bootstrap_mode::legacy)
		{
	...
	{
		btcnew::lock_guard<std::mutex> mutex (connection->attempt->mutex);
		--connection->attempt->pulling;
	}
	connection->attempt->condition.notify_all ();
}

void btcnew::bulk_pull_client::request ()
{
	assert (!pull.head.is_zero () || pull.retry_limit != std::numeric_limits<unsigned>::max ());
	expected = pull.head;
	btcnew::bulk_pull req;
	...
	if (connection->node->config.logging.bulk_pull_logging ())
	{
		btcnew::unique_lock<std::mutex> lock (connection->attempt->mutex);
	...
	req, [this_l] (boost::system::error_code const & ec, size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (btcnew::stat::type::bootstrap, btcnew::stat::detail::bulk_pull_request_failure, btcnew::stat::dir::in);
		}
	},
	false); // is bootstrap traffic is_droppable false
}

void btcnew::bulk_pull_client::throttled_receive_block ()
{
...
void btcnew::bulk_pull_client::receive_block ()
{
	auto this_l (shared_from_this ());
	if (auto socket_l = connection->channel->socket.lock ())
	{
		socket_l->async_read (connection->receive_buffer, 1, [this_l] (boost::system::error_code const & ec, size_t size_a) {
			...
			else
			{
				...
				this_l->connection->node->stats.inc (btcnew::stat::type::bootstrap, btcnew::stat::detail::bulk_pull_receive_block_failure, btcnew::stat::dir::in);
...
void btcnew::bulk_pull_client::received_type ()
{
	auto this_l (shared_from_this ());
	btcnew::block_type type (static_cast<btcnew::block_type> (connection->receive_buffer->data ()[0]));
	...
	if (auto socket_l = connection->channel->socket.lock ())
	{
		switch (type)
		{
			case btcnew::block_type::send: {
				socket_l->async_read (connection->receive_buffer, btcnew::send_block::size, [this_l, type] (boost::system::error_code const & ec, size_t size_a) {
					this_l->received_block (ec, size_a, type);
				});
				break;
			}
			case btcnew::block_type::receive: {
				socket_l->async_read (connection->receive_buffer, btcnew::receive_block::size, [this_l, type] (boost::system::error_code const & ec, size_t size_a) {
					this_l->received_block (ec, size_a, type);
				});
				break;
			}
			case btcnew::block_type::open: {
				socket_l->async_read (connection->receive_buffer, btcnew::open_block::size, [this_l, type] (boost::system::error_code const & ec, size_t size_a) {
					this_l->received_block (ec, size_a, type);
				});
				break;
			}
			case btcnew::block_type::change: {
				socket_l->async_read (connection->receive_buffer, btcnew::change_block::size, [this_l, type] (boost::system::error_code const & ec, size_t size_a) {
					this_l->received_block (ec, size_a, type);
				});
				break;
			}
			case btcnew::block_type::state: {
				socket_l->async_read (connection->receive_buffer, btcnew::state_block::size, [this_l, type] (boost::system::error_code const & ec, size_t size_a) {
					this_l->received_block (ec, size_a, type);
				});
				break;
			}
...
void btcnew::bulk_pull_client::received_block (boost::system::error_code const & ec, size_t size_a, btcnew::block_type type_a)
{
	if (!ec)
	{
		btcnew::bufferstream stream (connection->receive_buffer->data (), size_a);
		std::shared_ptr<btcnew::block> block (btcnew::deserialize_block (stream, type_a));
		if (block != nullptr && !btcnew::work_validate (*block))
		{
			...
			// Unconfirmed head is used only for lazy destinations if legacy bootstrap is not available, see btcnew::bootstrap_attempt::lazy_destinations_increment (...)
			...
			if (!stop_pull && !connection->hard_stop.load ())
			{
				...
				to prevent spam */
				if (connection->attempt->mode != btcnew::bootstrap_mode::legacy || unexpected_count < 16384)
				{
		...
		else
		{
			...
			connection->node->stats.inc (btcnew::stat::type::bootstrap, btcnew::stat::detail::bulk_pull_deserialize_receive_block, btcnew::stat::dir::in);
		}
	}
	else
	{
		...
		connection->node->stats.inc (btcnew::stat::type::bootstrap, btcnew::stat::detail::bulk_pull_receive_block_failure, btcnew::stat::dir::in);
		network_error = true;
	}
}

btcnew::bulk_pull_account_client::bulk_pull_account_client (std::shared_ptr<btcnew::bootstrap_client> connection_a, btcnew::account const & account_a) :
connection (connection_a),
account (account_a),
pull_blocks (0)
{
...
btcnew::bulk_pull_account_client::~bulk_pull_account_client ()
{
	{
		btcnew::lock_guard<std::mutex> mutex (connection->attempt->mutex);
		--connection->attempt->pulling;
...
void btcnew::bulk_pull_account_client::request ()
{
	btcnew::bulk_pull_account req;
	req.account = account;
	req.minimum_amount = connection->node->config.receive_minimum;
	req.flags = btcnew::bulk_pull_account_flags::pending_hash_and_amount;
	if (connection->node->config.logging.bulk_pull_logging ())
	{
		btcnew::unique_lock<std::mutex> lock (connection->attempt->mutex);
	...
	req, [this_l] (boost::system::error_code const & ec, size_t size_a) {
		...
		else
		{
			...
			this_l->connection->node->stats.inc (btcnew::stat::type::bootstrap, btcnew::stat::detail::bulk_pull_error_starting_request, btcnew::stat::dir::in);
		}
	},
	false); // is bootstrap traffic is_droppable false
}

void btcnew::bulk_pull_account_client::receive_pending ()
{
	auto this_l (shared_from_this ());
	size_t size_l (sizeof (btcnew::uint256_union) + sizeof (btcnew::uint128_union));
	if (auto socket_l = connection->channel->socket.lock ())
	{
		socket_l->async_read (connection->receive_buffer, size_l, [this_l, size_l] (boost::system::error_code const & ec, size_t size_a) {
			...
			if (size_a == size_l)
			{
				if (!ec)
				{
					btcnew::block_hash pending;
					btcnew::bufferstream frontier_stream (this_l->connection->receive_buffer->data (), sizeof (btcnew::uint256_union));
					auto error1 (btcnew::try_read (frontier_stream, pending));
					(void)error1;
					assert (!error1);
					btcnew::amount balance;
					btcnew::bufferstream balance_stream (this_l->connection->receive_buffer->data () + sizeof (btcnew::uint256_union), sizeof (btcnew::uint128_union));
					auto error2 (btcnew::try_read (balance_stream, balance));
					(void)error2;
					assert (!error2);
 ...
 */
void btcnew::bulk_pull_server::set_current_end ()
{
	include_start = false;
...
void btcnew::bulk_pull_server::send_next ()
{
	auto block (get_next ());
	if (block != nullptr)
	{
		...
		{
			btcnew::vectorstream stream (send_buffer);
			btcnew::serialize_block (stream, *block);
		}
		auto this_l (shared_from_this ());
		if (connection->node->config.logging.bulk_pull_logging ())
		{
		...
		connection->socket->async_write (btcnew::shared_const_buffer (std::move (send_buffer)), [this_l] (boost::system::error_code const & ec, size_t size_a) {
...
std::shared_ptr<btcnew::block> btcnew::bulk_pull_server::get_next ()
{
...
void btcnew::bulk_pull_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next ();
...
void btcnew::bulk_pull_server::send_finished ()
{
	btcnew::shared_const_buffer send_buffer (static_cast<uint8_t> (btcnew::block_type::not_a_block));
	auto this_l (shared_from_this ());
	if (connection->node->config.logging.bulk_pull_logging ())
	{
...
void btcnew::bulk_pull_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == 1);
...
btcnew::bulk_pull_server::bulk_pull_server (std::shared_ptr<btcnew::bootstrap_server> const & connection_a, std::unique_ptr<btcnew::bulk_pull> request_a) :
connection (connection_a),
 ...
 */
void btcnew::bulk_pull_account_server::set_params ()
{
	assert (request != nullptr);
	...
	if (request->flags == btcnew::bulk_pull_account_flags::pending_address_only)
	{
		pending_address_only = true;
	}
	else if (request->flags == btcnew::bulk_pull_account_flags::pending_hash_amount_and_address)
	{
		/**
	...
	else if (request->flags == btcnew::bulk_pull_account_flags::pending_hash_and_amount)
	{
...
void btcnew::bulk_pull_account_server::send_frontier ()
{
	/*
	...
	if (!invalid_request)
	{
		...
		btcnew::uint128_union account_frontier_balance (account_frontier_balance_int);

		// Write the frontier block hash and balance into a buffer
		std::vector<uint8_t> send_buffer;
		{
			btcnew::vectorstream output_stream (send_buffer);
...
void btcnew::bulk_pull_account_server::send_next_block ()
{
	/*
...
std::pair<std::unique_ptr<btcnew::pending_key>, std::unique_ptr<btcnew::pending_info>> btcnew::bulk_pull_account_server::get_next ()
{
	std::pair<std::unique_ptr<btcnew::pending_key>, std::unique_ptr<btcnew::pending_info>> result;
	...
	while (true)
	{
		...
		if (stream == btcnew::store_iterator<btcnew::pending_key, btcnew::pending_info> (nullptr))
		{
			break;
		}

		btcnew::pending_key key (stream->first);
		btcnew::pending_info info (stream->second);
		...
		result.first = std::unique_ptr<btcnew::pending_key> (new btcnew::pending_key (key));
		result.second = std::unique_ptr<btcnew::pending_info> (new btcnew::pending_info (info));
...
void btcnew::bulk_pull_account_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		send_next_block ();
...
void btcnew::bulk_pull_account_server::send_finished ()
{
	/*
	...
	{
		btcnew::vectorstream output_stream (send_buffer);
		btcnew::uint256_union account_zero (0);
		btcnew::uint128_union balance_zero (0);
...
void btcnew::bulk_pull_account_server::complete (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		if (pending_address_only)
...
btcnew::bulk_pull_account_server::bulk_pull_account_server (std::shared_ptr<btcnew::bootstrap_server> const & connection_a, std::unique_ptr<btcnew::bulk_pull_account> request_a) :
connection (connection_a),
request (std::move (request_a)),
current_key (0, 0)
{
