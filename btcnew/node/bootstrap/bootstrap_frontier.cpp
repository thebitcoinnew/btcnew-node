...
constexpr double btcnew::bootstrap_limits::bootstrap_connection_warmup_time_sec;
constexpr double btcnew::bootstrap_limits::bootstrap_minimum_elapsed_seconds_blockrate;
constexpr double btcnew::bootstrap_limits::bootstrap_minimum_frontier_blocks_per_sec;
constexpr unsigned btcnew::bootstrap_limits::bulk_push_cost_limit;

constexpr size_t btcnew::frontier_req_client::size_frontier;

void btcnew::frontier_req_client::run ()
{
	btcnew::frontier_req request;
	request.start.clear ();
...
btcnew::frontier_req_client::frontier_req_client (std::shared_ptr<btcnew::bootstrap_client> connection_a) :
...
btcnew::frontier_req_client::~frontier_req_client ()
{
}

void btcnew::frontier_req_client::receive_frontier ()
{
	auto this_l (shared_from_this ());
	if (auto socket_l = connection->channel->socket.lock ())
	{
		socket_l->async_read (connection->receive_buffer, btcnew::frontier_req_client::size_frontier, [this_l] (boost::system::error_code const & ec, size_t size_a) {
			// An issue with asio is that sometimes, instead of reporting a bad file descriptor during disconnect,
			// we simply get a size of 0.
			if (size_a == btcnew::frontier_req_client::size_frontier)
			{
			...
			else
			{
				...
				{
					this_l->connection->node->logger.try_log (boost::str (boost::format ("Invalid size: expected %1%, got %2%") % btcnew::frontier_req_client::size_frontier % size_a));
				}
			}
		});
	}
...
void btcnew::frontier_req_client::unsynced (btcnew::block_hash const & head, btcnew::block_hash const & end)
{
	if (bulk_push_cost < btcnew::bootstrap_limits::bulk_push_cost_limit)
	{
...
void btcnew::frontier_req_client::received_frontier (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		assert (size_a == btcnew::frontier_req_client::size_frontier);
		btcnew::account account;
		btcnew::bufferstream account_stream (connection->receive_buffer->data (), sizeof (account));
		auto error1 (btcnew::try_read (account_stream, account));
		(void)error1;
		assert (!error1);
		btcnew::block_hash latest;
		btcnew::bufferstream latest_stream (connection->receive_buffer->data () + sizeof (account), sizeof (latest));
		auto error2 (btcnew::try_read (latest_stream, latest));
		(void)error2;
		assert (!error2);
		if (count == 0)
		{
		...
		double elapsed_sec = std::max (time_span.count (), btcnew::bootstrap_limits::bootstrap_minimum_elapsed_seconds_blockrate);
		double blocks_per_sec = static_cast<double> (count) / elapsed_sec;
		if (elapsed_sec > btcnew::bootstrap_limits::bootstrap_connection_warmup_time_sec && blocks_per_sec < btcnew::bootstrap_limits::bootstrap_minimum_frontier_blocks_per_sec)
		{
			...
			if (!current.is_zero ())
			{
				if (account == current)
				{
					...
					else
					{
						...
						{
							connection->attempt->add_pull (btcnew::pull_info (account, latest, frontier, 0, connection->node->network_params.bootstrap.frontier_retry_limit));
				...
				else
				{
					...
					connection->attempt->add_pull (btcnew::pull_info (account, latest, btcnew::block_hash (0), 0, connection->node->network_params.bootstrap.frontier_retry_limit));
				}
			}
			else
			{
...
void btcnew::frontier_req_client::next (btcnew::transaction const & transaction_a)
{
	...
	if (accounts.empty ())
	{
		...
		{
			accounts.emplace_back (btcnew::account (0), btcnew::block_hash (0));
		}
	}
...
btcnew::frontier_req_server::frontier_req_server (std::shared_ptr<btcnew::bootstrap_server> const & connection_a, std::unique_ptr<btcnew::frontier_req> request_a) :
connection (connection_a),
...
void btcnew::frontier_req_server::send_next ()
{
...
void btcnew::frontier_req_server::send_finished ()
{
	std::vector<uint8_t> send_buffer;
	{
		btcnew::vectorstream stream (send_buffer);
		btcnew::uint256_union zero (0);
...
void btcnew::frontier_req_server::no_block_sent (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
...
void btcnew::frontier_req_server::sent_action (boost::system::error_code const & ec, size_t size_a)
{
	if (!ec)
	{
		count++;
...
void btcnew::frontier_req_server::next ()
{
	// Filling accounts deque to prevent often read transactions
	if (accounts.empty ())
	{
