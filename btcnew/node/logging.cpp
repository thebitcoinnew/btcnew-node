...
BOOST_LOG_ATTRIBUTE_KEYWORD (severity, "Severity", btcnew::severity_level)

boost::shared_ptr<boost::log::sinks::synchronous_sink<boost::log::sinks::text_file_backend>> btcnew::logging::file_sink;
std::atomic_flag btcnew::logging::logging_already_added ATOMIC_FLAG_INIT;

void btcnew::logging::init (boost::filesystem::path const & application_path_a)
{
	if (!logging_already_added.test_and_set ())
	{
		...
		if (!network_constants.is_test_network ())
		{
			...
			{
				static auto event_sink = boost::make_shared<boost::log::sinks::synchronous_sink<boost::log::sinks::simple_event_log_backend>> (boost::log::keywords::log_name = "Bitcoin New", boost::log::keywords::log_source = "Bitcoin New");
				event_sink->set_formatter (format);

				// Currently only mapping sys log errors
				boost::log::sinks::event_log::custom_event_type_mapping<btcnew::severity_level> mapping ("Severity");
				mapping[btcnew::severity_level::error] = boost::log::sinks::event_log::error;
				event_sink->locked_backend ()->set_event_type_mapper (mapping);

				// Only allow messages or error or greater severity to the event log
				event_sink->set_filter (severity >= btcnew::severity_level::error);
			...
			// Currently only mapping sys log errors
			boost::log::sinks::syslog::custom_severity_mapping<btcnew::severity_level> mapping ("Severity");
			mapping[btcnew::severity_level::error] = boost::log::sinks::syslog::error;
			sys_sink->locked_backend ()->set_severity_mapper (mapping);

			// Only allow messages or error or greater severity to the sys log
			sys_sink->set_filter (severity >= btcnew::severity_level::error);
...
void btcnew::logging::release_file_sink ()
{
	if (logging_already_added.test_and_set ())
	{
		boost::log::core::get ()->remove_sink (btcnew::logging::file_sink);
		btcnew::logging::file_sink.reset ();
	}
}

btcnew::error btcnew::logging::serialize_toml (btcnew::tomlconfig & toml) const
{
...
btcnew::error btcnew::logging::deserialize_toml (btcnew::tomlconfig & toml)
{
...
btcnew::error btcnew::logging::serialize_json (btcnew::jsonconfig & json) const
{
...
bool btcnew::logging::upgrade_json (unsigned version_a, btcnew::jsonconfig & json)
{
	json.put ("version", json_version ());
	switch (version_a)
	{
...
btcnew::error btcnew::logging::deserialize_json (bool & upgraded_a, btcnew::jsonconfig & json)
{
	int version_l{ 1 };
...
bool btcnew::logging::ledger_logging () const
{
	return ledger_logging_value;
}

bool btcnew::logging::ledger_duplicate_logging () const
{
	return ledger_logging () && ledger_duplicate_logging_value;
}

bool btcnew::logging::vote_logging () const
{
	return vote_logging_value;
}

bool btcnew::logging::network_logging () const
{
	return network_logging_value;
}

bool btcnew::logging::network_timeout_logging () const
{
	return network_logging () && network_timeout_logging_value;
}

bool btcnew::logging::network_message_logging () const
{
	return network_logging () && network_message_logging_value;
}

bool btcnew::logging::network_publish_logging () const
{
	return network_logging () && network_publish_logging_value;
}

bool btcnew::logging::network_packet_logging () const
{
	return network_logging () && network_packet_logging_value;
}

bool btcnew::logging::network_keepalive_logging () const
{
	return network_logging () && network_keepalive_logging_value;
}

bool btcnew::logging::network_node_id_handshake_logging () const
{
	return network_logging () && network_node_id_handshake_logging_value;
}

bool btcnew::logging::node_lifetime_tracing () const
{
	return node_lifetime_tracing_value;
}

bool btcnew::logging::insufficient_work_logging () const
{
	return network_logging () && insufficient_work_logging_value;
}

bool btcnew::logging::log_ipc () const
{
	return network_logging () && log_ipc_value;
}

bool btcnew::logging::bulk_pull_logging () const
{
	return network_logging () && bulk_pull_logging_value;
}

bool btcnew::logging::callback_logging () const
{
	return network_logging ();
}

bool btcnew::logging::work_generation_time () const
{
	return work_generation_time_value;
}

bool btcnew::logging::upnp_details_logging () const
{
	return upnp_details_logging_value;
}

bool btcnew::logging::timing_logging () const
{
	return timing_logging_value;
}

bool btcnew::logging::active_update_logging () const
{
	return active_update_value;
}

bool btcnew::logging::log_to_cerr () const
{
	return log_to_cerr_value;
}

bool btcnew::logging::single_line_record () const
{
