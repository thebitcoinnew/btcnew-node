...
// once btcnew::network is factored out of node.{c|h}pp
...
const char * default_beta_peer_network = "peering-beta.bitev.be";
const char * default_live_peer_network = "peering.bitev.be";
}

btcnew::node_config::node_config () :
node_config (0, btcnew::logging ())
{
}

btcnew::node_config::node_config (uint16_t peering_port_a, btcnew::logging const & logging_a) :
...
logging (logging_a)
{
	...
	max_work_generate_difficulty = btcnew::difficulty::from_multiplier (max_work_generate_multiplier, network_params.network.publish_threshold);
	switch (network_params.network.network ())
	{
		...
		case btcnew::btcnew_networks::btcnew_beta_network:
			...
			preconfigured_representatives.emplace_back ("259A45BDF6418D51982223E41B40B0104F859ACEBBBB71D84FB2C390718EB1B2");
			break;
...
btcnew::error btcnew::node_config::serialize_toml (btcnew::tomlconfig & toml) const
{
	...
	toml.put ("lmdb_max_dbs", lmdb_max_dbs, "Maximum open lmdb databases. Increase default if more than 100 wallets is required.\nNote: external management is recommended when a large amounts of wallets are required (see https://docs.btcnew.org/integration-guides/key-management/).\ntype:uint64");
	...
	btcnew::tomlconfig experimental_l;
...
btcnew::error btcnew::node_config::deserialize_toml (btcnew::tomlconfig & toml)
{
	try
	{
		...
		btcnew::network_constants network;
		toml.get<double> ("max_work_generate_multiplier", max_work_generate_multiplier);
		max_work_generate_difficulty = btcnew::difficulty::from_multiplier (max_work_generate_multiplier, network.publish_threshold);
		...
		if (frontiers_confirmation == btcnew::frontiers_confirmation_mode::invalid)
		{
...
btcnew::error btcnew::node_config::serialize_json (btcnew::jsonconfig & json) const
{
...
bool btcnew::node_config::upgrade_json (unsigned version_a, btcnew::jsonconfig & json)
{
	json.put ("version", json_version ());
	switch (version_a)
	{
		...
		case 2: {
			json.put ("inactive_supply", btcnew::uint128_union (0).to_string_dec ());
		...
		case 3:
			...
			json.put ("receive_minimum", btcnew::btcnew_ratio.convert_to<std::string> ());
		case 4:
			json.erase ("receive_minimum");
			json.put ("receive_minimum", btcnew::btcnew_ratio.convert_to<std::string> ());
		case 5:
		...
		case 9:
			json.put ("state_block_parse_canary", btcnew::block_hash (0).to_string ());
			json.put ("state_block_generate_canary", btcnew::block_hash (0).to_string ());
		case 10:
			json.put ("online_weight_minimum", online_weight_minimum.to_string_dec ());
		...
		case 15: {
			...
			// Update to the new preconfigured_peers url for rebrand if it is found (rai -> btcnew)
			auto peers_l (json.get_required_child (preconfigured_peers_key));
			btcnew::jsonconfig peers;
...
btcnew::error btcnew::node_config::deserialize_json (bool & upgraded_a, btcnew::jsonconfig & json)
{
	try
	{
...
std::string btcnew::node_config::serialize_frontiers_confirmation (btcnew::frontiers_confirmation_mode mode_a) const
{
	switch (mode_a)
	{
		case btcnew::frontiers_confirmation_mode::always:
			return "always";
		case btcnew::frontiers_confirmation_mode::automatic:
			return "auto";
		case btcnew::frontiers_confirmation_mode::disabled:
			return "disabled";
		default:
			return "auto";
	}
...
btcnew::frontiers_confirmation_mode btcnew::node_config::deserialize_frontiers_confirmation (std::string const & string_a)
{
	if (string_a == "always")
	{
		return btcnew::frontiers_confirmation_mode::always;
	}
	else if (string_a == "auto")
	{
		return btcnew::frontiers_confirmation_mode::automatic;
	}
	else if (string_a == "disabled")
	{
		return btcnew::frontiers_confirmation_mode::disabled;
	}
	else
	{
		return btcnew::frontiers_confirmation_mode::invalid;
	}
}

void btcnew::node_config::deserialize_address (std::string const & entry_a, std::vector<std::pair<std::string, uint16_t>> & container_a) const
{
...
btcnew::account btcnew::node_config::random_representative () const
{
	assert (!preconfigured_representatives.empty ());
	size_t index (btcnew::random_pool::generate_word32 (0, static_cast<CryptoPP::word32> (preconfigured_representatives.size () - 1)));
