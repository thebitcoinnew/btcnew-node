...
std::string btcnew::port_mapping::get_config_port (std::string const & node_port_a)
{
	return node.config.external_port != 0 ? std::to_string (node.config.external_port) : node_port_a;
}

void btcnew::port_mapping::refresh_devices ()
{
...
btcnew::endpoint btcnew::port_mapping::external_address ()
{
	btcnew::endpoint result_l (boost::asio::ip::address_v6{}, 0);
	btcnew::lock_guard<std::mutex> guard_l (mutex);
	for (auto & protocol : protocols)
	{
		if (protocol.external_port != 0)
		{
			result_l = btcnew::endpoint (protocol.external_address, protocol.external_port);
		}
	}
	return result_l;
}

void btcnew::port_mapping::refresh_mapping ()
{
	if (!network_params.network.is_test_network ())
	{
		btcnew::lock_guard<std::mutex> guard_l (mutex);
		...
		for (auto & protocol : protocols)
		{
			auto upnp_description = std::string ("Bitcoin New Node (") + network_params.network.get_current_network_as_string () + ")";
...
int btcnew::port_mapping::check_mapping ()
{
	int result_l (3600);
	if (!network_params.network.is_test_network ())
	{
...
void btcnew::port_mapping::check_mapping_loop ()
{
...
btcnew::upnp_state & btcnew::upnp_state::operator= (btcnew::upnp_state && other_a)
{
	if (this == &other_a)
	{
		return *this;
