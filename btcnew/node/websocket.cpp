...
btcnew::websocket::confirmation_options::confirmation_options (btcnew::node & node_a) :
node (node_a)
{
}

btcnew::websocket::confirmation_options::confirmation_options (boost::property_tree::ptree const & options_a, btcnew::node & node_a) :
node (node_a)
{
...
bool btcnew::websocket::confirmation_options::should_filter (btcnew::websocket::message const & message_a) const
{
	...
	if (destination_opt_l)
	{
		...
		if (all_local_accounts)
		{
			...
			btcnew::account source_l (0), destination_l (0);
...
btcnew::websocket::vote_options::vote_options (boost::property_tree::ptree const & options_a, btcnew::node & node_a) :
node (node_a)
{
...
bool btcnew::websocket::vote_options::should_filter (btcnew::websocket::message const & message_a) const
{
...
btcnew::websocket::session::session (btcnew::websocket::listener & listener_a, socket_type socket_a) :
...
btcnew::websocket::session::~session ()
{
	{
		btcnew::unique_lock<std::mutex> lk (subscriptions_mutex);
...
void btcnew::websocket::session::handshake ()
{
...
void btcnew::websocket::session::close ()
{
...
void btcnew::websocket::session::write (btcnew::websocket::message message_a)
{
	// clang-format off
	btcnew::unique_lock<std::mutex> lk (subscriptions_mutex);
	auto subscription (subscriptions.find (message_a.topic));
	if (message_a.topic == btcnew::websocket::topic::ack || (subscription != subscriptions.end () && !subscription->second->should_filter (message_a)))
	{
		lk.unlock ();
		auto this_l (shared_from_this ());
...
void btcnew::websocket::session::write_queued_messages ()
{
	auto msg (send_queue.front ().to_string ());
	auto this_l (shared_from_this ());

	// clang-format off
	ws.async_write (btcnew::shared_const_buffer (msg),
...
void btcnew::websocket::session::read ()
{
	auto this_l (shared_from_this ());
...
namespace
{
btcnew::websocket::topic to_topic (std::string const & topic_a)
{
	btcnew::websocket::topic topic = btcnew::websocket::topic::invalid;
	if (topic_a == "confirmation")
	{
		topic = btcnew::websocket::topic::confirmation;
	}
	else if (topic_a == "stopped_election")
	{
		topic = btcnew::websocket::topic::stopped_election;
	}
	else if (topic_a == "vote")
	{
		topic = btcnew::websocket::topic::vote;
	}
	else if (topic_a == "ack")
	{
		topic = btcnew::websocket::topic::ack;
	}
	else if (topic_a == "active_difficulty")
	{
		topic = btcnew::websocket::topic::active_difficulty;
	}
	else if (topic_a == "work")
	{
		topic = btcnew::websocket::topic::work;
	}

	return topic;
}

std::string from_topic (btcnew::websocket::topic topic_a)
{
	std::string topic = "invalid";
	if (topic_a == btcnew::websocket::topic::confirmation)
	{
		topic = "confirmation";
	}
	else if (topic_a == btcnew::websocket::topic::stopped_election)
	{
		topic = "stopped_election";
	}
	else if (topic_a == btcnew::websocket::topic::vote)
	{
		topic = "vote";
	}
	else if (topic_a == btcnew::websocket::topic::ack)
	{
		topic = "ack";
	}
	else if (topic_a == btcnew::websocket::topic::active_difficulty)
	{
		topic = "active_difficulty";
	}
	else if (topic_a == btcnew::websocket::topic::work)
	{
		topic = "work";
	}
	return topic;
...
void btcnew::websocket::session::send_ack (std::string action_a, std::string id_a)
{
	...
	btcnew::websocket::message msg (btcnew::websocket::topic::ack);
...
void btcnew::websocket::session::handle_message (boost::property_tree::ptree const & message_a)
{
	...
	if (action == "subscribe" && topic_l != btcnew::websocket::topic::invalid)
	{
		auto options_text_l (message_a.get_child_optional ("options"));
		btcnew::lock_guard<std::mutex> lk (subscriptions_mutex);
		std::unique_ptr<btcnew::websocket::options> options_l{ nullptr };
		if (options_text_l && topic_l == btcnew::websocket::topic::confirmation)
		{
			options_l = std::make_unique<btcnew::websocket::confirmation_options> (options_text_l.get (), ws_listener.get_node ());
		}
		else if (options_text_l && topic_l == btcnew::websocket::topic::vote)
		{
			options_l = std::make_unique<btcnew::websocket::vote_options> (options_text_l.get (), ws_listener.get_node ());
		}
		else
		{
			options_l = std::make_unique<btcnew::websocket::options> ();
		}
	...
	else if (action == "unsubscribe" && topic_l != btcnew::websocket::topic::invalid)
	{
		btcnew::lock_guard<std::mutex> lk (subscriptions_mutex);
...
void btcnew::websocket::listener::stop ()
{
	stopped = true;
	acceptor.close ();

	btcnew::lock_guard<std::mutex> lk (sessions_mutex);
...
btcnew::websocket::listener::listener (btcnew::node & node_a, boost::asio::ip::tcp::endpoint endpoint_a) :
node (node_a),
...
void btcnew::websocket::listener::run ()
{
	if (acceptor.is_open ())
	{
		accept ();
...
void btcnew::websocket::listener::accept ()
{
	auto this_l (shared_from_this ());
...
void btcnew::websocket::listener::on_accept (boost::system::error_code ec)
{
	if (ec)
	{
	...
	else
	{
		...
		auto session (std::make_shared<btcnew::websocket::session> (*this, std::move (socket)));
		sessions_mutex.lock ();
...
void btcnew::websocket::listener::broadcast_confirmation (std::shared_ptr<btcnew::block> block_a, btcnew::account const & account_a, btcnew::amount const & amount_a, std::string subtype, btcnew::election_status const & election_status_a)
{
	btcnew::websocket::message_builder builder;

	btcnew::lock_guard<std::mutex> lk (sessions_mutex);
	boost::optional<btcnew::websocket::message> msg_with_block;
	boost::optional<btcnew::websocket::message> msg_without_block;
	for (auto & weak_session : sessions)
	{
		auto session_ptr (weak_session.lock ());
		if (session_ptr)
		{
			auto subscription (session_ptr->subscriptions.find (btcnew::websocket::topic::confirmation));
			if (subscription != session_ptr->subscriptions.end ())
			{
				btcnew::websocket::confirmation_options default_options (node);
				auto conf_options (dynamic_cast<btcnew::websocket::confirmation_options *> (subscription->second.get ()));
...
void btcnew::websocket::listener::broadcast (btcnew::websocket::message message_a)
{
	btcnew::lock_guard<std::mutex> lk (sessions_mutex);
	for (auto & weak_session : sessions)
	{
...
void btcnew::websocket::listener::increase_subscriber_count (btcnew::websocket::topic const & topic_a)
{
	topic_subscriber_count[static_cast<std::size_t> (topic_a)] += 1;
}

void btcnew::websocket::listener::decrease_subscriber_count (btcnew::websocket::topic const & topic_a)
{
...
btcnew::websocket::message btcnew::websocket::message_builder::stopped_election (btcnew::block_hash const & hash_a)
{
	btcnew::websocket::message message_l (btcnew::websocket::topic::stopped_election);
...
btcnew::websocket::message btcnew::websocket::message_builder::block_confirmed (std::shared_ptr<btcnew::block> block_a, btcnew::account const & account_a, btcnew::amount const & amount_a, std::string subtype, bool include_block_a, btcnew::election_status const & election_status_a, btcnew::websocket::confirmation_options const & options_a)
{
	btcnew::websocket::message message_l (btcnew::websocket::topic::confirmation);
	set_common_fields (message_l);
...
btcnew::websocket::message btcnew::websocket::message_builder::vote_received (std::shared_ptr<btcnew::vote> vote_a)
{
	btcnew::websocket::message message_l (btcnew::websocket::topic::vote);
	set_common_fields (message_l);
...
btcnew::websocket::message btcnew::websocket::message_builder::difficulty_changed (uint64_t publish_threshold_a, uint64_t difficulty_active_a)
{
	btcnew::websocket::message message_l (btcnew::websocket::topic::active_difficulty);
	set_common_fields (message_l);

	// Active difficulty information
	boost::property_tree::ptree difficulty_l;
	difficulty_l.put ("network_minimum", btcnew::to_string_hex (publish_threshold_a));
	difficulty_l.put ("network_current", btcnew::to_string_hex (difficulty_active_a));
	auto multiplier = btcnew::difficulty::to_multiplier (difficulty_active_a, publish_threshold_a);
	difficulty_l.put ("multiplier", btcnew::to_string (multiplier));

	message_l.contents.add_child ("message", difficulty_l);
	return message_l;
}

btcnew::websocket::message btcnew::websocket::message_builder::work_generation (btcnew::block_hash const & root_a, uint64_t work_a, uint64_t difficulty_a, uint64_t publish_threshold_a, std::chrono::milliseconds const & duration_a, std::string const & peer_a, std::vector<std::string> const & bad_peers_a, bool completed_a, bool cancelled_a)
{
	btcnew::websocket::message message_l (btcnew::websocket::topic::work);
	set_common_fields (message_l);
	...
	request_l.put ("difficulty", btcnew::to_string_hex (difficulty_a));
	auto request_multiplier_l (btcnew::difficulty::to_multiplier (difficulty_a, publish_threshold_a));
	request_l.put ("multiplier", btcnew::to_string (request_multiplier_l));
	...
	if (completed_a)
	{
		...
		result_l.put ("work", btcnew::to_string_hex (work_a));
		uint64_t result_difficulty_l;
		btcnew::work_validate (root_a, work_a, &result_difficulty_l);
		result_l.put ("difficulty", btcnew::to_string_hex (result_difficulty_l));
		auto result_multiplier_l (btcnew::difficulty::to_multiplier (result_difficulty_l, publish_threshold_a));
		result_l.put ("multiplier", btcnew::to_string (result_multiplier_l));
...
btcnew::websocket::message btcnew::websocket::message_builder::work_cancelled (btcnew::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a)
{
	return work_generation (root_a, 0, difficulty_a, publish_threshold_a, duration_a, "", bad_peers_a, false, true);
}

btcnew::websocket::message btcnew::websocket::message_builder::work_failed (btcnew::block_hash const & root_a, uint64_t const difficulty_a, uint64_t const publish_threshold_a, std::chrono::milliseconds const & duration_a, std::vector<std::string> const & bad_peers_a)
{
	return work_generation (root_a, 0, difficulty_a, publish_threshold_a, duration_a, "", bad_peers_a, false, false);
}

void btcnew::websocket::message_builder::set_common_fields (btcnew::websocket::message & message_a)
{
...
std::string btcnew::websocket::message::to_string () const
{
