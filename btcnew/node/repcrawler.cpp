...
btcnew::rep_crawler::rep_crawler (btcnew::node & node_a) :
node (node_a)
{
	if (!node.flags.disable_rep_crawler)
	{
		node.observers.endpoint.add ([this] (std::shared_ptr<btcnew::transport::channel> channel_a) {
			this->query (channel_a);
		});
	}
}

void btcnew::rep_crawler::add (btcnew::block_hash const & hash_a)
{
	btcnew::lock_guard<std::mutex> lock (active_mutex);
	active.insert (hash_a);
}

void btcnew::rep_crawler::remove (btcnew::block_hash const & hash_a)
{
	btcnew::lock_guard<std::mutex> lock (active_mutex);
	active.erase (hash_a);
}

bool btcnew::rep_crawler::exists (btcnew::block_hash const & hash_a)
{
	btcnew::lock_guard<std::mutex> lock (active_mutex);
	return active.count (hash_a) != 0;
}

void btcnew::rep_crawler::start ()
{
	ongoing_crawl ();
}

void btcnew::rep_crawler::ongoing_crawl ()
{
...
std::vector<std::shared_ptr<btcnew::transport::channel>> btcnew::rep_crawler::get_crawl_targets (btcnew::uint128_t total_weight_a)
{
	...
	std::vector<std::shared_ptr<btcnew::transport::channel>> result;
	result.insert (result.end (), random_peers.begin (), random_peers.end ());
	return result;
}

void btcnew::rep_crawler::query (std::vector<std::shared_ptr<btcnew::transport::channel>> const & channels_a)
{
	auto transaction (node.store.tx_begin_read ());
	std::shared_ptr<btcnew::block> block (node.store.block_random (transaction));
...
void btcnew::rep_crawler::query (std::shared_ptr<btcnew::transport::channel> channel_a)
{
	std::vector<std::shared_ptr<btcnew::transport::channel>> peers;
	peers.push_back (channel_a);
	query (peers);
}

bool btcnew::rep_crawler::response (std::shared_ptr<btcnew::transport::channel> channel_a, btcnew::account const & rep_account_a, btcnew::amount const & weight_a)
{
	auto updated_or_inserted (false);
	btcnew::lock_guard<std::mutex> lock (probable_reps_mutex);
	auto existing (probable_reps.find (rep_account_a));
	if (existing != probable_reps.end ())
	{
		probable_reps.modify (existing, [weight_a, &updated_or_inserted, rep_account_a, channel_a] (btcnew::representative & info) {
	...
	{
		probable_reps.insert (btcnew::representative (rep_account_a, weight_a, channel_a));
		updated_or_inserted = true;
	}
	return updated_or_inserted;
}

btcnew::uint128_t btcnew::rep_crawler::total_weight () const
{
	btcnew::lock_guard<std::mutex> lock (probable_reps_mutex);
...
void btcnew::rep_crawler::on_rep_request (std::shared_ptr<btcnew::transport::channel> channel_a)
{
	btcnew::lock_guard<std::mutex> lock (probable_reps_mutex);
	...
	{
		channel_ref_index.modify (itr_pair.first, [] (btcnew::representative & value_a) {
			value_a.last_request = std::chrono::steady_clock::now ();
		});
	}
}

void btcnew::rep_crawler::cleanup_reps ()
{
	std::vector<std::shared_ptr<btcnew::transport::channel>> channels;
	{
	...
	{
		bool equal (false);
		if (i->get_type () == btcnew::transport::transport_type::tcp)
		{
			auto find_channel (node.network.tcp_channels.find_channel (i->get_tcp_endpoint ()));
			if (find_channel != nullptr && *find_channel == *static_cast<btcnew::transport::channel_tcp *> (i.get ()))
			{
				equal = true;
			}
		}
		else if (i->get_type () == btcnew::transport::transport_type::udp)
		{
			auto find_channel (node.network.udp_channels.channel (i->get_endpoint ()));
			if (find_channel != nullptr && *find_channel == *static_cast<btcnew::transport::channel_udp *> (i.get ()))
			{
				equal = true;
			}
		}
...
void btcnew::rep_crawler::update_weights ()
{
	btcnew::lock_guard<std::mutex> lock (probable_reps_mutex);
	for (auto i (probable_reps.get<tag_last_request> ().begin ()), n (probable_reps.get<tag_last_request> ().end ()); i != n;)
	{
		...
		if (weight > 0)
		{
			...
			{
				probable_reps.get<tag_last_request> ().modify (i, [weight] (btcnew::representative & info) {
					info.weight = weight;
				});
			}
			++i;
...
std::vector<btcnew::representative> btcnew::rep_crawler::representatives (size_t count_a)
{
...
std::vector<std::shared_ptr<btcnew::transport::channel>> btcnew::rep_crawler::representative_endpoints (size_t count_a)
{
	std::vector<std::shared_ptr<btcnew::transport::channel>> result;
...
size_t btcnew::rep_crawler::representative_count ()
{
	btcnew::lock_guard<std::mutex> lock (probable_reps_mutex);
