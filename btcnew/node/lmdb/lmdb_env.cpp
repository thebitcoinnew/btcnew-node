...
btcnew::mdb_env::mdb_env (bool & error_a, boost::filesystem::path const & path_a, int max_dbs_a, bool use_no_mem_init_a, size_t map_size_a)
{
	init (error_a, path_a, max_dbs_a, use_no_mem_init_a, map_size_a);
}

void btcnew::mdb_env::init (bool & error_a, boost::filesystem::path const & path_a, int max_dbs_a, bool use_no_mem_init_a, size_t map_size_a)
{
	boost::system::error_code error_mkdir, error_chmod;
	if (path_a.has_parent_path ())
	{
		boost::filesystem::create_directories (path_a.parent_path (), error_mkdir);
		btcnew::set_secure_perm_directory (path_a.parent_path (), error_chmod);
		if (!error_mkdir)
		{
...
btcnew::mdb_env::operator MDB_env * () const
{
	return environment;
}

btcnew::read_transaction btcnew::mdb_env::tx_begin_read (mdb_txn_callbacks mdb_txn_callbacks) const
{
	return btcnew::read_transaction{ std::make_unique<btcnew::read_mdb_txn> (*this, mdb_txn_callbacks) };
}

btcnew::write_transaction btcnew::mdb_env::tx_begin_write (mdb_txn_callbacks mdb_txn_callbacks) const
{
	return btcnew::write_transaction{ std::make_unique<btcnew::write_mdb_txn> (*this, mdb_txn_callbacks) };
}

MDB_txn * btcnew::mdb_env::tx (btcnew::transaction const & transaction_a) const
{
