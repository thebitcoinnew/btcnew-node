...
{
public:
	mdb_iterator (btcnew::transaction const & transaction_a, MDB_dbi db_a)
	{
	...
	mdb_iterator (btcnew::transaction const & transaction_a, MDB_dbi db_a, MDB_val const & val_a)
	{
		auto status (mdb_cursor_open (tx (transaction_a), db_a, &cursor));
		release_assert (status == 0);
		current.first = val_a;
	...
	mdb_iterator (btcnew::mdb_iterator<T, U> && other_a)
	{
	...
	mdb_iterator (btcnew::mdb_iterator<T, U> const &) = delete;
	...
	std::pair<btcnew::db_val<MDB_val>, btcnew::db_val<MDB_val>> * operator-> ()
	{
		return &current;
	}
	...
	{
		auto const other_a (boost::polymorphic_downcast<btcnew::mdb_iterator<T, U> const *> (&base_a));
	...
	void clear ()
	{
		current.first = btcnew::db_val<MDB_val> ();
		current.second = btcnew::db_val<MDB_val> ();
		assert (is_end_sentinal ());
	}

	btcnew::mdb_iterator<T, U> & operator= (btcnew::mdb_iterator<T, U> && other_a)
	{
		if (cursor != nullptr)
		{
			mdb_cursor_close (cursor);
	...
	std::pair<btcnew::db_val<MDB_val>, btcnew::db_val<MDB_val>> current;
...
{
public:
	mdb_merge_iterator (btcnew::transaction const & transaction_a, MDB_dbi db1_a, MDB_dbi db2_a) :
	impl1 (std::make_unique<btcnew::mdb_iterator<T, U>> (transaction_a, db1_a)),
	impl2 (std::make_unique<btcnew::mdb_iterator<T, U>> (transaction_a, db2_a))
	{
	}

	mdb_merge_iterator () :
	impl1 (std::make_unique<btcnew::mdb_iterator<T, U>> ()),
	impl2 (std::make_unique<btcnew::mdb_iterator<T, U>> ())
	{
	}

	mdb_merge_iterator (btcnew::transaction const & transaction_a, MDB_dbi db1_a, MDB_dbi db2_a, MDB_val const & val_a) :
	impl1 (std::make_unique<btcnew::mdb_iterator<T, U>> (transaction_a, db1_a, val_a)),
	impl2 (std::make_unique<btcnew::mdb_iterator<T, U>> (transaction_a, db2_a, val_a))
	{
	}

	mdb_merge_iterator (btcnew::mdb_merge_iterator<T, U> && other_a)
	{
		impl1 = std::move (other_a.impl1);
		impl2 = std::move (other_a.impl2);
	}

	mdb_merge_iterator (btcnew::mdb_merge_iterator<T, U> const &) = delete;
	...
	bool operator== (btcnew::store_iterator_impl<T, U> const & base_a) const override
	{
		assert ((dynamic_cast<btcnew::mdb_merge_iterator<T, U> const *> (&base_a) != nullptr) && "Incompatible iterator comparison");
		auto & other (static_cast<btcnew::mdb_merge_iterator<T, U> const &> (base_a));
	...
	btcnew::mdb_merge_iterator<T, U> & operator= (btcnew::mdb_merge_iterator<T, U> &&) = default;
	btcnew::mdb_merge_iterator<T, U> & operator= (btcnew::mdb_merge_iterator<T, U> const &) = delete;

	mutable bool from_first_database{ false };

private:
	btcnew::mdb_iterator<T, U> & least_iterator () const
	{
		btcnew::mdb_iterator<T, U> * result;
	...
	std::unique_ptr<btcnew::mdb_iterator<T, U>> impl1;
	std::unique_ptr<btcnew::mdb_iterator<T, U>> impl2;
};
}
