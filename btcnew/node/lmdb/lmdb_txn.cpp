...
{
public:
	explicit matches_txn (const btcnew::transaction_impl * transaction_impl_a) :
	transaction_impl (transaction_impl_a)
	{
	}

	bool operator() (btcnew::mdb_txn_stats const & mdb_txn_stats)
	{
...
btcnew::read_mdb_txn::read_mdb_txn (btcnew::mdb_env const & environment_a, btcnew::mdb_txn_callbacks txn_callbacks_a) :
...
btcnew::read_mdb_txn::~read_mdb_txn ()
{
...
void btcnew::read_mdb_txn::reset ()
{
	mdb_txn_reset (handle);
	txn_callbacks.txn_end (this);
}

void btcnew::read_mdb_txn::renew ()
{
...
void * btcnew::read_mdb_txn::get_handle () const
{
	return handle;
}

btcnew::write_mdb_txn::write_mdb_txn (btcnew::mdb_env const & environment_a, btcnew::mdb_txn_callbacks txn_callbacks_a) :
env (environment_a),
txn_callbacks (txn_callbacks_a)
{
	renew ();
...
btcnew::write_mdb_txn::~write_mdb_txn ()
{
	commit ();
}

void btcnew::write_mdb_txn::commit () const
{
	auto status (mdb_txn_commit (handle));
	release_assert (status == MDB_SUCCESS);
	txn_callbacks.txn_end (this);
...
void btcnew::write_mdb_txn::renew ()
{
...
void * btcnew::write_mdb_txn::get_handle () const
{
	return handle;
}

bool btcnew::write_mdb_txn::contains (btcnew::tables table_a) const
{
	// LMDB locks on every write
	return true;
}

btcnew::mdb_txn_tracker::mdb_txn_tracker (btcnew::logger_mt & logger_a, btcnew::txn_tracking_config const & txn_tracking_config_a, std::chrono::milliseconds block_processor_batch_max_time_a) :
logger (logger_a),
...
void btcnew::mdb_txn_tracker::serialize_json (boost::property_tree::ptree & json, std::chrono::milliseconds min_read_time, std::chrono::milliseconds min_write_time)
{
	...
	for (size_t i = 0; i < times_since_start.size (); ++i)
	{
		...
		{
			btcnew::jsonconfig mdb_lock_config;
			...
			btcnew::jsonconfig stack (stacktrace_config);
...
void btcnew::mdb_txn_tracker::output_finished (btcnew::mdb_txn_stats const & mdb_txn_stats) const
{
	...
	bool is_blk_processing_thread = mdb_txn_stats.thread_name == btcnew::thread_role::get_string (btcnew::thread_role::name::block_processing);
...
void btcnew::mdb_txn_tracker::add (const btcnew::transaction_impl * transaction_impl)
{
	btcnew::lock_guard<std::mutex> guard (mutex);
	// clang-format off
...
void btcnew::mdb_txn_tracker::erase (const btcnew::transaction_impl * transaction_impl)
{
	btcnew::lock_guard<std::mutex> guard (mutex);
	// clang-format off
...
btcnew::mdb_txn_stats::mdb_txn_stats (const btcnew::transaction_impl * transaction_impl) :
transaction_impl (transaction_impl),
thread_name (btcnew::thread_role::get_string ()),
stacktrace (std::make_shared<boost::stacktrace::stacktrace> ())
{
	timer.start ();
}

bool btcnew::mdb_txn_stats::is_write () const
{
	return (dynamic_cast<const btcnew::write_transaction_impl *> (transaction_impl) != nullptr);
}
