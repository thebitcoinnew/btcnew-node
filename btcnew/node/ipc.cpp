...
class session : public btcnew::ipc::socket_base, public std::enable_shared_from_this<session<SOCKET_TYPE>>
{
public:
	session (btcnew::ipc::ipc_server & server_a, boost::asio::io_context & io_ctx_a, btcnew::ipc::ipc_config_transport & config_transport_a) :
	...
	void handle_json_query (bool allow_unsafe)
	{
		...
		// This is called when btcnew::rpc_handler#process_request is done. We convert to
		...
		auto response_handler_l ([this_l, request_id_l] (std::string const & body) {
			...
			btcnew::async_write (this_l->socket, btcnew::shared_const_buffer (buffer), [this_l] (boost::system::error_code const & error_a, size_t size_a) {
				this_l->timer_cancel ();
				if (!error_a)
				{
		...
		node.stats.inc (btcnew::stat::type::ipc, btcnew::stat::detail::invocations);
		auto body (std::string (reinterpret_cast<char *> (buffer.data ()), buffer.size ()));

		// Note that if the rpc action is async, the shared_ptr<json_handler> lifetime will be extended by the action handler
		auto handler (std::make_shared<btcnew::json_handler> (node, server.node_rpc_config, body, response_handler_l, [& server = server] () {
			server.stop ();
	...
	void read_next_request ()
	{
		...
		async_read_exactly (buffer.data (), buffer.size (), std::chrono::seconds::max (), [this_l] () {
			if (this_l->buffer[btcnew::ipc::preamble_offset::lead] != 'N' || this_l->buffer[btcnew::ipc::preamble_offset::reserved_1] != 0 || this_l->buffer[btcnew::ipc::preamble_offset::reserved_2] != 0)
			{
				if (this_l->node.config.logging.log_ipc ())
				{
			...
			else if (this_l->buffer[btcnew::ipc::preamble_offset::encoding] == static_cast<uint8_t> (btcnew::ipc::payload_encoding::json_legacy) || this_l->buffer[btcnew::ipc::preamble_offset::encoding] == static_cast<uint8_t> (btcnew::ipc::payload_encoding::json_unsafe))
			{
				auto allow_unsafe (this_l->buffer[btcnew::ipc::preamble_offset::encoding] == static_cast<uint8_t> (btcnew::ipc::payload_encoding::json_unsafe));
				// Length of payload
...
private:
	...
	btcnew::timer<std::chrono::microseconds> session_timer;
	...
	btcnew::ipc::ipc_config_transport & config_transport;
};

/** Domain and TCP socket transport */
template <typename ACCEPTOR_TYPE, typename SOCKET_TYPE, typename ENDPOINT_TYPE>
class socket_transport : public btcnew::ipc::transport
{
public:
	socket_transport (btcnew::ipc::ipc_server & server_a, ENDPOINT_TYPE endpoint_a, btcnew::ipc::ipc_config_transport & config_transport_a, int concurrency_a) :
	server (server_a), config_transport (config_transport_a)
	{
		...
		if (concurrency_a > 0)
		{
			runner = std::make_unique<btcnew::thread_runner> (*io_ctx, static_cast<unsigned> (concurrency_a));
		}
	}
...
btcnew::ipc::ipc_server::ipc_server (btcnew::node & node_a, btcnew::node_rpc_config const & node_rpc_config_a) :
node (node_a),
...
btcnew::ipc::ipc_server::~ipc_server ()
{
	node.logger.always_log ("IPC: server stopped");
}

void btcnew::ipc::ipc_server::stop ()
{
