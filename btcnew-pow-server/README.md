# Bitcoin New PoW Server
[![Build Status](https://travis-ci.org/btcnewcurrency/btcnew-pow-server.svg)](https://travis-ci.org/btcnewcurrency/btcnew-pow-server)

This project is a standalone work server for [btcnew-pow](https://github.com/btcnewcurrency/btcnew-pow), the proof-of-work algorithm used by Bitcoin New.
...
```
git clone --recursive https://github.com/btcnewcurrency/btcnew-pow-server.git
cmake .
make
```
...
```
./btcnew_pow_server --config server.log_to_stderr=true
```
...
To override defaults using a config file, create a file called `btcnew-pow-server.toml` and add the required keys and values under their respective TOML table. The file name can optionally be specified with the `--config_path` option (if not specified, the working directory is searched)

**A documented configuration file can be created with the --generate_config command**

**A documented configuration file is created in the build root when the btcnew_pow_server target is created**
...
Authentication and access control is beyond the scope of the Bitcoin New PoW Server. To expose functionality, consider using a reverse proxy; this can be used to control access to the REST, WebSocket and admin interfaces.
