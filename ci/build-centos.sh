...
run_build() {
    ...
    mv -f ~/btcnew-${VERSIONS}.tar.gz ~/rpmbuild/SOURCES/.
    scl enable llvm-toolset-7 devtoolset-7 'rpmbuild -ba btcnew.spec'
    scl enable llvm-toolset-7 devtoolset-7 'rpmbuild -ba btcnew-beta.spec'
}

run_update() {
    for file in ./btcnew*.in; do
